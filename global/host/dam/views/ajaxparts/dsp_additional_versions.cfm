...
<cfelse>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="grid">
		...
		<tr>
			<td colspan="4">
				<table>
					<tr>
						<td>
							<strong>#myFusebox.getApplicationData().defaults.trans("rend_add")#</strong><br />
						...
						<td style="padding-left:15px;">
							<cfif !application.razuna.isp>
								<strong>... #myFusebox.getApplicationData().defaults.trans("rend_add_2")#</strong><br />
		...
		<cfif qry_av.recordcount NEQ 0>
			<tr>
				<cfif qry_av.av_type eq 'img'><th>#myFusebox.getApplicationData().defaults.trans("thumb")#</th></cfif>
					...
					<cfif av_type eq 'img'>
						<td>
							<cfif application.razuna.storage EQ 'local'>
								<cfset thumb_url = '//#cgi.http_host##dynpath#/assets/#session.hostid##av_thumb_url#'>
							<cfelse>
								<cfset thumb_url = '#av_thumb_url#'>
							</cfif>
							<cfif av_thumb_url NEQ ""><a href="#thumb_url#" target="_blank"><img src="#thumb_url#" height="50"></a></cfif>
						</td>
					</cfif>
					<td valign="top" nowrap="nowrap" style="width:400px"><a href="<cfif application.razuna.storage EQ "local">//#cgi.http_host##dynpath#/assets/#session.hostid##av_link_url#<cfelse>#av_link_url#</cfif>" target="_blank">#av_link_title#</a> <em>(#myFusebox.getApplicationData().global.converttomb('#thesize#')#MB<cfif av_type EQ "img" OR av_type EQ "vid">, #thewidth#x#theheight# pixel</cfif>)</em></td>
					<td valign="top" nowrap="nowrap" style="width:400px;text-decoration:underline;"><a href="<cfif application.razuna.storage EQ "local">//#cgi.http_host##dynpath#/assets/#session.hostid##av_link_url#<cfelse>#av_link_url#</cfif>" target="_blank">#myFusebox.getApplicationData().defaults.trans("click_to_view")#</a></td>
