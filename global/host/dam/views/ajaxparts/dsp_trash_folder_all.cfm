...
<cfoutput>
	...
	<cfif qry_trash.recordcount NEQ 0>
			...
			<div style="float:left;">
				...
				<button class="button" onclick="$('##rightside').load('#myself#c.trash_remove_folder&col=false');">#myFusebox.getApplicationData().defaults.trans("empty_trash")#</button>
				<!--- Restore all folders in the trash --->
				<button onclick="showwindow('#myself#c.trash_restore_folders&type=restorefolderall','#JSStringFormat(myFusebox.getApplicationData().defaults.trans("trash_restoreall"))#',650,1);return false;" class="button">#myFusebox.getApplicationData().defaults.trans("trash_restoreall")#</button>
			</div>
		...
		<div style="float:right;">
			<cfif session.trash_folder_offset GTE 1>
				...
				<a href="##" onclick="loadcontent('folders','#myself#c.trash_folder_all&trashkind=folders&offset=#newoffset#');">&lt; #myFusebox.getApplicationData().defaults.trans("back")#</a> |
			</cfif>
			...
			<cfif qry_trash.recordcount GT session.trash_folder_rowmaxpage AND NOT shownextrecord GTE qry_trash.recordcount> | 
				<!--- For Next --->
				<cfset newoffset = session.trash_folder_offset + 1>
				<a href="##" onclick="loadcontent('folders','#myself#c.trash_folder_all&trashkind=folders&offset=#newoffset#');">#myFusebox.getApplicationData().defaults.trans("next")# &gt;</a>
			</cfif>
						...
						<div class="assetbox">
							...
							<div style="float:left;padding-top:5px;">
								<div style="float:left;padding-top:2px;">
									<input type="checkbox" name="file_id" value="#id#-#kind#" onclick="enablesub('allform_folders');"<cfif listfindnocase(session.file_id,"#id#-img") NEQ 0> checked="checked"</cfif>>
								</div>
