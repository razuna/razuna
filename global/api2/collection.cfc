...
<cfcomponent output="false" extends="authentication">
	...
	<cffunction name="getassets" access="remote" output="false" returntype="query" returnformat="json">
		...
		<cfif thesession>
			...
			<cfquery datasource="#application.razuna.datasource#" name="colfolder">
			SELECT folder_id_r
			...
			<cfif folderaccess EQ "R"  OR folderaccess EQ "W" OR folderaccess EQ "X">
				...
				<cfquery datasource="#application.razuna.datasource#" name="qry_col" cachedwithin="1" region="razcache">
				...
				<cfif qry_col.recordcount NEQ 0>
					...
					<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
						...
						concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',i.path_to_asset,'/','thumb_',i.img_id,'.',i.thumb_extension) AS local_url_thumb,
					<cfelseif application.razuna.thedatabase EQ "mssql">
						...
						'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + i.path_to_asset + '/' + 'thumb_' + i.img_id + '.' + i.thumb_extension AS local_url_thumb,
					</cfif>
					CASE
					...
					CASE
						WHEN (ct.col_file_format = 'original' OR ct.col_file_format = 'thumb') THEN
							<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
								concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',i.path_to_asset,'/',i.img_filename_org)
							<cfelseif application.razuna.thedatabase EQ "mssql">
								'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + i.path_to_asset + '/' + i.img_filename_org
							</cfif>
						ELSE
							<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
								concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',(SELECT path_to_asset FROM #session.hostdbprefix#images WHERE img_id = ct.col_file_format),'/',(SELECT img_filename_org FROM #session.hostdbprefix#images WHERE img_id = ct.col_file_format))
							<cfelseif application.razuna.thedatabase EQ "mssql">
								'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + (SELECT path_to_asset FROM #session.hostdbprefix#images WHERE img_id = ct.col_file_format) + '/' + (SELECT img_filename_org FROM #session.hostdbprefix#images WHERE img_id = ct.col_file_format)
							</cfif>
					...
					LEFT JOIN #session.hostdbprefix#collections_ct_files ct ON ct.file_id_r = i.img_id AND ct.col_id_r = '#arguments.collectionid#'
					...
					CASE
						WHEN (ct.col_file_format = 'video') THEN
							<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
								concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',v.path_to_asset,'/',v.vid_name_org)
							<cfelseif application.razuna.thedatabase EQ "mssql">
								'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + v.path_to_asset + '/' + v.vid_name_org
							</cfif>
						ELSE
							<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
								concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',(SELECT path_to_asset FROM #session.hostdbprefix#videos WHERE vid_id = ct.col_file_format),'/',(SELECT vid_name_org FROM #session.hostdbprefix#videos WHERE vid_id = ct.col_file_format))
							<cfelseif application.razuna.thedatabase EQ "mssql">
								'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + (SELECT path_to_asset FROM #session.hostdbprefix#videos WHERE vid_id = ct.col_file_format) + '/' + (SELECT vid_name_org FROM #session.hostdbprefix#videos WHERE vid_id = ct.col_file_format)
							</cfif>
					END as rendition_url,
					'' AS colorspace,
					'' AS xdpi,
					...
					LEFT JOIN #session.hostdbprefix#collections_ct_files ct ON ct.file_id_r = v.vid_id AND ct.col_id_r = '#arguments.collectionid#'
					...
					CASE
						WHEN (ct.col_file_format = '' OR ct.col_file_format = 'audio') THEN
							<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
								concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',a.path_to_asset,'/',a.aud_name_org)
							<cfelseif application.razuna.thedatabase EQ "mssql">
								'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + a.path_to_asset + '/' + a.aud_name_org
							</cfif>
						ELSE
							<cfif application.razuna.thedatabase EQ "oracle" OR application.razuna.thedatabase EQ "mysql" OR application.razuna.thedatabase EQ "h2">
								concat('#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/',(SELECT path_to_asset FROM #session.hostdbprefix#audios WHERE aud_id = ct.col_file_format),'/',(SELECT aud_name_org FROM #session.hostdbprefix#audios WHERE aud_id = ct.col_file_format))
							<cfelseif application.razuna.thedatabase EQ "mssql">
								'#application.razuna.thehttp##cgi.HTTP_HOST#/#application.razuna.dynpath#/assets/#session.hostid#/' + (SELECT path_to_asset FROM #session.hostdbprefix#audios WHERE aud_id = ct.col_file_format) + '/' + (SELECT aud_name_org FROM #session.hostdbprefix#audios WHERE aud_id = ct.col_file_format)
							</cfif>
					END as rendition_url,
					'' AS colorspace,
					'' AS xdpi,
					...
					LEFT JOIN #session.hostdbprefix#collections_ct_files ct ON ct.file_id_r = a.aud_id AND ct.col_id_r = '#arguments.collectionid#'
	...
	<cffunction name="getcollections" access="remote" output="false" returntype="query" returnformat="json">
		...
		<cfif thesession>
			...
			<cfif folderaccess EQ "R" OR folderaccess EQ "W" OR folderaccess EQ "X">
				...
				LEFT JOIN #session.hostdbprefix#collections_text ct ON c.col_id = ct.col_id_r AND ct.lang_id_r = <cfqueryparam value="1" cfsqltype="cf_sql_numeric">
