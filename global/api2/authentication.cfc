...
<cfcomponent output="false">
	...
	<cffunction name="checkFolderPerm" access="public" output="no">
		...
		<cfelse>
			...
			FROM #application.razuna.api.prefix["#api_key#"]#images a, #application.razuna.api.prefix["#api_key#"]#folders f LEFT JOIN #application.razuna.api.prefix["#api_key#"]#folders_groups fg ON f.folder_id = fg.folder_id_r AND fg.host_id = f.host_id
			...
			UNION ALL
			SELECT a.folder_id_r, f.folder_owner, fg.grp_id_r, fg.grp_permission
			FROM #application.razuna.api.prefix["#api_key#"]#videos a, #application.razuna.api.prefix["#api_key#"]#folders f LEFT JOIN #application.razuna.api.prefix["#api_key#"]#folders_groups fg ON f.folder_id = fg.folder_id_r AND fg.host_id = f.host_id
			...
			UNION ALL
			SELECT a.folder_id_r, f.folder_owner, fg.grp_id_r, fg.grp_permission
			FROM #application.razuna.api.prefix["#api_key#"]#audios a, #application.razuna.api.prefix["#api_key#"]#folders f LEFT JOIN #application.razuna.api.prefix["#api_key#"]#folders_groups fg ON f.folder_id = fg.folder_id_r AND fg.host_id = f.host_id
			...
			UNION ALL
			SELECT a.folder_id_r, f.folder_owner, fg.grp_id_r, fg.grp_permission
			FROM #application.razuna.api.prefix["#api_key#"]#files a, #application.razuna.api.prefix["#api_key#"]#folders f LEFT JOIN #application.razuna.api.prefix["#api_key#"]#folders_groups fg ON f.folder_id = fg.folder_id_r AND fg.host_id = f.host_id
	...
	<cffunction hint="Get Folder Access" name="checkFolderAccess" output="true" returntype="string">
		...
		<cfelse>
			...
			FROM  #application.razuna.api.prefix["#api_key#"]#folders f LEFT JOIN  #application.razuna.api.prefix["#api_key#"]#folders_groups fg ON f.folder_id = fg.folder_id_r AND f.host_id = fg.host_id
	...
	<cffunction hint="Get Label Permissions" name="checkLabelPerm" output="true" returntype="string">
		...
		<cfelse>
			...
			SELECT 1
				FROM ct_labels l
				LEFT JOIN #application.razuna.api.prefix["#api_key#"]#images i ON l.ct_id_r = i.img_id AND  l.ct_type =<cfqueryparam value="img" cfsqltype="cf_sql_varchar"/>
				LEFT JOIN #application.razuna.api.prefix["#api_key#"]#audios a ON l.ct_id_r = a.aud_id  AND  l.ct_type =<cfqueryparam value="aud" cfsqltype="cf_sql_varchar"/>
				LEFT JOIN #application.razuna.api.prefix["#api_key#"]#videos v ON l.ct_id_r = v.vid_id  AND  l.ct_type =<cfqueryparam value="vid" cfsqltype="cf_sql_varchar"/>
				LEFT JOIN #application.razuna.api.prefix["#api_key#"]#files f ON l.ct_id_r = f.file_id  AND  l.ct_type =<cfqueryparam value="doc" cfsqltype="cf_sql_varchar"/>
				LEFT JOIN #application.razuna.api.prefix["#api_key#"]#folders fo ON l.ct_id_r = fo.folder_id  AND  l.ct_type =<cfqueryparam value="folder" cfsqltype="cf_sql_varchar"/>
				WHERE
				...
				EXISTS (
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders WHERE folder_id =  i.folder_id_r AND folder_owner = '#session.theuserid#' 
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders WHERE folder_id =  a.folder_id_r AND folder_owner = '#session.theuserid#' 
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders WHERE folder_id =  v.folder_id_r AND folder_owner = '#session.theuserid#' 
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders WHERE folder_id =  f.folder_id_r AND folder_owner = '#session.theuserid#' 
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders WHERE folder_id =  fo.folder_id_r AND folder_owner = '#session.theuserid#' 
					) 
				OR
				...
				EXISTS (
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders_groups f WHERE i.folder_id_r = f.folder_id_r AND  f.grp_id_r ='0' AND lower(f.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders_groups f WHERE  a.folder_id_r = f.folder_id_r AND f.grp_id_r ='0' AND lower(f.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders_groups f WHERE  v.folder_id_r = f.folder_id_r AND f.grp_id_r = '0' AND lower(f.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders_groups fg WHERE  f.folder_id_r = fg.folder_id_r AND fg.grp_id_r = '0' AND lower(fg.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM #application.razuna.api.prefix["#api_key#"]#folders_groups fg WHERE  fo.folder_id = fg.folder_id_r AND fg.grp_id_r = '0' AND lower(fg.grp_permission) IN  ('w','x')
					)
				OR
				...
				EXISTS (
					SELECT 1 FROM ct_groups_users c, #application.razuna.api.prefix["#api_key#"]#folders_groups f WHERE ct_g_u_user_id ='#session.theuserid#' AND i.folder_id_r = f.folder_id_r AND f.grp_id_r = c.ct_g_u_grp_id AND lower(f.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM ct_groups_users c, #application.razuna.api.prefix["#api_key#"]#folders_groups f WHERE ct_g_u_user_id ='#session.theuserid#' AND a.folder_id_r = f.folder_id_r AND f.grp_id_r = c.ct_g_u_grp_id AND lower(f.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM ct_groups_users c, #application.razuna.api.prefix["#api_key#"]#folders_groups f WHERE ct_g_u_user_id ='#session.theuserid#' AND v.folder_id_r = f.folder_id_r AND f.grp_id_r = c.ct_g_u_grp_id  AND lower(f.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM ct_groups_users c, #application.razuna.api.prefix["#api_key#"]#folders_groups fg WHERE ct_g_u_user_id ='#session.theuserid#' AND f.folder_id_r = fg.folder_id_r AND fg.grp_id_r = c.ct_g_u_grp_id AND lower(fg.grp_permission) IN  ('w','x')
					UNION
					SELECT 1 FROM ct_groups_users c, #application.razuna.api.prefix["#api_key#"]#folders_groups fg WHERE ct_g_u_user_id ='#session.theuserid#' AND fo.folder_id = fg.folder_id_r AND fg.grp_id_r = c.ct_g_u_grp_id AND lower(fg.grp_permission) IN  ('w','x')
					)
				)
			</cfquery>
