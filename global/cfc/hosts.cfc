...
<cffunction name="remove" output="false" access="public">
	...
	<cfif qry_rhost.recordcount EQ 1>
		<cftry>
			...
			LEFT JOIN ct_users_hosts ct ON ct.ct_u_h_host_id = #arguments.thestruct.id# and ct.ct_u_h_user_id
			WHERE EXISTS (SELECT ct_g_u_grp_id FROM ct_groups_users WHERE ct_g_u_user_id = u.user_id AND ct_g_u_grp_id != '1')
			AND 1 IN (SELECT count(cts.ct_u_h_host_id) FROM ct_users_hosts cts WHERE cts.ct_u_h_user_id = u.user_id)
			</cfquery>
