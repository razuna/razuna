...
<cffunction name="getFolderAssets" access="public" description="GET ALL RECORDS OF THIS TYPE IN A FOLDER" output="false" returntype="query">
	...
	<cfif request.razuna.session.showsubfolders EQ "T">
		<cfinvoke component="folders" method="getfoldersinlist" dsn="#request.razuna.application.datasource#" folder_id="#arguments.folder_id#" database="#request.razuna.application.thedatabase#" hostid="#request.razuna.session.hostid#" returnvariable="thefolders">
	...
	<cfelse>
		...
		AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#videos WHERE vid_id = c.asset_id_r AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
		</cfquery>
	...
	<cfif request.razuna.session.customfileid NEQ "">
		...
		SELECT *
		FROM qLocal
		WHERE vid_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfquery>
	</cfif>
...
<cffunction  name="showvideo" output="true">
	...
	<!--- Storage Decision --->
	<cfset var thestorage = "#request.razuna.session.thehttp##cgi.http_host##arguments.thestruct.dynpath#/assets/#request.razuna.session.hostid#/">
	<cfset var thestoragefullpath = "#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/">
...
<cffunction name="create_previews" output="true">
	...
	<cfif !request.razuna.application.rfs>
		<cftry>
			...
			<cfif arguments.thestruct.importpath AND request.razuna.application.storage EQ "local">
...
<cffunction  name="removevideo" output="true">
	...
	<cfif thedetail.recordcount NEQ 0>
		...
		DELETE FROM #request.razuna.session.hostdbprefix#videos
		WHERE vid_id = <cfqueryparam value="#arguments.thestruct.id#" cfsqltype="CF_SQL_VARCHAR">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		</cfquery>
		<cfquery datasource="#request.razuna.application.datasource#">
		DELETE FROM #request.razuna.session.hostdbprefix#videos_text
...
<cffunction name="gettrashvideos" output="false" returntype="Query">
	...
	<cfset var cachetoken = getcachetoken(type="videos", hostid=request.razuna.session.hostid)>
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_video" cachedwithin="#CreateTimeSpan(0,0,5,0)#" region="razcache">
...
<cffunction name="deletefromfilesystem" output="false">
	...
	<cftry>
		...
		<cfif request.razuna.application.storage EQ "local">
			...
			<!--- Versions --->
			<cfif DirectoryExists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/versions/vid/#arguments.thestruct.id#") AND arguments.thestruct.id NEQ "">
				<cfdirectory action="delete" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/versions/vid/#arguments.thestruct.id#" recurse="true">
			</cfif>
		<!--- Nirvanix --->
...
<cffunction name="detail" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#videos v
	LEFT JOIN #request.razuna.session.hostdbprefix#settings_2 s ON s.set2_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.application.setid#"> AND s.host_id = v.host_id
	LEFT JOIN users u ON u.user_id = v.vid_owner
	LEFT JOIN #request.razuna.session.hostdbprefix#folders fo ON fo.folder_id = v.folder_id_r AND fo.host_id = v.host_id
...
<cffunction name="updatethread" output="false">
	...
	<cfloop list="#arguments.thestruct.file_id#" delimiters="," index="i">
		...
		<cfif qryorg.recordcount neq 0>
			...
			<cfif request.razuna.application.storage EQ "local">
				<!--- MD5 video --->
				<cfif FileExists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#qryorg.path_to_asset#/#qryorg.vid_name_org#")>
					<cfset var md5hash = hashbinary("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#qryorg.path_to_asset#/#qryorg.vid_name_org#")>
					<!--- Update DB --->
					<cfquery datasource="#request.razuna.application.datasource#">
					UPDATE #request.razuna.session.hostdbprefix#videos
			...
			<cfset log_assets(theuserid=request.razuna.session.theuserid,logaction='Update',logdesc='#updated##rend#: #qryorg.vid_filename#',logfiletype='vid',assetid='#theid#',folderid='#arguments.thestruct.folder_id#', hostid=request.razuna.session.hostid)>
		<cfelse>
...
<cffunction name="convertvideo" output="true">
	...
	<cftry>
		...
		<cfif request.razuna.application.storage EQ "local">
			<!--- Set the input path --->
			<cfset var inputpath = "#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thestruct.qrydetail.path_to_asset#/#arguments.thestruct.qrydetail.vid_name_org#">
			<!--- Set the input path for the still image --->
			<cfset var inputpathimage = "#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thestruct.qrydetail.path_to_asset#/#arguments.thestruct.qrydetail.vid_name_image#">
		...
		<cfloop delimiters="," list="#arguments.thestruct.convert_to#" index="theformat">
			...
			<cfelse>
				...
				<cfif request.razuna.application.storage EQ "local">
					...
					<!--- Create folder first --->
					<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thestruct.qrydetail.folder_id_r#/vid/#arguments.thestruct.newid#" mode="775">
					<!--- Move video --->
					<cffile action="#_file_action#" source="#thisfolder#/#previewvideo#" destination="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thestruct.qrydetail.folder_id_r#/vid/#arguments.thestruct.newid#" mode="775">
					<!--- Move still image --->
					<cffile action="#_file_action#" source="#thisfolder#/#previewimage#" destination="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thestruct.qrydetail.folder_id_r#/vid/#arguments.thestruct.newid#" mode="775">
				...
				<cfif arguments.thestruct.save_renditions>
					...
					<!--- If UPC is enabled then rename rendition according to UPC naming convention --->
					 <cfif upcstruct.upcenabled>
						...
						<cfquery datasource="#request.razuna.application.datasource#" name="get_upc">
							SELECT vid_upc_number as upcnumber FROM  #request.razuna.session.hostdbprefix#videos
							WHERE vid_id =
					...
					<cfif structKeyExists(arguments.thestruct,'option_rendition_meta') AND arguments.thestruct.option_rendition_meta EQ 'true'>
						...
						<cfquery datasource="#request.razuna.application.datasource#" name="qry_thevidtxt">
...
<cffunction name="writevideo" output="true">
	...
	<cfloop delimiters="," list="#request.razuna.session.artofimage#" index="art">
		...
		FROM #request.razuna.session.hostdbprefix#videos v, #request.razuna.session.hostdbprefix#settings_2 s
...
<cffunction name="move" output="false">
	<cfargument name="thestruct" type="struct">
		<cftry>
			...
			<cfinvoke component="global" method="getAlias" asset_id_r="#arguments.thestruct.vid_id#" folder_id_r="#request.razuna.session.thefolderorg#" returnvariable="qry_alias" />
			<!--- If this is an alias --->
			<cfif qry_alias>
				<!--- Move alias --->
				<cfinvoke component="global" method="moveAlias" asset_id_r="#arguments.thestruct.vid_id#" new_folder_id_r="#arguments.thestruct.folder_id#" pre_folder_id_r="#request.razuna.session.thefolderorg#" />
			<cfelse>
				...
				<cfif arguments.thestruct.qryvid.recordcount NEQ 0 AND arguments.thestruct.folder_id NEQ arguments.thestruct.qryvid.folder_id_r>
					...
					<cfset log_assets(theuserid=request.razuna.session.theuserid,logaction='Move',logdesc='#moved#: #arguments.thestruct.qryvid.vid_filename#',logfiletype='vid',assetid=arguments.thestruct.vid_id,folderid='#arguments.thestruct.folder_id#', hostid=request.razuna.session.hostid)>
				</cfif>
			</cfif>
			<cfcatch type="any">
			</cfcatch>
...
<cffunction name="copymetadataupdate" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="thevidtext">
		...
		<cfloop list="#arguments.thestruct.idlist#" index="i">
			<cfloop query = "thevidtext">
				...
				<cfif checkid.RecordCount>
					<cfquery datasource="#request.razuna.application.datasource#" name="updatevidtext">
						UPDATE #request.razuna.session.hostdbprefix#videos_text
	...
	<cfelse>
		<cfloop list="#arguments.thestruct.idlist#" index="i">
			<!--- <cfquery datasource="#request.razuna.application.datasource#" name="theviddetail">
				SELECT vid_filename
				FROM #request.razuna.session.hostdbprefix#videos
			...
			<cfloop query ="thevidtext">
				<cfquery datasource="#request.razuna.application.datasource#" name="thevidtextdetail">
