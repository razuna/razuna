...
<cffunction name="init" returntype="folders" access="public" output="false">
	<cfreturn this />
</cffunction>
...
<cffunction  name="getTree" output="false" access="public" returntype="query">
	...
	<cfreturn getTreeByCollection(id=Arguments.id, max_level_depth=Arguments.max_level_depth, ColumnList=Arguments.ColumnList, thestruct=arguments.thestruct) />
</cffunction>
...
<cffunction name="getTreeByCollection" output="false" access="public" returntype="query">
	...
	<!--- Do the select --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="f_1" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken##arguments.thestruct.razuna.session.theUserID#getTreeByCollection */ #Arguments.ColumnList#,
		<!--- Permission follow but not for sysadmin and admin --->
		<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
			CASE
				<!--- If this folder is protected with a group and this user belongs to this group --->
				WHEN EXISTS(
					SELECT fg.folder_id
					FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg, ct_groups_users gu
					WHERE fg.folder_id_r = f.folder_id
					AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
				...
				WHEN EXISTS(
					...
					FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg2 LEFT JOIN ct_groups_users gu2 ON gu2.ct_g_u_grp_id = fg2.grp_id_r AND gu2.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
				...
				WHEN ( f.folder_of_user = 't' AND f.folder_owner = '#arguments.thestruct.razuna.session.theUserID#' ) THEN 'unlocked'
		...
		<cfelse>
			CASE
				WHEN ( f.folder_of_user = 't' AND f.folder_owner = '#arguments.thestruct.razuna.session.theUserID#' AND f.folder_name = 'my folder') THEN 'unlocked'
	...
	<cfif not arguments.thestruct.razuna.session.is_system_admin>
		AND
			(
			<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2">NVL<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mssql">isnull</cfif>(f.folder_of_user,<cfqueryparam cfsqltype="cf_sql_varchar" value="f">) <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
			OR f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">
			)
	</cfif>
...
<cffunction name="getfolder" output="false" access="public" description="GET FOLDER RECORD" returntype="query">
	...
	<cfif structKeyExists(arguments,'avoid_link_path') AND arguments.avoid_link_path EQ 'yes'>
		AND (f.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR f.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR f.link_path IS NULL)
	</cfif>
	...
	<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
		AND (
			...
			EXISTS(
				...
				WHERE fg.folder_id_r = f.folder_id
				AND fg.grp_permission IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="r,w,x" list="true">)
				AND fg.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				AND (
					<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2">
						NVL(fg.grp_id_r, 0)
			...
			NOT EXISTS(
				...
				WHERE gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
				AND fg.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
...
<cffunction name="getfolderproperties" output="false" access="public" description="GET FOLDER RECORD" returntype="query">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfquery name="qLocal" datasource="#arguments.thestruct.razuna.application.datasource#" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getfolderproperties */ f.folder_id, f.folder_id_r, f.folder_name, f.folder_level, f.folder_of_user,
...
<cffunction  name="getfolderdesc" output="false">
	<cfargument name="folder_id" required="yes" type="string">
	<cfargument name="thestruct" type="struct" required="true" />
	<cfset var qry = "">
	<!--- Get the cachetoken for here --->
	...
	SELECT /* #cachetoken#getfolderdesc */ folder_desc, lang_id_r
	FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_desc
...
<cffunction name="getfoldernames" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getfoldernames */ lang_id_r, folder_name
	...
	<cfif qry.recordcount EQ 0>
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getfoldernames */ '1' AS lang_id_r, folder_name
...
<cffunction  name="getfoldergroups" output="false">
	<cfargument name="folder_id" default="" required="yes" type="string">
	<cfargument name="qrygroup" required="yes" type="query">
	<cfargument name="in_folder_group" required="no" type="string" default="no" >
	<cfargument name="thestruct" type="struct" required="true" />
	<!--- Set --->
	...
	<cfif arguments.qrygroup.recordcount NEQ 0>
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thegroups" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getfoldergroups */ grp_id_r, grp_permission
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_groups
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		AND grp_id_r IN (
...
<cffunction  name="getfoldergroupszero" output="false">
	<cfargument name="folder_id" default="" required="yes" type="string">
	<cfargument name="thestruct" type="struct" required="true" />
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thegroups" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getfoldergroupszero */ grp_id_r, grp_permission
	FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_groups
	WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
...
<!--- ------------------------------------------------------------------------------------- --->
<!--- THE FOLDER LIST --->
<cffunction  name="folderlist" output="false" access="public" returntype="query">
	<cfargument name="folder_id" required="yes" type="string">
	<cfargument name="ignoreCollections" required="no" type="boolean" default="0">
	<cfargument name="onlyCollections" required="no" type="boolean" default="0">
	<cfargument name="thestruct" type="struct" required="true" />
...
<!--- ------------------------------------------------------------------------------------- --->
<!--- ADD A NEW FOLDER --->
<cffunction name="add" output="true" returntype="string">
	<cfargument name="thefolderparam" required="no" type="struct" default="#StructNew()#" >
	<cfargument name="formStruct" required="no" type="struct" default="#Form#" >
	<cfargument name="noTransaction" required="no" type="boolean" default="0" >
	<cfargument name="thestruct" type="struct" required="true" />
	<!--- Params --->
	...
	<cfif arguments.thestruct.link_path EQ "">
		<cftry>
			...
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="samefolder">
			SELECT folder_id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
			...
			<cfif samefolder.recordcount EQ 0>
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "local" OR arguments.thestruct.razuna.application.storage EQ "akamai">
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/vid")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/vid" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/doc")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/doc" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/aud")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/aud" mode="775">
					</cfif>
				</cfif>
				<!--- Check on any plugin that call the on_folder_add action --->
				<cfset arguments.thestruct.folder_id = newfolderid>
				<cfinvoke component="plugins" method="getactions" theaction="on_folder_add" args="#arguments.thestruct#" thestruct="#arguments.thestruct#" />
...
<cffunction name="fnew_detail" output="true" returntype="string" access="public">
		...
		<cfloop list="#arguments.thestruct.langcount#" index="langindex">
			...
			<cfif structkeyexists(arguments.thestruct, "folder_name_#langindex#")>
				...
				<cfif thisfolder CONTAINS langindex AND evaluate(thisfolder) NEQ "">
					<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
					INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#folders_name
	...
	<cfif structKeyExists(arguments.thestruct,'theid') AND arguments.thestruct.theid NEQ ''>
		...
		<cfinvoke component="labels" method="getlabels" theid="#arguments.thestruct.theid#" thetype="folder" checkUPC="true" returnvariable="arguments.thestruct.qry_labels" thestruct="#arguments.thestruct#">
	...
	<cfinvoke method="apply_custom_shared_setting" folder_id="#newfolderid#" thestruct="#arguments.thestruct#" />
	<!--- Log --->
...
<cffunction name="apply_custom_shared_setting" output="false" returntype="void">
	...
	AND subf.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	</cfquery>
...
<cffunction name="remove_folder_thread" output="false">
	...
	<cftry>
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="foldername">
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="parentid">
...
<cffunction name="trash_folder_thread" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="get_folder">
...
<cffunction name="_updateFilesInTrash" access="remote" output="false">
	...
	<cfloop query="arguments.qry_files">
		...
		<!--- Update record --->
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
		UPDATE #arguments.thestruct.razuna.session.hostdbprefix##_db#
		SET
...
<cffunction name="gettrashfolder" output="false" returntype="Query">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#gettrashfolder */
	f.folder_id AS id,
	...
	<cfelse>
		...
		CASE
			...
			WHEN (f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">) THEN 'X'
		END as permfolder
	</cfif>
	FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f
	...
	<cfif qry.RecordCount NEQ 0>
		...
		<cfloop query="qry">
			...
			<cfquery name="getColfolderIDs" datasource="#arguments.thestruct.razuna.application.datasource#" >
			SELECT folder_id from #arguments.thestruct.razuna.session.hostdbprefix#folders
			WHERE folder_level >= (
				SELECT
				folder_level
				FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
			...
			<cfif !listlen(folderIDs) AND folderIDs NEQ "">
				<cfinvoke method="getAssetsDetails" folder_id="#folderIDs#" returnvariable="flag" thestruct="#arguments.thestruct#">
...
<cffunction name="trash_file_values_thread" output="false">
	...
	<!--- trash images --->
	<cfthread intstruct="#arguments.thestruct#">
		<cfinvoke component="images" method="gettrashimage" returnvariable="imagetrash" noread="true" nocount="true" thestruct="#attributes.intstruct#" />
	...
	<!--- trash audios --->
	<cfthread intstruct="#arguments.thestruct#">
		<cfinvoke component="audios" method="gettrashaudio" returnvariable="audiotrash"  noread="true" nocount="true" thestruct="#attributes.intstruct#" />
	...
	<!--- trash files --->
	<cfthread intstruct="#arguments.thestruct#">
		<cfinvoke component="files" method="gettrashfile" returnvariable="filetrash"  noread="true" nocount="true" thestruct="#attributes.intstruct#" />
	...
	<!--- trash videos --->
	<cfthread intstruct="#arguments.thestruct#">
		<cfinvoke component="videos" method="gettrashvideos" returnvariable="videotrash"  noread="true" nocount="true" thestruct="#attributes.intstruct#" />
	...
	<cfset arguments.thestruct.razuna.session.thefileid = ids> --->
	<!--- Flush Cache --->
	<!--- <cfset resetcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfset resetcachetoken(type="images", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfset resetcachetoken(type="videos", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
...
<cffunction name="trash_folder_values" output="false">
	...
	<cfinvoke component="folders" method="gettrashfolder" thestruct="#arguments.thestruct#" returnvariable="qry_trash" noread="true"/>
	<!--- set folder id --->
...
<cffunction name="restoreselectedfolders" output="false">
	<cfargument name="thestruct" type="struct">
	<!--- Get the details --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thenewrootid">
		SELECT folder_id_r,folder_main_id_r,folder_level,folder_name FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
...
<cffunction name="restorefolder" output="false">
	...
	<!--- get the details --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thenewrootid">
	SELECT folder_id_r,folder_main_id_r,folder_level FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
	WHERE folder_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.intofolderid#">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	</cfquery>
	<!--- Get the Folder Name/Folder Level for the Log --->
...
<cffunction name="deleteassetsinfolder" output="false">
	...
	<!--- Images --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryimg">
	Select img_id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#images
	...
	<!--- Videos --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryvid">
	Select vid_id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#videos
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	...
	<!--- Audios --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryaud">
	Select aud_id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#audios
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	...
	<!--- Docs --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qrydoc">
	Select file_id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#files
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	...
	<!--- Collections --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qrycol">
	Select col_id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	</cfquery>
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryfolder">
	SELECT img_id as id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#images
	...
	<cfif qryfolder.recordcount EQ 0>
		<!--- Delete Folder --->
		<cfif arguments.thestruct.razuna.application.storage EQ "local" OR arguments.thestruct.razuna.application.storage EQ "akamai">
			<cfif isdefined("arguments.thestruct.assetpath") AND directoryexists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#arguments.thefolderid#")>
				<cfdirectory action="delete" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#arguments.thefolderid#" recurse="true">
			</cfif>
		...
		<cfelseif arguments.thestruct.razuna.application.storage EQ "amazon">
			<cfinvoke component="amazon" method="deletefolder" folderpath="#arguments.thefolderid#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
		</cfif>
		<!--- Execute plugins --->
		<cfset arguments.thestruct.folder_id = arguments.thefolderid>
		<cfset arguments.thestruct.folder_action = false>
		<cfinvoke component="plugins" method="getactions" theaction="on_folder_remove" args="#arguments.thestruct#" thestruct="#arguments.thestruct#" />
	</cfif>
	<!--- Flush Cache --->
	<cfset resetcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfset resetcachetoken(type="images", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
...
<cffunction name="update" output="true" returntype="string">
	...
	WHERE folder_name = <cfqueryparam value="#arguments.thestruct.folder_name#" cfsqltype="cf_sql_varchar">
	AND folder_level = <cfqueryparam value="#arguments.thestruct.level#" cfsqltype="cf_sql_numeric">
	AND folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	AND folder_of_user <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="t" cfsqltype="cf_sql_varchar">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	</cfquery>
	...
	<cfif samefolder.recordcount EQ 0>
		...
		<cfloop list="#arguments.thestruct.langcount#" index="langindex">
			...
			<cfif thisfolder CONTAINS langindex AND evaluate(thisfolder) NEQ "">
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="langFolder">
				SELECT folder_id_r
				FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_name
				WHERE folder_id_r = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
			...
			<cfif thisfield CONTAINS langindex>
				<!--- Check if description in this language exists --->
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="langDesc">
				SELECT folder_id_r
				FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_desc
				WHERE folder_id_r = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
				AND lang_id_r = <cfqueryparam value="#langindex#" cfsqltype="cf_sql_numeric">
		...
		<cfloop delimiters="," index="myform" list="#arguments.thestruct.fieldnames#">
			<cfif myform CONTAINS "grp_">
				...
				<cfinvoke component="global.cfc.groups" method="add_grp_users2notify" group_id='#grpid#' thestruct="#arguments.thestruct#">
			</cfif>
		</cfloop>
		...
		<cfif structkeyexists(arguments.thestruct,"perm_inherit")>
			<!--- Get the subfolders --->
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="arguments.thestruct.qrysubfolder">
			SELECT folder_id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
			WHERE folder_id_r = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
			AND folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			</cfquery>
		...
		<cfset log_folders(theuserid=arguments.thestruct.razuna.session.theuserid,logaction='Update',logdesc='#updated#: #arguments.thestruct.folder_name# (ID: #arguments.thestruct.folder_id#)', hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
		<!--- Flush Cache --->
		<cfset resetcachetoken(type="search", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
		<cfset resetcachetoken(type="labels", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
		<cfset resetcachetoken(type="images", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
...
<cffunction name="apifiletotalcount" output="false">
	...
	<cfargument name="thestruct" type="struct" required="true" />
	<!--- Call function --->
	<cfinvoke method="filetotalcount" folder_id="#arguments.folder_id#" thestruct="#arguments.thestruct#" theoverall="F" returnvariable="total">
	<!--- Return --->
	<cfreturn total>
</cffunction>
...
<cffunction name="filetotalcount_temp" output="false">
	...
	<cfset s.thetotal = arguments.thestruct.razuna.session.total_count_of_folder>
...
<cffunction name="filetotalcount" output="false">
	...
	<cfif arguments.thestruct.razuna.session.showsubfolders EQ "T">
		<cfinvoke method="getfoldersinlist" dsn="#arguments.thestruct.razuna.application.datasource#" folder_id="#arguments.folder_id#" database="#arguments.thestruct.razuna.application.thedatabase#" hostid="#arguments.thestruct.razuna.session.hostid#" returnvariable="thefolders" thestruct="#arguments.thestruct#">
	...
	<cfset var total = "">
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="total" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#filetotalcount */
		(
		...
		(
			...
			FROM #arguments.thestruct.razuna.session.hostdbprefix#files fi, #arguments.thestruct.razuna.session.hostdbprefix#folders f
			WHERE fi.folder_id_r = f.folder_id
			AND (f.folder_is_collection IS NULL OR folder_is_collection = '')
			AND fi.folder_id_r IS NOT NULL
			AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			AND fi.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			AND fi.in_trash = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="F">
			AND fi.is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			...
			<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
				AND fi.file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
			</cfif>
		...
		(
			...
			FROM #arguments.thestruct.razuna.session.hostdbprefix#images i, #arguments.thestruct.razuna.session.hostdbprefix#folders f
			...
			AND i.is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			<cfif arguments.theoverall EQ "F" AND arguments.folder_id NEQ "">
				AND i.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
			</cfif>
			<!--- If coming from custom view and the arguments.thestruct.razuna.session.customfileid is not empty --->
			<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
				AND i.img_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
			</cfif>
			<cfif arguments.folderaccess EQ 'R'>
		...
		(
			...
			<!--- If coming from custom view and the arguments.thestruct.razuna.session.customfileid is not empty --->
			<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
				AND v.vid_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
			</cfif>
			<cfif arguments.folderaccess EQ 'R'>
		...
		(
			...
			FROM #arguments.thestruct.razuna.session.hostdbprefix#audios a, #arguments.thestruct.razuna.session.hostdbprefix#folders f
			...
			AND a.is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			<cfif arguments.theoverall EQ "F" AND arguments.folder_id NEQ "">
				AND a.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
			</cfif>
			<!--- If coming from custom view and the arguments.thestruct.razuna.session.customfileid is not empty --->
			<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
				AND a.aud_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
			</cfif>
			<cfif arguments.folderaccess EQ 'R'>
...
<cffunction name="filetotaltype" output="false">
	...
	<cfif arguments.thestruct.folder_id NEQ "">
		<cfif arguments.thestruct.razuna.session.showsubfolders EQ "T">
			<cfinvoke method="getfoldersinlist" dsn="#arguments.thestruct.razuna.application.datasource#" folder_id="#arguments.thestruct.folder_id#" database="#arguments.thestruct.razuna.application.thedatabase#" hostid="#arguments.thestruct.razuna.session.hostid#" returnvariable="thefolders" thestruct="#arguments.thestruct#">
	...
	<cfif arguments.thestruct.kind EQ "img">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="total" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#imgfiletotaltype */ count(img_id) as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#images
		WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		...
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
			AND img_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfif>
	...
	<cfelseif arguments.thestruct.kind EQ "vid">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="total" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#vidfiletotaltype */ count(vid_id) as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#videos
		WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		...
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
			AND vid_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR expiry_date is null)
		</cfif>
	...
	<cfelseif arguments.thestruct.kind EQ "aud">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="total" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#audfiletotaltype */ count(aud_id) as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#audios
		WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		...
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
			AND aud_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR expiry_date is null)
		</cfif>
	...
	<cfelseif arguments.thestruct.kind EQ "doc">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="total" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#docfiletotaltype */ count(file_id) as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#files
		WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		AND in_trash = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="F">
		<cfif thefolderlist NEQ "">
		...
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
			AND file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR expiry_date is null)
		</cfif>
...
<!--- GET ASSETS DETAILS --->
<cffunction name="getAssetsDetails" output="false"  returntype="Any" >
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qTab" cachedwithin="1" region="razcache">
	...
	UNION ALL
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#audios a LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#audios_text aut ON a.aud_id = aut.aud_id_r AND aut.lang_id_r = 1 AND a.host_id = aut.host_id
...
<cffunction name="fileTotalAllTypes" output="false" >
	<cfargument name="folder_id" default="" required="yes" type="string">
	<cfargument name="folderaccess" default="" required="no" type="string">
	...
	<!--- Query Aliases --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_aliases" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getallaliases */ c.asset_id_r, c.type, f.file_extension as file_ext
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qTab" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#fileTotalAllTypes */ 'doc' as ext, count(file_id) as cnt, 'doc' as typ, 'tab_word' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND SUBSTR<cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) = <cfqueryparam cfsqltype="cf_sql_varchar" value="doc">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		AND is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
			AND file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.folderaccess EQ 'R'>
		...
		UNION ALL
		SELECT /* #cachetoken#fileTotalAllTypes */ 'xls' as ext, count(file_id) as cnt, 'doc' as typ, 'tab_excel' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND SUBSTR<cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) = <cfqueryparam cfsqltype="cf_sql_varchar" value="xls">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		AND is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
		...
		UNION ALL
		SELECT /* #cachetoken#fileTotalAllTypes */ 'pdf' as ext, count(file_id) as cnt, 'doc' as typ, 'tab_pdf' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND SUBSTR<cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) = <cfqueryparam cfsqltype="cf_sql_varchar" value="pdf">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		AND is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
		...
		UNION ALL
		SELECT /* #cachetoken#fileTotalAllTypes */ 'other' as ext, count(file_id) as cnt, 'doc' as typ, 'tab_others' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND ((SUBSTR<cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="doc">
		AND SUBSTR<cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="xls">
		AND file_extension <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="pdf">)
		OR  file_type = 'other')
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		AND is_available <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
		...
		UNION ALL
		SELECT /* #cachetoken#fileTotalAllTypes */ 'img' as ext, count(img_id) as cnt, 'img' as typ, 'tab_images' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#images
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND (img_group IS NULL OR img_group = '')
		...
		UNION ALL
		SELECT /* #cachetoken#fileTotalAllTypes */ 'vid' as ext, count(vid_id) as cnt, 'vid' as typ, 'tab_videos' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#videos
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND (vid_group IS NULL OR vid_group = '')
		...
		UNION ALL
		SELECT /* #cachetoken#fileTotalAllTypes */ 'aud' as ext, count(aud_id) as cnt, 'aud' as typ, 'tab_audios' as scr, '0' as thetotal
		FROM #arguments.thestruct.razuna.session.hostdbprefix#audios
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND (aud_group IS NULL OR aud_group = '')
...
<cffunction  name="setaccess" output="true" returntype="string">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="fprop" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#setaccess */ <cfif arguments.sf>'0' as folder_owner, '0' as folder_id_r<cfelse>f.folder_owner</cfif>,
	<cfif arguments.thestruct.razuna.session.is_system_admin OR arguments.thestruct.razuna.session.is_administrator>
		'X' as permfolder
	<cfelse>
		CASE
			...
			WHEN (<cfif arguments.sf>f.sf_who<cfelse>f.folder_owner</cfif> = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">) THEN 'X'
		END as permfolder
	</cfif>
	<cfif arguments.sf>
		FROM #arguments.thestruct.razuna.session.hostdbprefix#smart_folders f LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg ON f.sf_id = fg.folder_id_r AND f.host_id = fg.host_id
		WHERE f.sf_id = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	<cfelse>
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg ON f.folder_id = fg.folder_id_r AND f.host_id = fg.host_id
	...
	<cfif (arguments.thestruct.razuna.session.is_system_admin OR arguments.thestruct.razuna.session.is_administrator) OR fprop.folder_owner EQ arguments.thestruct.razuna.session.theuserid>
		<cfset var folderaccess = "x">
	</cfif>
	<!--- If arguments.thestruct.razuna.session.customaccess is here and is not empty --->
	<cfif structKeyExists(arguments.thestruct.razuna.session,"customaccess") AND arguments.thestruct.razuna.session.customaccess NEQ "">
		<cfset var folderaccess = arguments.thestruct.razuna.session.customaccess>
	</cfif>
	<cfreturn folderaccess />
</cffunction>

<!--- SET ALIAS ACCESS PERMISSION --->
<cffunction  name="setaliasaccess" output="true" returntype="string" >
	<cfargument name="thestruct" type="struct" required="true">
	<cfargument name="asset_id" required="true" type="string">
	<cfset var folderaccess = "n">
	<cfset var aliasfolders = "">
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="aliasfolders" cachedwithin="1" region="razcache">
	...
	<cfloop query="aliasfolders">
		<cfset var permfolder = setaccess(thestruct=arguments.thestruct, folder_id=folder_id_r)>
		<cfif permfolder EQ "R" AND folderaccess NEQ "W" AND folderaccess NEQ "X">
			<cfset var folderaccess = permfolder>
		<cfelseif permfolder EQ "W" AND folderaccess NEQ "X">
			<cfset var folderaccess = permfolder>
...
<cffunction  name="move" output="true">
	<cfargument name="thestruct" type="struct">
	...
	<cftry>
		...
		<!--- Flush Cache --->
		<cfset resetcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
		<!--- Clear arguments.thestruct.razuna.session.type --->
		<cfset arguments.thestruct.razuna.session.type = "">
		<!--- Log --->
		<cfinvoke component="defaults" method="trans" transid="moved" returnvariable="moved" />
		<cfset log_folders(theuserid=arguments.thestruct.razuna.session.theuserid,logaction='Move',logdesc='#moved#: #foldername.folder_name# (ID: #arguments.thestruct.tomovefolderid#)', hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
...
<cffunction name="recfolder" output="false" access="public" returntype="string">
	...
	<cfargument name="thelevel" required="false" >
	<cfargument name="thestruct" type="struct" required="true" />
	<!--- Cache --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- function internal vars --->
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="local_query" cachedwithin="1" region="razcache">
	...
	AND folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> folder_id_r
	AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	</cfquery>
...
<cffunction name="getuserfolder" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getuserfolder */ folder_id
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
		WHERE folder_of_user = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
		AND folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		</cfquery>
...
<cffunction name="getallassets" output="true" returnType="query">
	...
	<cfset var qry = "">
		...
		SELECT /* #cachetoken#getallassets */ i.img_id as id,
		...
		<!--- If coming from custom view and the arguments.thestruct.razuna.session.customfileid is not empty --->
		<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
			AND f.file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif qry_aliases.recordCount>
			OR (
	...
	<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
		...
		SELECT *
		FROM qry
		WHERE id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfquery>
	</cfif>
	...
	<cfset qry = addCustomFieldsToQuery(theqry=qry, thestruct=arguments.thestruct)>
	<!--- Return --->
	<cfreturn qry>
</cffunction>
...
<cffunction name="addCustomFieldsToQuery" output="false" returntype="query">
	...
	<cfloop query="arguments.theqry">
		...
		WHERE
		cft.lang_id_r = 1
		AND cft.host_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.hostid#">
		<cfif kind EQ "img">
...
<cffunction name="trashall" output="true">
	...
	<cfloop list="#arguments.thestruct.id#" delimiters="," index="i">
		<cfquery name="isalias" datasource="#arguments.thestruct.razuna.application.datasource#">
...
<cffunction name="getfoldersinlist" output="false">
	...
	<cfif arguments.prefix EQ "">
		<cfset arguments.prefix = arguments.thestruct.razuna.session.hostdbprefix>
	</cfif>
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Query --->
	<cfquery datasource="#arguments.dsn#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getfoldersinlist */ folder_id
...
<cffunction name="getfoldersfortree" access="public" output="true">
	...
	<cfinvoke component="defaults" method="getlangs" thestruct="#arguments.thestruct#" returnvariable="qry_lang">
	...
	<cfif NOT arguments.thestruct.razuna.session.is_system_admin AND NOT arguments.thestruct.razuna.session.is_administrator>
		<cfset arguments.thestruct.razuna.session.showmyfolder = "F">
	</cfif>
	<!--- Param --->
	<cfparam default="0" name="arguments.thestruct.razuna.session.thefolderorg">
	<cfparam default="0" name="arguments.thestruct.razuna.session.type">
	<cfparam default="0" name="arguments.thestruct.razuna.session.thegroupofuser">
	<cfset var qry = "">
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken##arguments.thestruct.razuna.session.theUserID#getfoldersfortree */ folder_id, folder_name, folder_id_r, folder_of_user, folder_owner, folder_level, in_trash,link_path, username, perm, subhere, permfolder
	FROM (
		...
		<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
			CASE
				...
				<!--- If this is the user folder or he is the owner --->
				WHEN f.folder_owner = '#arguments.thestruct.razuna.session.theUserID#' THEN 'unlocked'
				<!--- If this is the upload bin --->
				WHEN f.folder_id = '1' THEN 'unlocked'
		...
		CASE
			...
			WHEN EXISTS(
				SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>*
				FROM #arguments.thestruct.razuna.session.hostdbprefix#folders s1
				WHERE s1.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id
				AND s1.folder_id_r = f.folder_id
				ANd s1.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				AND s1.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				<!--- AND lower(s.folder_of_user) = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">  --->
				<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
					AND s1.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
				</cfif>
				<!--- If this is a move then dont show the folder that we are moving --->
				<cfif (arguments.actionismove EQ "T" AND arguments.thestruct.razuna.session.type EQ "movefolder") OR arguments.thestruct.razuna.session.type EQ "copyfolder">
					AND s1.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.razuna.session.thefolderorg#">
				</cfif>
				<!--- RAZ-583 : exclude link folder from subfolder count --->
				<cfif arguments.thestruct.razuna.session.type NEQ ''>
					AND (s1.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR s1.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR s1.link_path IS NULL)
				</cfif>
			...
			WHEN EXISTS(
				SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>*
				FROM #arguments.thestruct.razuna.session.hostdbprefix#folders s2, #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg3
				WHERE s2.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id
				AND s2.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				...
				AND fg3.grp_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.thegroupofuser#" list="true">)
				<!--- If this is a move then dont show the folder that we are moving --->
				<cfif (arguments.actionismove EQ "T" AND arguments.thestruct.razuna.session.type EQ "movefolder") OR arguments.thestruct.razuna.session.type EQ "copyfolder">
					AND s2.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.razuna.session.thefolderorg#">
				</cfif>
				<!--- RAZ-583 : exclude link folder from subfolder count --->
				<cfif arguments.thestruct.razuna.session.type NEQ ''>
					AND (s2.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR s2.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR s2.link_path IS NULL)
				</cfif>
				<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle">
					AND ROWNUM = 1
				<cfelseif  arguments.thestruct.razuna.application.thedatabase EQ "mysql" OR arguments.thestruct.razuna.application.thedatabase EQ "h2">
			...
			WHEN EXISTS(
				SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>*
				FROM #arguments.thestruct.razuna.session.hostdbprefix#folders s3, #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg4
				WHERE s3.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id
				...
				AND s3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				AND s3.host_id = fg4.host_id
				<!--- If this is a move then dont show the folder that we are moving --->
				<cfif (arguments.actionismove EQ "T" AND arguments.thestruct.razuna.session.type EQ "movefolder") OR arguments.thestruct.razuna.session.type EQ "copyfolder">
					AND s3.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.razuna.session.thefolderorg#">
				</cfif>
				<!--- RAZ-583 : exclude link folder from subfolder count --->
				<cfif arguments.thestruct.razuna.session.type NEQ ''>
					AND (s3.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR s3.link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR s3.link_path IS NULL)
				</cfif>
				<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle">
					AND ROWNUM = 1
				<cfelseif  arguments.thestruct.razuna.application.thedatabase EQ "mysql" OR arguments.thestruct.razuna.application.thedatabase EQ "h2">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON u.user_id = f.folder_owner
		WHERE
		<cfif theid gt 0>
			f.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id_r
			AND
		...
		<cfif iscol EQ "F" AND (NOT arguments.thestruct.razuna.session.is_system_admin AND NOT arguments.thestruct.razuna.session.is_administrator)>
			AND
				(
				<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2">NVL<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mssql">isnull</cfif>(f.folder_of_user,<cfqueryparam cfsqltype="cf_sql_varchar" value="f">) <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
				OR f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">
	...
	<cfif arguments.thestruct.razuna.session.type EQ "movefolder" OR arguments.thestruct.razuna.session.type EQ "copyfolder">
		AND itb.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.razuna.session.thefolderorg#">
	</cfif>
	<!--- RAZ-583 : exclude link folder from select --->
	<cfif arguments.thestruct.razuna.session.type NEQ ''>
		AND (link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR link_path <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR link_path IS NULL)
	</cfif>
	ORDER BY folder_name
	</cfquery>

	<!--- <cfset consoleoutput(true, true)>
	<cfset console(arguments.thestruct.razuna.session.theuserid)>
	...
	<cfif theid EQ 0>
		...
		<cfif arguments.thestruct.razuna.session.showmyfolder EQ "F" AND iscol NEQ "T">
			<!--- <cfset console("HERE !!!!!!!!!!!!!!!")> --->
			<cfquery dbtype="query" name="qry">
			SELECT *
			FROM qry
			WHERE (folder_of_user = <cfqueryparam cfsqltype="cf_sql_varchar" value="f"> OR folder_of_user IS NULL)
			OR (folder_name = <cfqueryparam cfsqltype="cf_sql_varchar" value="my folder"> AND folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">)
			OR folder_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="1">
			OR folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">
			</cfquery>
		</cfif>
	</cfif>
	...
	<cfif arguments.actionismove EQ "F">
		...
		<cfloop query="qry">
			<cfif qry.in_trash EQ 'F'>
				...
				<cfif theid EQ 0>
					<cfif iscol EQ "F">
						<cfif arguments.thestruct.razuna.session.theuserid NEQ folder_owner AND folder_owner NEQ "">
	...
	<cfelse>
		<cfloop query="qry">
			<cfif qry.in_trash EQ 'F'>
				...
				<cfif structKeyExists(arguments.thestruct.razuna.session, "thefileid") AND NOT arguments.thestruct.razuna.session.thefileid CONTAINS ",">
				...
				<!--- Folder name --->
				<cfset var _folder_name = folder_name>
				<cfif iscol EQ "F" AND folder_name EQ "my folder" AND (arguments.thestruct.razuna.session.is_system_admin OR arguments.thestruct.razuna.session.is_administrator)>
					<cfif arguments.thestruct.razuna.session.theuserid NEQ folder_owner AND folder_owner NEQ "">
				...
				<cfif !listfindnocase('w,x',qry.permfolder)>
					<!--- Nothing here --->
				<!--- movefile --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "movefile">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfif arguments.kind EQ "search">
							<cfset _attr.onclick = "$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#', function(){$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').html('The file(s) are being moved now.<br />Note: For a large batch of files this can take some time until it reflects in the system!<br />You can close this window.');});">
						<cfelse>
							<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#', function(){$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').html('The file(s) are being moved now.<br />Note: For a large batch of files this can take some time until it reflects in the system!<br />You can close this window.');});">
						</cfif>
					</cfif>
				<!--- movefolder --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "movefolder">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#', function(){$('##explorer').load('index.cfm?fa=#_attr_fa_explorer#');#_fromtrash#});destroywindow(1);return false;">
					</cfif>
				<!--- copyfolder --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "copyfolder">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "loadcontent('div_forall','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#&inherit_perm='+$('##perm_inherit').is(':checked'), function(){#_fromtrash#');});$('#_attr_div_explorer#').load('index.cfm?fa=#_attr_fa_explorer#');destroywindow(1);return false;">
					</cfif>
				<!--- restorefolder --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restorefolder">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#');$('##rightside').load('index.cfm?fa=c.folder_explorer_trash&trashkind=folders');destroywindow(1);return false;">
					</cfif>
				<!--- restoreselectedfolders --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreselectedfolders">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#');destroywindow(1);return false;">
					</cfif>
				<!--- restorefile --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restorefile">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset var _for_here_1 = "">
						<cfset var _for_here_2 = "">
						<cfif arguments.thestruct.razuna.session.thefileid CONTAINS ",">
							<cfset var _for_here_1 = "loadoverlay();">
						</cfif>
						<cfif NOT arguments.thestruct.razuna.session.thefileid CONTAINS ",">
							<cfset var _for_here_2_start = "loadcontent('thewindowcontent1','index.cfm?fa=c.">
							<cfif arguments.thestruct.razuna.session.thetype EQ "doc">
								<cfset _for_here_2_sub = "files">
							<cfelseif arguments.thestruct.razuna.session.thetype EQ "img">
								<cfset _for_here_2_sub = "images">
							<cfelseif arguments.thestruct.razuna.session.thetype EQ "vid">
								<cfset _for_here_2_sub = "videos">
							<cfelseif arguments.thestruct.razuna.session.thetype EQ "aud">
								<cfset _for_here_2_sub = "audios">
							</cfif>
							<cfset var _for_here_2 = _for_here_2_start & _for_here_2_sub & "_detail&file_id=" & arguments.thestruct.razuna.session.thefileid & "&what=" & _for_here_2_sub & "&loaddiv=&folder_id=" & folder_id>
						</cfif>
						<cfset _attr.onclick = "#_for_here_1#$('##rightside').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#&intolevel=#folder_level#', function(){loadfolderwithdelay('#arguments.thestruct.razuna.session.thefolderorg#');$('##bodyoverlay').remove();});#_attr_destroywindow##_for_here_2#');">
					</cfif>
				<!--- restorefileall --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restorefileall">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
					</cfif>
				<!--- restoreselectedfiles --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreselectedfiles">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#');delayloadingoflist();destroywindow(1);return false;">
					</cfif>
				<!--- restorefolderall --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restorefolderall">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
					</cfif>
				<!--- saveaszip or as a collection --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "saveaszip" OR arguments.thestruct.razuna.session.type EQ "saveascollection">
					<cfset _attr.onclick = "loadcontent('win_choosefolder_#arguments.thestruct.razuna.session.tmpid#','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#');">
				<!--- upload --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "uploadinto">
					<cfset _attr.onclick = "showwindow('index.cfm?fa=c.asset_add&folder_id=#folder_id#','Add your files',650,1);return false;">
				<!--- customization --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "customization">
				...
				<!--- group detail--->
				<cfelseif arguments.thestruct.razuna.session.type EQ "groups_detail">
				...
				<!--- scheduler --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "scheduler">
				...
				<!--- choose a collection --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "choosecollection">
				...
				<cfelseif arguments.thestruct.razuna.session.type EQ "restore_collection_file">
					<cfset _attr.onclick = "loadcontent('div_choosecol','index.cfm?fa=c.collection_chooser&withfolder=T&folder_id=#folder_id#');">
				<!--- Restore all collection files in the trash --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreallcollectionfiles">
					<cfset _attr.onclick = "loadcontent('div_choosecol','index.cfm?fa=c.collection_chooser&withfolder=T&folder_id=#folder_id#');">
				<!--- Restore selected collection files in the trash --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreselectedcolfiles">
					<cfset _attr.onclick = "loadcontent('div_choosecol','index.cfm?fa=c.collection_chooser&withfolder=T&folder_id=#folder_id#');">
				<!--- choose a folder for restore collection --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restore_collection">
					<cfset _attr.onclick = "loadcontent('collections','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
				<!--- Restore all collections in the trash --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreallcollections">
					<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
				<!--- Restore selected collections  --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreselectedcollection">
					<cfset _attr.onclick = "loadcontent('collections','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
				<!--- Restore collection folder in the trash --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restorecolfolder">
					<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
				<!--- Restore all collection folder in the trash --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restorecolfolderall">
					<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
				<!--- Restore all collection folder in the trash --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "restoreselectedcolfolder">
					<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
				<!--- copy metadata --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "copymetadata">
					<cfset _attr.onclick = "loadcontent('result','index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#&what=#arguments.thestruct.razuna.session.thetype#&fid=#arguments.thestruct.razuna.session.file_id#');destroywindow(2);return false;">
				<!--- Plugin --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "plugin">
					<cfset _attr.onclick = "$('##wf_folder_id_2').val('#folder_id#'); $('##wf_folder_name_2').val('#Encodeforjavascript(folder_name)#');destroywindow(1);">
				<!--- From Smart Folder --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "sf_download">
					<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#');$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').html('All file(s) are going to be downloaded now and stored in the chosen folder!');return false;">
				<!--- Alias --->
				<cfelseif arguments.thestruct.razuna.session.type EQ "alias">
					<cfif arguments.thestruct.razuna.session.thefolderorg NEQ folder_id>
						<cfif arguments.kind EQ "search">
							<cfset _attr.onclick = "$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#', function(){$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').html('The alias has been created in the selected folder.<br />Note: If you want to create an alias for the same file(s) in another folder simply select it from the list above!<br />You can close this window.');});">
						<cfelse>
							<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#arguments.thestruct.razuna.session.savehere#&folder_id=#folder_id#', function(){$('##div_choosefolder_status_#arguments.thestruct.razuna.session.tmpid#').html('The alias has been created in the selected folder.<br />Note: If you want to create an alias for the same file(s) in another folder simply select it from the list above!<br />You can close this window.');});">
						</cfif>
					</cfif>
				</cfif>
...
<cffunction name="sharecheckperm" output="true">
	...
	<cfparam name="arguments.thestruct.razuna.session.theuserid" default="">
	<cfparam name="arguments.thestruct.razuna.session.iscol" default="F">
	<cfset var qry = "">
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Check if folder is even shared or not --->
	<cfif arguments.thestruct.razuna.session.iscol EQ "F">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#sharecheckperm */ folder_shared shared
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
	...
	<cfelse>
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#sharecheckperm2 */ col_shared shared
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections
	...
	<cfif qry.shared EQ "T">
		<cfif arguments.thestruct.razuna.session.iscol EQ "F">
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryfolder" cachedwithin="1" region="razcache">
			SELECT /* #cachetoken#sharecheckperm3 */ grp_id_r
			FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_groups
		...
		<cfelse>
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryfolder" cachedwithin="1" region="razcache">
			SELECT /* #cachetoken#sharecheckperm4 */ grp_id_r
			FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups
...
<cffunction name="sharecheckpermfolder" access="public" output="true">
	...
	<cfif arguments.thestruct.razuna.session.iscol EQ "F">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#sharecheckpermfolder */ folder_id,
			<!--- Permission follow but not for sysadmin and admin --->
			<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
				CASE
					WHEN EXISTS(
						SELECT fg.folder_id_r
						FROM #arguments.thestruct.razuna.session.hostdbprefix#folders_groups fg INNER JOIN ct_groups_users gu ON gu.ct_g_u_grp_id = fg.grp_id_r AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
						WHERE fg.folder_id_r = f.folder_id
						AND fg.grp_permission IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="r,w,x" list="true">)
						) THEN 'unlocked'
					<!--- If this is the user folder or he is the owner --->
	...
	<cfelse>
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#sharecheckpermfolder */ col_id,
			<!--- Permission follow but not for sysadmin and admin --->
			<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
				CASE
					WHEN EXISTS(
						SELECT fg.col_id_r
						FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups fg INNER JOIN ct_groups_users gu ON gu.ct_g_u_grp_id = fg.grp_id_r AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
						WHERE fg.col_id_r = f.col_id
						AND fg.grp_permission IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="r,w,x" list="true">)
						) THEN 'unlocked'
					<!--- If this is the user folder or he is the owner --->
					WHEN ( f.col_owner = '#arguments.thestruct.razuna.session.theUserID#' ) THEN 'unlocked'
					<!--- If nothing meets the above lock the folder --->
					ELSE 'locked'
				END AS perm
			<cfelse>
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections f
...
<cffunction name="batch_sharing" output="true">
	...
	<cfloop list="#arguments.thestruct.file_ids#" index="i">
		...
		<cfelseif thetype EQ "img">
			...
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qryrel">
...
<cffunction name="getusername" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getusername */ u.user_first_name, u.user_last_name, f.folder_owner
	...
	<cfloop delimiters="," index="myform" list="#arguments.thestruct.fieldnames#">
		...
		<cfif myform CONTAINS "img_">
			...
			<cfif imgid NEQ theid>
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="here_img">
				SELECT img_id_r
		...
		<cfelseif myform CONTAINS "vid_">
			...
			<cfif vidid NEQ theid>
				...
				<!--- And the keywords & desc --->
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="here_vid">
				SELECT vid_id_r
		...
		<cfelseif myform CONTAINS "aud_">
			...
			<cfif audid NEQ theid>
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="here_aud">
				SELECT aud_id_r
		...
		<cfelseif myform CONTAINS "doc_">
			...
			<cfif docid NEQ theid>
				...
				<!--- And the keywords & desc --->
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="here_doc">
				SELECT file_id_r
...
<cffunction name="link_check" output="false">
	<cfargument name="thestruct" type="struct" required="true">
		...
		<cfif (arguments.thestruct.razuna.application.isp AND ListContains(arguments.thestruct.link_path, 'home', '/\') AND directoryexists("#arguments.thestruct.link_path#")) OR (!arguments.thestruct.razuna.application.isp AND directoryexists("#arguments.thestruct.link_path#"))>
			<!--- Set to true --->
...
<cffunction name="getsubfolders" output="false">
	...
	AND (f.folder_is_collection IS NULL OR folder_is_collection = '')
	<!--- filter user folders, but not for collections --->
	<cfif ( not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator ) AND NOT structkeyexists(arguments,"external")>
		AND
			(
			<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2">NVL<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mssql">isnull</cfif>(f.folder_of_user,<cfqueryparam cfsqltype="cf_sql_varchar" value="f">) <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
			OR f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">
	...
	<cfloop query="qRet">
		...
		<cfset var _filecount = filetotalcount(folder_id=count_folder_id, thestruct=arguments.thestruct)>
...
<cffunction name="getbreadcrumb" output="false">
	...
	<cfargument name="dsn" type="string" default="#arguments.thestruct.razuna.application.datasource#" required="false">
	<cfargument name="prefix" type="string" default="#arguments.thestruct.razuna.session.hostdbprefix#" required="false">
	<cfargument name="hostid" type="string" default="#arguments.thestruct.razuna.session.hostid#" required="false">
	<cfargument name="thestruct" type="struct" required="true" />
	<cftry>
		<!--- Params --->
		<cfset var qry = "">
		...
		<cfif arguments.fromshare>
			...
			SELECT /* #cachetoken#getrootfolderidshared */ folder_id_r
			FROM #arguments.prefix#folders
			WHERE folder_id = <cfqueryparam value="#arguments.thestruct.razuna.session.fid#" cfsqltype="CF_SQL_VARCHAR">
			</cfquery>
			<!--- Check the permission settings of a widget --->
			<cfif structKeyExists(arguments.thestruct.razuna.session,"widget_id")>
				<cfset s = structnew()>
				<cfset s.widget_id = arguments.thestruct.razuna.session.widget_id>
				<cfset s.external = "t">
		...
		<cfquery datasource="#arguments.dsn#" name="qry" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getbreadcrumb */ f.folder_id_r, f.folder_id
		<cfif checkperm>
			<cfif arguments.thestruct.razuna.session.iscol EQ "F">
				,
				CASE
		...
		<cfif checkperm AND (NOT arguments.thestruct.razuna.session.is_system_admin AND NOT arguments.thestruct.razuna.session.is_administrator)>
			<cfquery dbtype="query" name="qry">
			SELECT *
			FROM qry
			WHERE perm = <cfqueryparam cfsqltype="cf_sql_varchar" value="unlocked">
		...
		<cfif qry.recordcount NEQ 0>
			...
			<cfif qry.folder_id_r NEQ arguments.folder_id_r>
				...
				<cfinvoke component="folders" returnvariable="flist" method="getbreadcrumb" folder_id_r="#qry.folder_id_r#" folderlist="#flist#" fromshare="#arguments.fromshare#" dsn="#arguments.dsn#" prefix="#arguments.prefix#" hostid="#arguments.hostid#" thestruct="#arguments.thestruct#" />
			</cfif>
		<cfelse>
			<!--- Set the current values into the list --->
...
<cffunction name="download_folder" output="false">
	...
	<cfif arguments.thestruct.prefs.set2_meta_export EQ 't'>
		...
		<cfelse>
			<cfset var suffix = "#arguments.thestruct.razuna.session.hostid#-#arguments.thestruct.razuna.session.theuserid#">
		</cfif>
...
<cffunction name="download_selected" output="false">
	...
	<cfinvoke component="global.cfc.settings"  method="getsettingsfromdam" thestruct="#arguments.thestruct#" returnvariable="arguments.thestruct.getsettings" />
	<cfset var count = 1>
	...
	<cfif arguments.dl_renditions>
		...
		<cfif rend_av EQ 'f'>
			...
			<cfquery name="arguments.dl_query" datasource="#arguments.thestruct.razuna.application.datasource#">
		...
		<cfelseif rend_av EQ 't'>
			...
			<cfquery name="arguments.dl_query" datasource="#arguments.thestruct.razuna.application.datasource#">
				...
				WHERE v.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				...
				WHERE a.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				...
				WHERE f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	...
	<cfloop query="arguments.dl_query">
		...
		<cfif theorgname NEQ "">
			...
			<!--- Local --->
			<cfif arguments.thestruct.razuna.application.storage EQ "local" AND link_kind EQ "">
				<cftry>
				<cffile action="copy" source="#arguments.assetpath#/#arguments.thestruct.razuna.session.hostid#/#path_to_asset#/#theorgname#" destination="#arguments.dl_folder#/#thefinalname#" mode="775">
				<cfcatch type="any">
						<cfset console("#now()# ---------------- Error")>
						<cfset consoleoutput(true, true)>
						<cfset console(cfcatch)>
			...
			<!--- Nirvanix --->
			<cfelseif arguments.thestruct.razuna.application.storage EQ "nirvanix" AND link_kind EQ "">
				<cftry>
			...
			<!--- Akamai --->
			<cfelseif arguments.thestruct.razuna.application.storage EQ "akamai" AND link_kind EQ "">
			...
			<!--- Amazon --->
			<cfelseif arguments.thestruct.razuna.application.storage EQ "amazon" AND link_kind EQ "">
					<cfif rend_av EQ 't'>
...
<cffunction name="store_values" output="false" returntype="void">
	...
	<cfif arguments.thestruct.folder_id neq 1>
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR arguments.thestruct.thekind EQ "img">
			SELECT /* #cachetoken#sv */ <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">img_id + '-img'<cfelse>concat(img_id,'-img')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#images
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND (img_group IS NULL OR img_group = '')
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			...
			UNION
			SELECT /* #cachetoken#sv */ <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">img_id + '-img'<cfelse>concat(img_id,'-img')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#images i, ct_aliases ct
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR arguments.thestruct.thekind EQ "vid">
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">vid_id + '-vid'<cfelse>concat(vid_id,'-vid')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#videos
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND (vid_group IS NULL OR vid_group = '')
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			...
			UNION
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">vid_id + '-vid'<cfelse>concat(vid_id,'-vid')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#videos v, ct_aliases ct
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR arguments.thestruct.thekind EQ "aud">
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">aud_id + '-aud'<cfelse>concat(aud_id,'-aud')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#audios
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND (aud_group IS NULL OR aud_group = '')
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			...
			UNION
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">aud_id + '-aud'<cfelse>concat(aud_id,'-aud')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#audios a, ct_aliases ct
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR (arguments.thestruct.thekind NEQ "vid" AND arguments.thestruct.thekind NEQ "img" AND arguments.thestruct.thekind NEQ "aud")>
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">file_id + '-doc'<cfelse>concat(file_id,'-doc')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#files
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
			AND in_trash = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="F">
			...
			UNION
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">file_id + '-doc'<cfelse>concat(file_id,'-doc')</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#files f, ct_aliases ct
	...
	<cfelse>
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
			SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">file_id_r + '-' + col_file_type<cfelse>concat(file_id_r,'-',col_file_type)</cfif> as id
			FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files
	...
	<!--- Set the sessions --->
	<cfset arguments.thestruct.razuna.session.file_id = l>
	<cfset arguments.thestruct.razuna.session.thefileid = l>
	<cfset arguments.thestruct.razuna.session.editids = l>
</cffunction>
...
<cffunction name="store_selection" output="false" returntype="void">
	<cfargument name="thestruct" required="yes" type="struct">
	<!--- session --->
	<cfparam name="arguments.thestruct.razuna.session.file_id" default="">
	...
	<cfif isdefined("arguments.thestruct.individual_select") AND arguments.thestruct.individual_select EQ 'true'>
		<cfset arguments.thestruct.razuna.session.individual_select = true>
	</cfif>
	<!--- Now simply add the selected fileids to the session --->
	<cfset arguments.thestruct.razuna.session.file_id = "">
	<cfset arguments.thestruct.razuna.session.file_id = listappend(arguments.thestruct.razuna.session.file_id,"#arguments.thestruct.file_id#")>
	<cfset arguments.thestruct.razuna.session.thefileid = arguments.thestruct.razuna.session.file_id>
	<cfif arguments.thestruct.razuna.session.file_id NEQ "">
		<cfset list_file_ids = "">
		<cfloop index="idx" from="1" to="#listlen(arguments.thestruct.razuna.session.file_id)#">
			<!--- <cfif !listFindNoCase(#arguments.thestruct.del_file_id#,#listGetAt(arguments.thestruct.razuna.session.file_id,idx)#)> --->
				<cfset list_file_ids = listAppend(list_file_ids,#listGetAt(arguments.thestruct.razuna.session.file_id,idx)#,',')>
			<!--- </cfif> --->
		</cfloop>
		<cfset arguments.thestruct.razuna.session.thefileid = list_file_ids>
		<cfset arguments.thestruct.razuna.session.file_id = list_file_ids>
		<cfset arguments.thestruct.razuna.session.editids = list_file_ids>
	</cfif>
</cffunction>
...
<cffunction name="samefoldernamecheck" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getfoldername */ folder_id
	FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
	WHERE folder_name = <cfqueryparam value="#arguments.thestruct.folder_name#" cfsqltype="CF_SQL_VARCHAR">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
...
<cffunction name="trashcount" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="asset_count" cachedwithin="#CreateTimeSpan(0,0,5,0)#" region="razcache">
	SELECT /* #cachetoken#trashcount */ COUNT(img_id) AS cnt FROM #arguments.thestruct.razuna.session.hostdbprefix#images i
	...
	UNION ALL
	SELECT COUNT(aud_id) AS cnt  FROM #arguments.thestruct.razuna.session.hostdbprefix#audios a
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	AND CASE
			<cfif arguments.thestruct.razuna.session.is_system_admin OR arguments.thestruct.razuna.session.is_administrator>
	...
	UNION ALL
	SELECT COUNT(vid_id) AS cnt FROM #arguments.thestruct.razuna.session.hostdbprefix#videos v
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	AND CASE
			<cfif arguments.thestruct.razuna.session.is_system_admin OR arguments.thestruct.razuna.session.is_administrator>
	...
	UNION ALL
	SELECT COUNT(file_id) AS cnt FROM #arguments.thestruct.razuna.session.hostdbprefix#files f
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	AND CASE
			<cfif arguments.thestruct.razuna.session.is_system_admin OR arguments.thestruct.razuna.session.is_administrator>
...
<cffunction name="folderTrashCount" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="folder_count" cachedwithin="#CreateTimeSpan(0,0,5,0)#" region="razcache">
	SELECT /* #cachetoken#trashcount */ COUNT(folder_id) AS cnt
	FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="getdetailnextback" output="false" returntype="struct">
	...
	<cfif structKeyExists(arguments.thestruct,"row")>
		...
		<cfif arguments.thestruct.what EQ "images">
			<cfset var thedb = "#arguments.thestruct.razuna.session.hostdbprefix#images">
		...
		<cfelseif arguments.thestruct.what EQ "videos">
			<cfset var thedb = "#arguments.thestruct.razuna.session.hostdbprefix#videos">
		...
		<cfelseif arguments.thestruct.what EQ "audios">
			<cfset var thedb = "#arguments.thestruct.razuna.session.hostdbprefix#audios">
		...
		<cfelseif arguments.thestruct.what EQ "files">
			<cfset var thedb = "#arguments.thestruct.razuna.session.hostdbprefix#files">
		...
		<cfif arguments.thestruct.razuna.session.sortby EQ "name">
			<cfset var sortby = "filename_forsort">
		<cfelseif arguments.thestruct.razuna.session.sortby EQ "kind">
		...
		<cfelseif arguments.thestruct.razuna.session.sortby EQ "datechanged">
			<cfset var sortby = "date_change DESC">
		<cfelseif arguments.thestruct.razuna.session.sortby EQ "hashtag">
		...
		<!--- Query Aliases --->
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_aliases" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getdetailnextbackalias */ asset_id_r, type
		FROM ct_aliases
		WHERE folder_id_r = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.folder_id#">
		</cfquery>
		<cfloop query="qry_aliases">
		...
		<cfif arguments.thestruct.loaddiv EQ "content">
			...
			SELECT /* #cachetoken#getdetailnextback */
			img_id as file_id,
...
<cffunction name="getallassetsinfolder" access="remote" output="false" returntype="Query">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Query --->
	<cfquery name="qry_folder" datasource="#arguments.thestruct.razuna.application.datasource#" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getallassetsinfolder */ img_id AS ID, 'img' as type<cfif arguments.columnlist NEQ "">, #arguments.columnlist#</cfif>
	FROM #arguments.thestruct.razuna.session.hostdbprefix#images
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
...
<cffunction  name="copy" output="true">
	...
	<!---<cftry>--->
		...
		<cfif arguments.thestruct.razuna.application.storage EQ "local">
			<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#">
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/aud')>
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img')>
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/vid')>
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/doc')>
		</cfif>

		<!--- Insert into folders table --->
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="insert_folder">
			INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#folders
		...
		<cfloop query="assets" >
			...
			<cfif kind EQ 'img'>
				...
				<cfquery name="select_images" datasource="#arguments.thestruct.razuna.application.datasource#">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#images
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "local">
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img/#newimgid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img/#id#" newdirectory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img/#newimgid#" >
					--->
					<cffile action="rename" source="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img/#newimgid#/thumb_#id#.#select_images.thumb_extension#" destination="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/img/#newimgid#/thumb_#newimgid#.#select_images.thumb_extension#" >
				</cfif>
				<cfquery name="select_images_text" datasource="#arguments.thestruct.razuna.application.datasource#">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#images_text
					WHERE img_id_r = <cfqueryparam value="#id#" cfsqltype="cf_sql_varchar" >
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				</cfquery>
				<cfquery name="select_xmp" datasource="#arguments.thestruct.razuna.application.datasource#">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#xmp
					WHERE id_r = <cfqueryparam value="#id#" cfsqltype="cf_sql_varchar" >
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
				</cfquery>

				<cfif arguments.thestruct.razuna.application.storage EQ "amazon">
					...
					<cfthread name="copyfolderimg#newimgid#" intupstruct="#arguments.thestruct#">
						<cfinvoke component="amazon" method="copyfolder">
							...
							<cfinvokeargument name="thestruct" value="#attributes.intupstruct#">
						</cfinvoke>
					</cfthread>
					...
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url" key="#newfolderid#/img/#newimgid#/thumb_#newimgid#.jpg" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
					<!--- Get SignedURL original --->
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url_org" key="#newfolderid#/img/#newimgid#/#select_images.img_filename_org#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				<cfelseif arguments.thestruct.razuna.application.storage EQ "nirvanix">
					<!--- Copy --->
					<cfthread name="copyfolderimg#newimgid#" intupstruct="#arguments.thestruct#">
				...
				<cfset link_path = '#arguments.thestruct.path#raz#arguments.thestruct.razuna.session.hostid#/dam/incoming/api#newimgid#'>
				<cfquery name="updateimages" datasource="#arguments.thestruct.razuna.application.datasource#">
					INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#images
					...
					<cfqueryparam value="#arguments.thestruct.razuna.session.HOSTID#" cfsqltype="cf_sql_integer" >,
				...
				<cfquery name="updateimges_text" datasource="#arguments.thestruct.razuna.application.datasource#">
					INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#images_text
			...
			<cfelseif kind EQ 'doc' OR kind EQ 'other'>
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="select_files">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#files
					WHERE file_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#id#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
					AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				</cfquery>
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "local">
					...
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/doc/#newfileid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/doc/#id#" newdirectory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/doc/#newfileid#" >
					--->
				</cfif>
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="select_files_desc">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#files_desc
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "amazon">
					...
					<cfif assets.ext EQ "pdf">
						<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url" key="#newfolderid#/doc/#newfileid#/#select_files.file_name_noext#.jpg" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
					</cfif>
					<!--- Get SignedURL original --->
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url_org" key="#newfolderid#/doc/#newfileid#/#select_files.file_name_org#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				<cfelseif arguments.thestruct.razuna.application.storage EQ "nirvanix">
				...
				<!--- Insert into tables --->
				<cfset link_path = '#arguments.thestruct.path#raz#arguments.thestruct.razuna.session.hostid#/dam/incoming/api#newfileid#'>
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_files" >
					INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#files
				...
				<cfif select_files_desc.recordcount>
					<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_files_desc" >
						INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#files_desc
			...
			<cfelseif kind EQ 'aud'>
				...
				<!--- Get audio records --->
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="select_audios">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#audios
					WHERE aud_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#id#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
					AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				</cfquery>
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "local">
					...
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/aud/#newaudid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/aud/#id#" newdirectory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/aud/#newaudid#" >
						--->
				</cfif>
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="select_audios_text">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#audios_text
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "amazon">
					...
					<cfpause interval="5" />
					<!--- Get SignedURL original --->
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url_org" key="#newfolderid#/aud/#newaudid#/#select_audios.aud_name_org#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				<cfelseif arguments.thestruct.razuna.application.storage EQ "nirvanix">
					<!--- Nirvanix CopyFolders --->
					<cfthread name="copyfolderaud#newaudid#" intupstruct="#arguments.thestruct#">
						<cfinvoke component="nirvanix" method="CopyFolders">
				...
				<cfset link_path = '#arguments.thestruct.path#raz#arguments.thestruct.razuna.session.hostid#/dam/incoming/api#newaudid#'>
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_audios">
					INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#audios
					...
					VALUES(
						...
						<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.thestruct.razuna.session.HOSTID#">,
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_audios_text">
					INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#audios_text
			...
			<cfelseif kind EQ 'vid'>
				...
				<!--- Get video records --->
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="select_videos">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#videos
					WHERE vid_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#id#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
					AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				</cfquery>
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "local">
					...
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/vid/#newvidid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/vid/#id#" newdirectory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#newfolderid#/vid/#newvidid#" >
					--->
				</cfif>
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="select_videos_text">
					SELECT * FROM #arguments.thestruct.razuna.session.hostdbprefix#videos_text
				...
				<cfif arguments.thestruct.razuna.application.storage EQ "amazon">
					...
					<cfpause interval="5" />
					<!--- Get SignedURL Thumb --->
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url" key="#newfolderid#/vid/#newvidid#/#select_videos.vid_name_image#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
					<!--- Get SignedURL original --->
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url_org" key="#newfolderid#/vid/#newvidid#/#select_videos.vid_name_org#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				...
				<cfset link_path = '#arguments.thestruct.path#raz#arguments.thestruct.razuna.session.hostid#/dam/incoming/api#newvidid#'>
				<!--- Insert video details --->
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_videos">
				...
				<cfif select_videos_text.RecordCount>
					<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_videos_text">
						INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#videos_text
...
<cffunction name="download_upc_folder" output="false">
	...
	<cfinvoke component="folders" method="getbreadcrumb" folder_id_r="#arguments.thestruct.folder_id#" returnvariable="crumbs" thestruct="#arguments.thestruct#" />
...
<cffunction name="subscribe" access="public" output="true">
	...
	<!--- Subscribe details --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qfoldersubscribe" cachedwithin="1" region="razcache">
	...
	AND user_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.razuna.session.theUserID#">
	</cfquery>
	...
	<cfif arguments.thestruct.emailnotify EQ 'no'>
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
		DELETE FROM #arguments.thestruct.razuna.session.hostdbprefix#folder_subscribe
		WHERE folder_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.theid#">
		AND user_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.razuna.session.theUserID#">
		</cfquery>
	<cfelse>
		...
		<cfelse>
			...
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
			INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#folder_subscribe
...
<cffunction name="getsubscribefolder" output="false" access="public" returntype="query">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_folder" cachedwithin="1" region="razcache">
...
<cffunction name="download_folder_structure_flat" output="false">
	...
	<!--- <cfif structKeyExists(arguments.thestruct,'export_template') AND arguments.thestruct.export_template.recordcount NEQ 0> --->
		<!--- <cffile action="move" destination="#arguments.thestruct.newpath#" source="#arguments.thestruct.thepath#/outgoing/metadata-export-#arguments.thestruct.razuna.session.hostid#-#arguments.thestruct.razuna.session.theuserid#.csv"> --->
	<!--- </cfif> --->
	<!--- Feedback --->
	<cfinvoke component="defaults" method="trans" transid="download_folder_output2" returnvariable="download_folder_output2" />
...
<cffunction name="getchildfolders" access="public" returntype="string" >
    <cfargument name="parentid" type="string" required="yes" default=0 >
    ...
    <cfquery name="objnav" datasource="#arguments.thestruct.razuna.application.datasource#" cachedwithin="1" region="razcache">
        SELECT /* #cachetoken#getchildfoldersobjnav */ folder_id, folder_name
        FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
    ...
    <cfloop query="objnav">
		...
		SELECT /* #cachetoken#getchildfolderscheckforkids*/ folder_id, folder_name
		FROM  #arguments.thestruct.razuna.session.hostdbprefix#folders
        ...
        <cfif checkforkids.recordcount gt 0><!--- this child has kids too! add it to the subfolderlist, then make the recursive call... --->
			...
			<cfset getchildfolders(parentid=objnav.folder_id, level=arguments.level + 1, thestruct=arguments.thestruct) >
...
<cffunction name="getInSearchSelection" output="false" returntype="query">
	...
	<!--- Subscribe folder details --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getInSearchSelection */ f.folder_id, f.folder_name,
	<!--- Permission follow but not for sysadmin and admin --->
	<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
		CASE
			<!--- Check permission on this folder --->
...
<cffunction name="GetTotalAllAssets" output="false" returntype="query" >
	<cfargument name ="folder_id" required="true" type="string">
	<cfargument name="thestruct" type="struct" required="true" />
	<cfset var qry = ''>
	<cfset var qry_sf = ''>
	<!--- Get list of all subfolders --->
	<cfset var subfolderslist = getchildfolders(parentid=arguments.folder_id, thestruct=arguments.thestruct)>
	<!--- Get file totals for main folder. Create clone of variable else original variable is modified when its value is changed. --->
	<cfset qry  = duplicate(filetotalalltypes(folder_id=arguments.folder_id, folderaccess='', sortby='scr', thestruct=arguments.thestruct))>
	<!--- Loop over all subfolders, get totals and add to main folder total --->
	<cfloop list="#subfolderslist#" index="i">
		<cfset qry_sf  = filetotalalltypes(folder_id=i, folderaccess='', sortby='scr', thestruct=arguments.thestruct)>
		<cfloop query = "qry_sf">
...
<cffunction name="checkfolder" output="false" returntype="boolean" >
	<cfargument name="folder_id" required="true" type="string">
	<cfargument name="thestruct" type="struct" required="true">
...
<cffunction name="getFlatFolderList" output="false" returntype="query">
	...
	<cfquery dataSource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getFlatFolderList */ f.folder_of_user, f.folder_id, f.folder_name, '0' as folder_level, f.folder_name as folder_path, <cfif arguments.thestruct.razuna.application.thedatabase EQ "h2">NVL<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mssql">isnull</cfif>(u.user_login_name,'Obsolete') as username
	FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON u.user_id = f.folder_owner
...
<cffunction name="recfoldername" output="false" access="public" returntype="query">
	...
	<cfloop query="arguments.theqry">
		<!--- Query --->
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="local_query" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#recfolder */ f.folder_of_user, f.folder_id, f.folder_name, '#thelevel#' as folder_level, <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">'#folder_path# / ' + f.folder_name<cfelse>concat('#folder_path# / ',f.folder_name)</cfif> as folder_path, <cfif arguments.thestruct.razuna.application.thedatabase EQ "h2">NVL<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mssql">isnull</cfif>(u.user_login_name,'Obsolete') as username
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON u.user_id = f.folder_owner
		WHERE f.folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#folder_id#">
		AND f.folder_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> f.folder_id_r
		AND f.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		</cfquery>
