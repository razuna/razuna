...
<cffunction name="getTreeByCollection" output="false" access="public" returntype="query">
	...
	<!--- Do the select --->
	<cfquery datasource="#request.razuna.application.datasource#" name="f_1" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken##request.razuna.session.theUserID#getTreeByCollection */ #Arguments.ColumnList#,
		<!--- Permission follow but not for sysadmin and admin --->
		<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
			CASE
				<!--- If this folder is protected with a group and this user belongs to this group --->
				WHEN EXISTS(
					SELECT fg.folder_id
					FROM #request.razuna.session.hostdbprefix#folders_groups fg, ct_groups_users gu
					WHERE fg.folder_id_r = f.folder_id
					AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
				...
				WHEN EXISTS(
					...
					FROM #request.razuna.session.hostdbprefix#folders_groups fg2 LEFT JOIN ct_groups_users gu2 ON gu2.ct_g_u_grp_id = fg2.grp_id_r AND gu2.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
				...
				WHEN ( f.folder_of_user = 't' AND f.folder_owner = '#request.razuna.session.theUserID#' ) THEN 'unlocked'
		...
		<cfelse>
			CASE
				WHEN ( f.folder_of_user = 't' AND f.folder_owner = '#request.razuna.session.theUserID#' AND f.folder_name = 'my folder') THEN 'unlocked'
	...
	<cfif Arguments.id gt 0>
		f.folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id_r
		AND
	...
	<cfif not request.razuna.session.is_system_admin>
		AND
			(
			<cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2">NVL<cfelseif request.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif request.razuna.application.thedatabase EQ "mssql">isnull</cfif>(f.folder_of_user,<cfqueryparam cfsqltype="cf_sql_varchar" value="f">) <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
			OR f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#">
			)
	</cfif>
...
<cffunction name="getfolder" output="false" access="public" description="GET FOLDER RECORD" returntype="query">
	...
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<cfquery name="qLocal" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
	...
	<cfif structKeyExists(arguments,'avoid_link_path') AND arguments.avoid_link_path EQ 'yes'>
		AND (f.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR f.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR f.link_path IS NULL)
	</cfif>
	...
	<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
		AND (
			...
			EXISTS(
				...
				WHERE fg.folder_id_r = f.folder_id
				AND fg.grp_permission IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="r,w,x" list="true">)
				AND fg.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				AND (
					<cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2">
						NVL(fg.grp_id_r, 0)
			...
			NOT EXISTS(
				...
				WHERE gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
				AND fg.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
...
<cffunction  name="getfolderdesc" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#folders_desc
...
<cffunction name="getfoldernames" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#folders_name
	WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
...
<cffunction  name="getfoldergroups" output="false">
	...
	<cfif arguments.qrygroup.recordcount NEQ 0>
		<cfquery datasource="#request.razuna.application.datasource#" name="thegroups" cachedwithin="1" region="razcache">
...
<cffunction  name="add" output="true" returntype="string">
	...
	<cfif arguments.thestruct.link_path EQ "">
		<cftry>
			...
			<cfquery datasource="#request.razuna.application.datasource#" name="samefolder">
			SELECT folder_id
			FROM #request.razuna.session.hostdbprefix#folders
			...
			<cfif samefolder.recordcount EQ 0>
				...
				<cfif request.razuna.application.storage EQ "local" OR request.razuna.application.storage EQ "akamai">
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/vid")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/vid" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/doc")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/doc" mode="775">
					</cfif>
					<cfif !directoryexists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/aud")>
						<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/aud" mode="775">
					</cfif>
				</cfif>
	...
	<cfelse>
		...
		<cfthread intstruct="#arguments.thestruct#">
			...
			<cfif request.razuna.application.storage EQ "local" OR request.razuna.application.storage EQ "akamai">
				<cfif !directoryexists("#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#")>
						<cfdirectory action="create" directory="#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#" mode="775">
				</cfif>
				<cfif !directoryexists("#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/img")>
					<cfdirectory action="create" directory="#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/img" mode="775">
				</cfif>
				<cfif !directoryexists("#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/vid")>
					<cfdirectory action="create" directory="#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/vid" mode="775">
				</cfif>
				<cfif !directoryexists("#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/doc")>
					<cfdirectory action="create" directory="#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/doc" mode="775">
				</cfif>
				<cfif !directoryexists("#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/aud")>
					<cfdirectory action="create" directory="#attributes.intstruct.assetpath#/#request.razuna.session.hostid#/#attributes.intstruct.newfolderid#/aud" mode="775">
				</cfif>
			</cfif>
...
<cffunction name="fnew_detail" output="true" returntype="string" access="public">
		...
		<cfloop list="#arguments.thestruct.langcount#" index="langindex">
			...
			<cfif structkeyexists(arguments.thestruct, "folder_name_#langindex#")>
				...
				<cfif thisfolder CONTAINS langindex AND evaluate(thisfolder) NEQ "">
					<cfquery datasource="#request.razuna.application.datasource#">
					INSERT INTO #request.razuna.session.hostdbprefix#folders_name
			...
			<cfif thisfield CONTAINS langindex>
				<cfquery datasource="#request.razuna.application.datasource#">
				INSERT INTO #request.razuna.session.hostdbprefix#folders_desc
	...
	<cfset log_folders(theuserid=request.razuna.session.theuserid,logaction='Add',logdesc='#added#: #arguments.thestruct.folder_name# (ID: #newfolderid#)', hostid=request.razuna.session.hostid)>
	<!--- Flush Cache --->
...
<cffunction name="apply_custom_shared_setting" output="false" returntype="void">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="getparentsharesettings">
	...
	AND subf.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
...
<cffunction name="trash" output="true">
	...
	<cfset resetcachetoken(type="labels", hostid=request.razuna.session.hostid)>
	<!--- Return --->
...
<cffunction name="remove_folder_thread" output="false">
	...
	<cftry>
		...
		<cfquery datasource="#request.razuna.application.datasource#" name="foldername">
		...
		<cfquery datasource="#request.razuna.application.datasource#" name="parentid">
		...
		<cfif foldername.recordcount NEQ 0>
			...
			<!--- Delete main folder --->
			<cfquery datasource="#request.razuna.application.datasource#">
			DELETE FROM	#request.razuna.session.hostdbprefix#folders
			WHERE folder_id = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			</cfquery>
			<!--- Log --->
			<cfinvoke component="defaults" method="trans" transid="deleted" returnvariable="deleted" />
			<cfset log_folders(theuserid=request.razuna.session.theuserid,logaction='Delete',logdesc='#deleted#: #foldername.folder_name# (ID: #arguments.thestruct.folder_id#)', hostid=request.razuna.session.hostid)>
			<!--- Flush Cache --->
			<cfset variables.cachetoken = resetcachetoken(type="folders", hostid=request.razuna.session.hostid)>
			...
			<cfthread intstruct="#arguments.thestruct#" priority="low">
				...
				<cfif request.razuna.application.thedatabase EQ "mssql">
					<!--- <cfquery datasource="#request.razuna.application.datasource#">
					ALTER TABLE #request.razuna.application.theschema#.#request.razuna.session.hostdbprefix#folders DROP CONSTRAINT
					</cfquery> --->
				<!--- MySQL --->
				<cfelseif request.razuna.application.thedatabase EQ "mysql">
					<cfquery datasource="#request.razuna.application.datasource#">
				...
				<cfelseif request.razuna.application.thedatabase EQ "h2">
					<cfquery datasource="#request.razuna.application.datasource#">
					ALTER TABLE #request.razuna.session.hostdbprefix#folders SET REFERENTIAL_INTEGRITY false
					</cfquery>
				</cfif>
				<!--- Delete Subscribe --->
				...
				<cfloop list="#folderids#" index="thefolderid" delimiters=",">
					<cfif thefolderid NEQ attributes.intstruct.folder_id>
						...
						<!--- Get the Folder Name for the Log --->
						<cfquery datasource="#request.razuna.application.datasource#" name="foldernamesub">
						SELECT folder_name
						FROM #request.razuna.session.hostdbprefix#folders
						...
						<!--- Log --->
						<cfinvoke component="defaults" method="trans" transid="deleted" returnvariable="deleted" />
						<cfinvoke component="extQueryCaching" method="log_folders" theuserid="#request.razuna.session.theuserid#" logaction="Delete" logdesc="#deleted#: #foldernamesub.folder_name# (ID: #thefolderid#)" hostid="#request.razuna.session.hostid#" />
						<!--- Delete folder in DB --->
						<cfquery datasource="#request.razuna.application.datasource#">
						DELETE FROM	#request.razuna.session.hostdbprefix#folders
						WHERE folder_id = <cfqueryparam value="#thefolderid#" cfsqltype="CF_SQL_VARCHAR">
...
<cffunction name="trash_folder_thread" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="get_folder">
...
<cffunction name="_updateFilesInTrash" access="remote" output="false">
	...
	<cfloop query="arguments.qry_files">
		...
		<!--- Update record --->
		<cfquery datasource="#request.razuna.application.datasource#">
		UPDATE #request.razuna.session.hostdbprefix##_db#
		SET
...
<cffunction name="_updateFolderInTrash" access="remote" output="false">
	...
	<cfloop list="#arguments.folder_ids#" index="f">
		<!--- Update the in_trash --->
		<cfquery datasource="#request.razuna.application.datasource#">
		UPDATE #request.razuna.session.hostdbprefix#folders
...
<cffunction name="_getFilesInFolder" access="remote" output="false" returntype="Query">
	...
	<cfset var qry = "">
	<!--- Query --->
	<cfquery name="qry" datasource="#request.razuna.application.datasource#">
...
<cffunction name="gettrashfolder" output="false" returntype="Query">
	...
	f.folder_main_id_r
	<!--- Permfolder --->
	<cfif request.razuna.session.is_system_admin OR request.razuna.session.is_administrator>
		, 'X' as permfolder
	<cfelse>
		,
		CASE
			WHEN (
				...
				FROM #request.razuna.session.hostdbprefix#folders_groups fg5
				WHERE fg5.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				AND fg5.folder_id_r = f.folder_id
				AND (
					fg5.grp_id_r = '0'
					OR fg5.grp_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.thegroupofuser#" list="true">)
				)
			) = 'R' THEN 'R'
			WHEN (
				SELECT DISTINCT max(fg5.grp_permission)
			...
			WHEN (f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">) THEN 'X'
		END as permfolder
	</cfif>
	FROM #request.razuna.session.hostdbprefix#folders f
	...
	<cfif qry.RecordCount NEQ 0>
		...
		<cfloop query="qry">
			...
			<cfquery name="getColfolderIDs" datasource="#request.razuna.application.datasource#" >
			SELECT folder_id from #request.razuna.session.hostdbprefix#folders
			WHERE folder_level >= (
				SELECT
				folder_level
				FROM #request.razuna.session.hostdbprefix#folders
...
<!--- 	<cfset consoleoutput(true, true)>
 ...
 --->
	...
	<!--- Set the sessions --->
	<!--- <cfset request.razuna.session.file_id = ids>
	<cfset request.razuna.session.thefileid = ids> --->
	<!--- Flush Cache --->
	<!--- <cfset resetcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<cfset resetcachetoken(type="images", hostid=request.razuna.session.hostid)>
	<cfset resetcachetoken(type="videos", hostid=request.razuna.session.hostid)>
...
<cffunction name="restoreselectedfolders" output="false">
	<cfargument name="thestruct" type="struct">
	<!--- Get the details --->
	<cfquery datasource="#request.razuna.application.datasource#" name="thenewrootid">
		SELECT folder_id_r,folder_main_id_r,folder_level,folder_name FROM #request.razuna.session.hostdbprefix#folders
...
<cffunction name="restorefolder" output="false">
	...
	<!--- get the details --->
	<cfquery datasource="#request.razuna.application.datasource#" name="thenewrootid">
	SELECT folder_id_r,folder_main_id_r,folder_level FROM #request.razuna.session.hostdbprefix#folders
	WHERE folder_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.intofolderid#">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Get the Folder Name/Folder Level for the Log --->
...
<cffunction name="deleteassetsinfolder" output="false">
	...
	<!--- Images --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qryimg">
	Select img_id
	FROM #request.razuna.session.hostdbprefix#images
	...
	<!--- Videos --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qryvid">
	Select vid_id
	FROM #request.razuna.session.hostdbprefix#videos
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	...
	<!--- Audios --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qryaud">
	Select aud_id
	FROM #request.razuna.session.hostdbprefix#audios
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	...
	<!--- Docs --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qrydoc">
	Select file_id
	FROM #request.razuna.session.hostdbprefix#files
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	...
	<!--- Collections --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qrycol">
	Select col_id
	FROM #request.razuna.session.hostdbprefix#collections
	WHERE folder_id_r = <cfqueryparam value="#arguments.thefolderid#" cfsqltype="CF_SQL_VARCHAR">
	AND in_trash = <cfqueryparam value="F" cfsqltype="CF_SQL_VARCHAR">
	</cfquery>
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qryfolder">
	SELECT img_id as id
	FROM #request.razuna.session.hostdbprefix#images
	...
	<cfif qryfolder.recordcount EQ 0>
		<!--- Delete Folder --->
		<cfif request.razuna.application.storage EQ "local" OR request.razuna.application.storage EQ "akamai">
			<cfif isdefined("arguments.thestruct.assetpath") AND directoryexists("#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thefolderid#")>
				<cfdirectory action="delete" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#arguments.thefolderid#" recurse="true">
			</cfif>
...
<cffunction name="update" output="true" returntype="string">
	...
	WHERE folder_name = <cfqueryparam value="#arguments.thestruct.folder_name#" cfsqltype="cf_sql_varchar">
	AND folder_level = <cfqueryparam value="#arguments.thestruct.level#" cfsqltype="cf_sql_numeric">
	AND folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	AND folder_of_user <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="t" cfsqltype="cf_sql_varchar">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- If there is not a record with the same name continue --->
	<cfif #samefolder.recordcount# EQ 0>
		<!--- Get Folder Name for the Log
		<cfquery datasource="#request.razuna.application.datasource#" name="thisfolder">
			SELECT folder_name
			FROM #request.razuna.session.hostdbprefix#folders
		...
		<cfloop list="#arguments.thestruct.langcount#" index="langindex">
			...
			<cfif thisfolder CONTAINS langindex AND evaluate(thisfolder) NEQ "">
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="langFolder">
				SELECT folder_id_r
				FROM #request.razuna.session.hostdbprefix#folders_name
				WHERE folder_id_r = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
				...
				<cfif langFolder.recordCount GT 0>
					<cfquery datasource="#request.razuna.application.datasource#">
					UPDATE #request.razuna.session.hostdbprefix#folders_name
			...
			<cfif thisfield CONTAINS langindex>
				<!--- Check if description in this language exists --->
				<cfquery datasource="#request.razuna.application.datasource#" name="langDesc">
				SELECT folder_id_r
				FROM #request.razuna.session.hostdbprefix#folders_desc
				WHERE folder_id_r = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
				AND lang_id_r = <cfqueryparam value="#langindex#" cfsqltype="cf_sql_numeric">
				...
				<cfif langDesc.recordCount GT 0>
					<cfquery datasource="#request.razuna.application.datasource#">
					UPDATE #request.razuna.session.hostdbprefix#folders_desc
		...
		<cfif structkeyexists(arguments.thestruct,"perm_inherit")>
			<!--- Get the subfolders --->
			<cfquery datasource="#request.razuna.application.datasource#" name="arguments.thestruct.qrysubfolder">
			SELECT folder_id
			FROM #request.razuna.session.hostdbprefix#folders
			WHERE folder_id_r = <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
			AND folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="#arguments.thestruct.folder_id#" cfsqltype="CF_SQL_VARCHAR">
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			</cfquery>
		...
		<cfset log_folders(theuserid=request.razuna.session.theuserid,logaction='Update',logdesc='#updated#: #arguments.thestruct.folder_name# (ID: #arguments.thestruct.folder_id#)', hostid=request.razuna.session.hostid)>
		<!--- Flush Cache --->
		<cfset resetcachetoken(type="search", hostid=request.razuna.session.hostid)>
		<cfset resetcachetoken(type="labels", hostid=request.razuna.session.hostid)>
		<cfset resetcachetoken(type="images", hostid=request.razuna.session.hostid)>
...
<cffunction name="filetotalcount_temp" output="false">
	<cfset var s = structNew()>
	<cfset s.thetotal = request.razuna.session.total_count_of_folder>
...
<cffunction name="filetotalcount" output="false">
	...
	<!--- Param --->
	<cfparam name="request.razuna.session.showsubfolders" default="F">
	<cfparam name="request.razuna.session.customfileid" default="">
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Show assets from subfolders or not --->
	<cfif request.razuna.session.showsubfolders EQ "T">
		<cfinvoke method="getfoldersinlist" dsn="#request.razuna.application.datasource#" folder_id="#arguments.folder_id#" database="#request.razuna.application.thedatabase#" hostid="#request.razuna.session.hostid#" returnvariable="thefolders">
	...
	<cfset var total = "">
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="total" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#filetotalcount */
		...
		(
			...
			FROM #request.razuna.session.hostdbprefix#files fi, #request.razuna.session.hostdbprefix#folders f
			WHERE fi.folder_id_r = f.folder_id
			AND (f.folder_is_collection IS NULL OR folder_is_collection = '')
			AND fi.folder_id_r IS NOT NULL
			AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			AND fi.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			AND fi.in_trash = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="F">
			AND fi.is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			...
			<!--- If coming from custom view and the request.razuna.session.customfileid is not empty --->
			<cfif request.razuna.session.customfileid NEQ "">
				AND fi.file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
			</cfif>
		...
		(
			...
			FROM #request.razuna.session.hostdbprefix#images i, #request.razuna.session.hostdbprefix#folders f
			...
			AND i.is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			<cfif arguments.theoverall EQ "F" AND arguments.folder_id NEQ "">
				AND i.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
			</cfif>
			<!--- If coming from custom view and the request.razuna.session.customfileid is not empty --->
			<cfif request.razuna.session.customfileid NEQ "">
				AND i.img_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
			</cfif>
			<cfif arguments.folderaccess EQ 'R'>
		...
		(
			...
			FROM #request.razuna.session.hostdbprefix#videos v, #request.razuna.session.hostdbprefix#folders f
			...
			AND v.is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			<cfif arguments.theoverall EQ "F" AND arguments.folder_id NEQ "">
				AND v.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
			</cfif>
			<!--- If coming from custom view and the request.razuna.session.customfileid is not empty --->
			<cfif request.razuna.session.customfileid NEQ "">
				AND v.vid_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
			</cfif>
			<cfif arguments.folderaccess EQ 'R'>
		...
		(
			...
			FROM #request.razuna.session.hostdbprefix#audios a, #request.razuna.session.hostdbprefix#folders f
			...
			AND a.is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
			<cfif arguments.theoverall EQ "F" AND arguments.folder_id NEQ "">
				AND a.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
			</cfif>
			<!--- If coming from custom view and the request.razuna.session.customfileid is not empty --->
			<cfif request.razuna.session.customfileid NEQ "">
				AND a.aud_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
			</cfif>
			<cfif arguments.folderaccess EQ 'R'>
...
<cffunction name="filetotaltype" output="false">
	...
	<cfif arguments.thestruct.folder_id NEQ "">
		<cfif request.razuna.session.showsubfolders EQ "T">
			<cfinvoke method="getfoldersinlist" dsn="#request.razuna.application.datasource#" folder_id="#arguments.thestruct.folder_id#" database="#request.razuna.application.thedatabase#" hostid="#request.razuna.session.hostid#" returnvariable="thefolders">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Query Aliases --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_aliases" cachedwithin="1" region="razcache">
	...
	<cfif arguments.thestruct.kind EQ "img">
		...
		<cfif request.razuna.session.customfileid NEQ "">
			AND img_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfif>
	...
	<cfelseif arguments.thestruct.kind EQ "vid">
		...
		<cfif request.razuna.session.customfileid NEQ "">
			AND vid_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR expiry_date is null)
		</cfif>
	...
	<cfelseif arguments.thestruct.kind EQ "aud">
		...
		<cfif request.razuna.session.customfileid NEQ "">
			AND aud_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR expiry_date is null)
		</cfif>
	...
	<cfelseif arguments.thestruct.kind EQ "doc">
		...
		<cfif request.razuna.session.customfileid NEQ "">
			AND file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR expiry_date is null)
		</cfif>
...
<cffunction name="getAssetsDetails" output="false"  returntype="Any" >
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qTab" cachedwithin="1" region="razcache">
	...
	UNION ALL
		...
		FROM #request.razuna.session.hostdbprefix#audios a LEFT JOIN #request.razuna.session.hostdbprefix#audios_text aut ON a.aud_id = aut.aud_id_r AND aut.lang_id_r = 1 AND a.host_id = aut.host_id
	...
	<cfif qTab.RecordCount NEQ 0>
		<cfloop query="qTab">
			...
			<cfquery name="alert_col" datasource="#request.razuna.application.datasource#">
			SELECT file_id_r
			FROM #request.razuna.session.hostdbprefix#collections_ct_files
...
<cffunction name="fileTotalAllTypes" output="false" >
	...
	FROM ct_aliases c, #request.razuna.session.hostdbprefix#files f
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qTab" cachedwithin="1" region="razcache">
		...
		FROM #request.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND SUBSTR<cfif request.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) = <cfqueryparam cfsqltype="cf_sql_varchar" value="doc">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		AND is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif request.razuna.session.customfileid NEQ "">
			AND file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif arguments.folderaccess EQ 'R'>
		...
		FROM #request.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND SUBSTR<cfif request.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) = <cfqueryparam cfsqltype="cf_sql_varchar" value="xls">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		AND is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif request.razuna.session.customfileid NEQ "">
		...
		FROM #request.razuna.session.hostdbprefix#files
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND SUBSTR<cfif request.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) = <cfqueryparam cfsqltype="cf_sql_varchar" value="pdf">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		AND is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif request.razuna.session.customfileid NEQ "">
		...
		WHERE folder_id_r = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
		AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND ((SUBSTR<cfif request.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="doc">
		AND SUBSTR<cfif request.razuna.application.thedatabase EQ "mssql">ING</cfif>(file_extension,1,3) <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="xls">
		AND file_extension <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="pdf">)
		OR  file_type = 'other')
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		AND is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif request.razuna.session.customfileid NEQ "">
...
<cffunction  name="setaccess" output="true" returntype="string">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="fprop" cachedwithin="1" region="razcache">
	...
	<cfelse>
		CASE
			...
			WHEN (<cfif arguments.sf>f.sf_who<cfelse>f.folder_owner</cfif> = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">) THEN 'X'
		END as permfolder
	</cfif>
	<cfif arguments.sf>
		FROM #request.razuna.session.hostdbprefix#smart_folders f LEFT JOIN #request.razuna.session.hostdbprefix#folders_groups fg ON f.sf_id = fg.folder_id_r AND f.host_id = fg.host_id
		WHERE f.sf_id = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	<cfelse>
		FROM #request.razuna.session.hostdbprefix#folders f LEFT JOIN #request.razuna.session.hostdbprefix#folders_groups fg ON f.folder_id = fg.folder_id_r AND f.host_id = fg.host_id
	...
	<cfif (request.razuna.session.is_system_admin OR request.razuna.session.is_administrator) OR fprop.folder_owner EQ request.razuna.session.theuserid>
		<cfset var folderaccess = "x">
	</cfif>
	<!--- If request.razuna.session.customaccess is here and is not empty --->
	<cfif structKeyExists(request.razuna.session,"customaccess") AND request.razuna.session.customaccess NEQ "">
		<cfset var folderaccess = request.razuna.session.customaccess>
	</cfif>
...
<cffunction  name="setaliasaccess" output="true" returntype="string" >
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="aliasfolders" cachedwithin="1" region="razcache">
...
<cffunction  name="move" output="true">
	...
	<cftry>
		...
		<!--- Flush Cache --->
		<cfset resetcachetoken(type="folders", hostid=request.razuna.session.hostid)>
		<!--- Clear request.razuna.session.type --->
		<cfset request.razuna.session.type = "">
		<!--- Log --->
		<cfinvoke component="defaults" method="trans" transid="moved" returnvariable="moved" />
		<cfset log_folders(theuserid=request.razuna.session.theuserid,logaction='Move',logdesc='#moved#: #foldername.folder_name# (ID: #arguments.thestruct.tomovefolderid#)', hostid=request.razuna.session.hostid)>
...
<cffunction name="recfolder" output="false" access="public" returntype="string">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="local_query" cachedwithin="1" region="razcache">
	...
	AND folder_id <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> folder_id_r
	AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
...
<cffunction name="getuserfolder" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		...
		AND folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#">
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		</cfquery>
...
<cffunction name="getallassets" output="true" returnType="query">
	...
	<cfif arguments.thestruct.pages EQ "" OR arguments.thestruct.pages EQ "current">
		...
		<cfif arguments.thestruct.qry_filecount LTE request.razuna.session.rowmaxpage>
		...
		<cfif request.razuna.session.offset EQ 0>
			<cfset var min = 0>
			<cfset var max = request.razuna.session.rowmaxpage>
		<cfelse>
			<cfset var min = request.razuna.session.offset * request.razuna.session.rowmaxpage>
			<cfset var max = (request.razuna.session.offset + 1) * request.razuna.session.rowmaxpage>
			<cfif request.razuna.application.thedatabase EQ "db2">
				<cfset var min = min + 1>
			</cfif>
		</cfif>
	...
	<!--- Set sortby variable --->
	<cfset var sortby = request.razuna.session.sortby>
	<!--- Set the order by --->
	...
	<cfset var qry = "">
		...
		<!--- MySQL Offset --->
		<cfset var mysqloffset = request.razuna.session.offset * request.razuna.session.rowmaxpage>
		<!--- For aliases --->
		<cfset var alias_img = '0,'>
		<cfset var alias_vid = '0,'>
		<cfset var alias_aud = '0,'>
		...
		FROM #request.razuna.session.hostdbprefix#images i
		LEFT JOIN #request.razuna.session.hostdbprefix#images_text it ON i.img_id = it.img_id_r AND it.lang_id_r = 1 AND i.host_id = it.host_id
		LEFT JOIN #request.razuna.session.hostdbprefix#xmp x ON x.id_r = i.img_id AND i.host_id = x.host_id
		...
		FROM #request.razuna.session.hostdbprefix#videos v LEFT JOIN #request.razuna.session.hostdbprefix#videos_text vt ON v.vid_id = vt.vid_id_r AND vt.lang_id_r = 1 AND v.host_id = vt.host_id
		...
		FROM #request.razuna.session.hostdbprefix#files f LEFT JOIN #request.razuna.session.hostdbprefix#files_desc ft ON f.file_id = ft.file_id_r AND ft.lang_id_r = 1 AND f.host_id = ft.host_id LEFT JOIN #request.razuna.session.hostdbprefix#files_xmp x ON x.asset_id_r = f.file_id AND f.host_id = x.host_id
		WHERE f.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
		AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		AND f.in_trash = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="F">
		AND f.is_available <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="2">
		<cfif arguments.thestruct.folderaccess EQ 'R'>
			AND (f.expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR f.expiry_date is null)
		</cfif>
		<!--- If coming from custom view and the request.razuna.session.customfileid is not empty --->
		<cfif request.razuna.session.customfileid NEQ "">
			AND f.file_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfif>
		<cfif qry_aliases.recordCount>
			OR (
		...
		<cfif request.razuna.application.thedatabase EQ "mssql">
			<cfif !structKeyExists(arguments.thestruct,'widget_style') OR arguments.thestruct.widget_style NEQ 's'>
				  ...
				  WHERE RowNum > #mysqloffset# AND RowNum <= #mysqloffset+request.razuna.session.rowmaxpage#
			</cfif>
		</cfif>
		...
		<cfif arguments.thestruct.pages EQ "" OR arguments.thestruct.pages EQ "current">
			...
			<cfif request.razuna.application.thedatabase EQ "mysql" OR request.razuna.application.thedatabase EQ "h2">
				...
				<cfif !structKeyExists(arguments.thestruct,'widget_style') OR arguments.thestruct.widget_style NEQ 's'>
					 LIMIT #mysqloffset#,#request.razuna.session.rowmaxpage#
				</cfif>
			</cfif>
		</cfif>
	</cfquery>
	...
	<cfif request.razuna.session.customfileid NEQ "">
		...
		SELECT *
		FROM qry
		WHERE id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.customfileid#" list="true">)
		</cfquery>
	</cfif>
	<!--- Only get the labels if in the combinded view --->
	<cfif request.razuna.session.view EQ "combined">
		<!--- Get the cachetoken for here --->
		<cfset variables.cachetokenlabels = getcachetoken(type="labels", hostid=request.razuna.session.hostid)>
...
<cffunction name="addCustomFieldsToQuery" output="false" returntype="query">
	...
	<!--- Get cachetokens --->
	<cfset variables.cachetokensetting = getcachetoken(type="settings", hostid=request.razuna.session.hostid)>
	<cfset variables.cachetokengeneral = getcachetoken(type="general", hostid=request.razuna.session.hostid)>
	<!--- Set var --->
	<cfset var cf_list = "">
	<cfset var qry_cf = "">
	...
	<cfquery name="qry_cf_img" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
	SELECT /* #variables.cachetokensetting#qry_cf_img */ custom_value
	FROM #request.razuna.session.hostdbprefix#custom
	WHERE custom_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="cf_images_metadata">
	AND host_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.hostid#">
	</cfquery>
	<cfquery name="qry_cf_vid" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
	...
	<cfquery name="qry_cf_aud" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
	...
	<cfquery name="qry_cf_doc" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
	...
	<cfloop query="arguments.theqry">
		<cfquery name="qry_cf" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
		...
		FROM #request.razuna.session.hostdbprefix#custom_fields_values cfv RIGHT JOIN #request.razuna.session.hostdbprefix#custom_fields_text cft INNER JOIN #request.razuna.session.hostdbprefix#custom_fields cf ON cf.cf_id = cft.cf_id_r
		...
		WHERE
		cft.lang_id_r = 1
		AND cft.host_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.hostid#">
		<cfif kind EQ "img">
...
<cffunction name="trashall" output="true">
	...
	<cfloop list="#arguments.thestruct.id#" delimiters="," index="i">
		<cfquery name="isalias" datasource="#request.razuna.application.datasource#">
...
<cffunction name="removeall" output="true">
	...
	<cfif arguments.thestruct.file_id EQ "all">
		...
		<cfinvoke component="search" method="getAllIdsMain" thestruct="#arguments.thestruct#" searchupc="#request.razuna.session.search.searchupc#" searchtext="#request.razuna.session.search.searchtext#" searchtype="#request.razuna.session.search.searchtype#" searchrenditions="#request.razuna.session.search.searchrenditions#" searchfolderid="#request.razuna.session.search.searchfolderid#" hostid="#request.razuna.session.hostid#" returnvariable="ids">
			<!--- Set the fileid --->
...
<cffunction name="getfoldersinlist" output="false">
	...
	<cfif arguments.prefix EQ "">
		<cfset arguments.prefix = request.razuna.session.hostdbprefix>
	</cfif>
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Query --->
...
<cffunction name="getfoldersfortree" access="public" output="true">
	...
	<cfif NOT request.razuna.session.is_system_admin AND NOT request.razuna.session.is_administrator>
		<cfset request.razuna.session.showmyfolder = "F">
	</cfif>
	<!--- Param --->
	<cfparam default="0" name="request.razuna.session.thefolderorg">
	<cfparam default="0" name="request.razuna.session.type">
	<cfparam default="0" name="request.razuna.session.thegroupofuser">
	<cfset var qry = "">
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken##request.razuna.session.theUserID#getfoldersfortree */ folder_id, folder_name, folder_id_r, folder_of_user, folder_owner, folder_level, in_trash,link_path, username, perm, subhere, permfolder
	FROM (
		SELECT f.folder_id, f.folder_id_r, f.folder_of_user, f.folder_owner, f.folder_level,f.in_trash,f.link_path,
		<cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2">NVL<cfelseif request.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif request.razuna.application.thedatabase EQ "mssql">isnull</cfif>(u.user_login_name,'Obsolete') as username,
		<!--- Folder name --->
		...
		<cfelse>
			CASE
				WHEN EXISTS (
					...
					AND lang_id_r = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.thelangid#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				) THEN (
					SELECT folder_name
					FROM #request.razuna.session.hostdbprefix#folders_name
		...
		<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
			CASE
				...
				<!--- If this is the user folder or he is the owner --->
				WHEN f.folder_owner = '#request.razuna.session.theUserID#' THEN 'unlocked'
				<!--- If this is the upload bin --->
				WHEN f.folder_id = '1' THEN 'unlocked'
		...
		CASE
			...
			WHEN EXISTS(
				SELECT <cfif request.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>*
				FROM #request.razuna.session.hostdbprefix#folders s1
				WHERE s1.folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id
				AND s1.folder_id_r = f.folder_id
				ANd s1.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				AND s1.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				<!--- AND lower(s.folder_of_user) = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">  --->
				<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
					AND s1.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
				</cfif>
				<!--- If this is a move then dont show the folder that we are moving --->
				<cfif (arguments.actionismove EQ "T" AND request.razuna.session.type EQ "movefolder") OR request.razuna.session.type EQ "copyfolder">
					AND s1.folder_id <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#request.razuna.session.thefolderorg#">
				</cfif>
				<!--- RAZ-583 : exclude link folder from subfolder count --->
				<cfif request.razuna.session.type NEQ ''>
					AND (s1.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR s1.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR s1.link_path IS NULL)
				</cfif>
			...
			WHEN EXISTS(
				SELECT <cfif request.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>*
				FROM #request.razuna.session.hostdbprefix#folders s2, #request.razuna.session.hostdbprefix#folders_groups fg3
				WHERE s2.folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id
				AND s2.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				...
				AND fg3.grp_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.thegroupofuser#" list="true">)
				<!--- If this is a move then dont show the folder that we are moving --->
				<cfif (arguments.actionismove EQ "T" AND request.razuna.session.type EQ "movefolder") OR request.razuna.session.type EQ "copyfolder">
					AND s2.folder_id <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#request.razuna.session.thefolderorg#">
				</cfif>
				<!--- RAZ-583 : exclude link folder from subfolder count --->
				<cfif request.razuna.session.type NEQ ''>
					AND (s2.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR s2.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR s2.link_path IS NULL)
				</cfif>
				<cfif request.razuna.application.thedatabase EQ "oracle">
					AND ROWNUM = 1
				<cfelseif  request.razuna.application.thedatabase EQ "mysql" OR request.razuna.application.thedatabase EQ "h2">
			...
			WHEN EXISTS(
				SELECT <cfif request.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>*
				FROM #request.razuna.session.hostdbprefix#folders s3, #request.razuna.session.hostdbprefix#folders_groups fg4
				WHERE s3.folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id
				...
				AND s3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				AND s3.host_id = fg4.host_id
				<!--- If this is a move then dont show the folder that we are moving --->
				<cfif (arguments.actionismove EQ "T" AND request.razuna.session.type EQ "movefolder") OR request.razuna.session.type EQ "copyfolder">
					AND s3.folder_id <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#request.razuna.session.thefolderorg#">
				</cfif>
				<!--- RAZ-583 : exclude link folder from subfolder count --->
				<cfif request.razuna.session.type NEQ ''>
					AND (s3.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR s3.link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR s3.link_path IS NULL)
				</cfif>
				<cfif request.razuna.application.thedatabase EQ "oracle">
					AND ROWNUM = 1
				<cfelseif  request.razuna.application.thedatabase EQ "mysql" OR request.razuna.application.thedatabase EQ "h2">
		...
		FROM #request.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON u.user_id = f.folder_owner
		WHERE
		<cfif theid gt 0>
			f.folder_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> f.folder_id_r
			AND
		...
		<cfif iscol EQ "F" AND (NOT request.razuna.session.is_system_admin AND NOT request.razuna.session.is_administrator)>
			AND
				(
				<cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2">NVL<cfelseif request.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif request.razuna.application.thedatabase EQ "mssql">isnull</cfif>(f.folder_of_user,<cfqueryparam cfsqltype="cf_sql_varchar" value="f">) <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
				OR f.folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#">
	...
	<cfif request.razuna.session.type EQ "movefolder" OR request.razuna.session.type EQ "copyfolder">
		AND itb.folder_id <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="#request.razuna.session.thefolderorg#">
	</cfif>
	<!--- RAZ-583 : exclude link folder from select --->
	<cfif request.razuna.session.type NEQ ''>
		AND (link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '/' OR link_path <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> '\\' OR link_path IS NULL)
	</cfif>
	ORDER BY folder_name
	</cfquery>

	<!--- <cfset consoleoutput(true, true)>
	<cfset console(request.razuna.session.theuserid)>
	...
	<cfif theid EQ 0>
		...
		<cfif request.razuna.session.showmyfolder EQ "F" AND iscol NEQ "T">
			<!--- <cfset console("HERE !!!!!!!!!!!!!!!")> --->
			<cfquery dbtype="query" name="qry">
			SELECT *
			FROM qry
			WHERE (folder_of_user = <cfqueryparam cfsqltype="cf_sql_varchar" value="f"> OR folder_of_user IS NULL)
			OR (folder_name = <cfqueryparam cfsqltype="cf_sql_varchar" value="my folder"> AND folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#">)
			OR folder_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="1">
			OR folder_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#">
			</cfquery>
		</cfif>
	</cfif>
	...
	<cfif arguments.actionismove EQ "F">
		...
		<cfloop query="qry">
			<cfif qry.in_trash EQ 'F'>
				...
				<cfif theid EQ 0>
					<cfif iscol EQ "F">
						<cfif request.razuna.session.theuserid NEQ folder_owner AND folder_owner NEQ "">
	...
	<cfelse>
		<cfloop query="qry">
			<cfif qry.in_trash EQ 'F'>
				...
				<cfif structKeyExists(request.razuna.session, "thefileid") AND NOT request.razuna.session.thefileid CONTAINS ",">
				...
				<!--- Folder name --->
				<cfset var _folder_name = folder_name>
				<cfif iscol EQ "F" AND folder_name EQ "my folder" AND (request.razuna.session.is_system_admin OR request.razuna.session.is_administrator)>
					<cfif request.razuna.session.theuserid NEQ folder_owner AND folder_owner NEQ "">
				...
				<cfif !listfindnocase('w,x',qry.permfolder)>
					<!--- Nothing here --->
				<!--- movefile --->
				<cfelseif request.razuna.session.type EQ "movefile">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfif arguments.kind EQ "search">
							<cfset _attr.onclick = "$('##div_choosefolder_status_#request.razuna.session.tmpid#').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#', function(){$('##div_choosefolder_status_#request.razuna.session.tmpid#').html('The file(s) are being moved now.<br />Note: For a large batch of files this can take some time until it reflects in the system!<br />You can close this window.');});">
						<cfelse>
							<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#', function(){$('##div_choosefolder_status_#request.razuna.session.tmpid#').html('The file(s) are being moved now.<br />Note: For a large batch of files this can take some time until it reflects in the system!<br />You can close this window.');});">
						</cfif>
					</cfif>
				<!--- movefolder --->
				<cfelseif request.razuna.session.type EQ "movefolder">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#', function(){$('##explorer').load('index.cfm?fa=#_attr_fa_explorer#');#_fromtrash#});destroywindow(1);return false;">
					</cfif>
				<!--- copyfolder --->
				<cfelseif request.razuna.session.type EQ "copyfolder">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "loadcontent('div_forall','index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#&inherit_perm='+$('##perm_inherit').is(':checked'), function(){#_fromtrash#');});$('#_attr_div_explorer#').load('index.cfm?fa=#_attr_fa_explorer#');destroywindow(1);return false;">
					</cfif>
				<!--- restorefolder --->
				<cfelseif request.razuna.session.type EQ "restorefolder">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#');$('##rightside').load('index.cfm?fa=c.folder_explorer_trash&trashkind=folders');destroywindow(1);return false;">
					</cfif>
				<!--- restoreselectedfolders --->
				<cfelseif request.razuna.session.type EQ "restoreselectedfolders">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#&iscol=#iscol#');destroywindow(1);return false;">
					</cfif>
				<!--- restorefile --->
				<cfelseif request.razuna.session.type EQ "restorefile">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset var _for_here_1 = "">
						<cfset var _for_here_2 = "">
						<cfif request.razuna.session.thefileid CONTAINS ",">
							<cfset var _for_here_1 = "loadoverlay();">
						</cfif>
						<cfif NOT request.razuna.session.thefileid CONTAINS ",">
							<cfset var _for_here_2_start = "loadcontent('thewindowcontent1','index.cfm?fa=c.">
							<cfif request.razuna.session.thetype EQ "doc">
								<cfset _for_here_2_sub = "files">
							<cfelseif request.razuna.session.thetype EQ "img">
								<cfset _for_here_2_sub = "images">
							<cfelseif request.razuna.session.thetype EQ "vid">
								<cfset _for_here_2_sub = "videos">
							<cfelseif request.razuna.session.thetype EQ "aud">
								<cfset _for_here_2_sub = "audios">
							</cfif>
							<cfset var _for_here_2 = _for_here_2_start & _for_here_2_sub & "_detail&file_id=" & request.razuna.session.thefileid & "&what=" & _for_here_2_sub & "&loaddiv=&folder_id=" & folder_id>
						</cfif>
						<cfset _attr.onclick = "#_for_here_1#$('##rightside').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#&intolevel=#folder_level#', function(){loadfolderwithdelay('#request.razuna.session.thefolderorg#');$('##bodyoverlay').remove();});#_attr_destroywindow##_for_here_2#');">
					</cfif>
				<!--- restorefileall --->
				<cfelseif request.razuna.session.type EQ "restorefileall">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
					</cfif>
				<!--- restoreselectedfiles --->
				<cfelseif request.razuna.session.type EQ "restoreselectedfiles">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#');delayloadingoflist();destroywindow(1);return false;">
					</cfif>
				<!--- restorefolderall --->
				<cfelseif request.razuna.session.type EQ "restorefolderall">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
					</cfif>
				<!--- saveaszip or as a collection --->
				<cfelseif request.razuna.session.type EQ "saveaszip" OR request.razuna.session.type EQ "saveascollection">
					<cfset _attr.onclick = "loadcontent('win_choosefolder_#request.razuna.session.tmpid#','index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#');">
				<!--- upload --->
				<cfelseif request.razuna.session.type EQ "uploadinto">
					<cfset _attr.onclick = "showwindow('index.cfm?fa=c.asset_add&folder_id=#folder_id#','Add your files',650,1);return false;">
				<!--- customization --->
				<cfelseif request.razuna.session.type EQ "customization">
				...
				<!--- group detail--->
				<cfelseif request.razuna.session.type EQ "groups_detail">
				...
				<!--- scheduler --->
				<cfelseif request.razuna.session.type EQ "scheduler">
				...
				<!--- choose a collection --->
				<cfelseif request.razuna.session.type EQ "choosecollection">
				...
				<cfelseif request.razuna.session.type EQ "restore_collection_file">
					<cfset _attr.onclick = "loadcontent('div_choosecol','index.cfm?fa=c.collection_chooser&withfolder=T&folder_id=#folder_id#');">
				<!--- Restore all collection files in the trash --->
				<cfelseif request.razuna.session.type EQ "restoreallcollectionfiles">
					<cfset _attr.onclick = "loadcontent('div_choosecol','index.cfm?fa=c.collection_chooser&withfolder=T&folder_id=#folder_id#');">
				<!--- Restore selected collection files in the trash --->
				<cfelseif request.razuna.session.type EQ "restoreselectedcolfiles">
					<cfset _attr.onclick = "loadcontent('div_choosecol','index.cfm?fa=c.collection_chooser&withfolder=T&folder_id=#folder_id#');">
				<!--- choose a folder for restore collection --->
				<cfelseif request.razuna.session.type EQ "restore_collection">
					<cfset _attr.onclick = "loadcontent('collections','index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
				<!--- Restore all collections in the trash --->
				<cfelseif request.razuna.session.type EQ "restoreallcollections">
					<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
				<!--- Restore selected collections  --->
				<cfelseif request.razuna.session.type EQ "restoreselectedcollection">
					<cfset _attr.onclick = "loadcontent('collections','index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#');destroywindow(1);return false;">
				<!--- Restore collection folder in the trash --->
				<cfelseif request.razuna.session.type EQ "restorecolfolder">
					<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
				<!--- Restore all collection folder in the trash --->
				<cfelseif request.razuna.session.type EQ "restorecolfolderall">
					<cfset _attr.onclick = "$('##rightside').load('index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
				<!--- Restore all collection folder in the trash --->
				<cfelseif request.razuna.session.type EQ "restoreselectedcolfolder">
					<cfset _attr.onclick = "loadcontent('folders','index.cfm?fa=#request.razuna.session.savehere#&intofolderid=#folder_id#&intolevel=#folder_level#');destroywindow(1);return false;">
				<!--- copy metadata --->
				<cfelseif request.razuna.session.type EQ "copymetadata">
					<cfset _attr.onclick = "loadcontent('result','index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#&what=#request.razuna.session.thetype#&fid=#request.razuna.session.file_id#');destroywindow(2);return false;">
				<!--- Plugin --->
				<cfelseif request.razuna.session.type EQ "plugin">
					<cfset _attr.onclick = "$('##wf_folder_id_2').val('#folder_id#'); $('##wf_folder_name_2').val('#Encodeforjavascript(folder_name)#');destroywindow(1);">
				<!--- From Smart Folder --->
				<cfelseif request.razuna.session.type EQ "sf_download">
					<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#');$('##div_choosefolder_status_#request.razuna.session.tmpid#').html('All file(s) are going to be downloaded now and stored in the chosen folder!');return false;">
				<!--- Alias --->
				<cfelseif request.razuna.session.type EQ "alias">
					<cfif request.razuna.session.thefolderorg NEQ folder_id>
						<cfif arguments.kind EQ "search">
							<cfset _attr.onclick = "$('##div_choosefolder_status_#request.razuna.session.tmpid#').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#', function(){$('##div_choosefolder_status_#request.razuna.session.tmpid#').html('The alias has been created in the selected folder.<br />Note: If you want to create an alias for the same file(s) in another folder simply select it from the list above!<br />You can close this window.');});">
						<cfelse>
							<cfset _attr.onclick = "$('##div_forall').load('index.cfm?fa=#request.razuna.session.savehere#&folder_id=#folder_id#', function(){$('##div_choosefolder_status_#request.razuna.session.tmpid#').html('The alias has been created in the selected folder.<br />Note: If you want to create an alias for the same file(s) in another folder simply select it from the list above!<br />You can close this window.');});">
						</cfif>
					</cfif>
				</cfif>
...
<cffunction name="sharecheckperm" output="true">
	...
	<cfparam name="request.razuna.session.theuserid" default="">
	<cfparam name="request.razuna.session.iscol" default="F">
	<cfset var qry = "">
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Check if folder is even shared or not --->
	<cfif request.razuna.session.iscol EQ "F">
		<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	...
	<cfif qry.shared EQ "T">
		<cfif request.razuna.session.iscol EQ "F">
			<cfquery datasource="#request.razuna.application.datasource#" name="qryfolder" cachedwithin="1" region="razcache">
		...
		<cfelse>
			...
			FROM #request.razuna.session.hostdbprefix#collections_groups
...
<cffunction name="sharecheckpermfolder" access="public" output="true">
		...
		SELECT /* #cachetoken#sharecheckpermfolder */ folder_id,
			...
			<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
				CASE
					WHEN EXISTS(
						SELECT fg.folder_id_r
						FROM #request.razuna.session.hostdbprefix#folders_groups fg INNER JOIN ct_groups_users gu ON gu.ct_g_u_grp_id = fg.grp_id_r AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
						WHERE fg.folder_id_r = f.folder_id
						AND fg.grp_permission IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="r,w,x" list="true">)
						) THEN 'unlocked'
					<!--- If this is the user folder or he is the owner --->
	...
	<cfelse>
		...
		SELECT /* #cachetoken#sharecheckpermfolder */ col_id,
			...
			<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
				CASE
					WHEN EXISTS(
						SELECT fg.col_id_r
						FROM #request.razuna.session.hostdbprefix#collections_groups fg INNER JOIN ct_groups_users gu ON gu.ct_g_u_grp_id = fg.grp_id_r AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
						WHERE fg.col_id_r = f.col_id
						AND fg.grp_permission IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="r,w,x" list="true">)
						) THEN 'unlocked'
					<!--- If this is the user folder or he is the owner --->
					WHEN ( f.col_owner = '#request.razuna.session.theUserID#' ) THEN 'unlocked'
					<!--- If nothing meets the above lock the folder --->
					ELSE 'locked'
				END AS perm
			<cfelse>
		...
		FROM #request.razuna.session.hostdbprefix#collections f
...
<cffunction name="batch_sharing" output="true">
	...
	<cfloop list="#arguments.thestruct.file_ids#" index="i">
		...
		<cfelseif thetype EQ "img">
			...
			<cfquery datasource="#request.razuna.application.datasource#" name="qryrel">
...
<cffunction name="getfoldername" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#folders f LEFT JOIN #request.razuna.session.hostdbprefix#folders_name fn ON f.folder_id = fn.folder_id_r
	WHERE f.folder_id = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	AND fn.lang_id_r = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.thelangid#">
	</cfquery>
	<!--- Set folder name --->
...
<cffunction name="getusername" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON f.folder_owner = u.user_id
	WHERE f.folder_id = <cfqueryparam value="#arguments.folder_id#" cfsqltype="CF_SQL_VARCHAR">
	AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Check --->
	...
	<cfloop delimiters="," index="myform" list="#arguments.thestruct.fieldnames#">
		...
		<cfif myform CONTAINS "img_">
			...
			<cfif imgid NEQ theid>
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="here_img">
				SELECT img_id_r
				FROM #request.razuna.session.hostdbprefix#images_text
				...
				<cfif here_img.recordcount NEQ 0>
					<cfquery datasource="#request.razuna.application.datasource#">
					UPDATE #request.razuna.session.hostdbprefix#images_text
					SET
		...
		<cfelseif myform CONTAINS "vid_">
			...
			<cfif vidid NEQ theid>
				...
				<!--- And the keywords & desc --->
				<cfquery datasource="#request.razuna.application.datasource#" name="here_vid">
				SELECT vid_id_r
				FROM #request.razuna.session.hostdbprefix#videos_text
		...
		<cfelseif myform CONTAINS "aud_">
			...
			<cfif audid NEQ theid>
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="here_aud">
				SELECT aud_id_r
				...
				<cfif here_aud.recordcount NEQ 0>
					<!--- And the keywords & desc --->
					<cfquery datasource="#request.razuna.application.datasource#">
					UPDATE #request.razuna.session.hostdbprefix#audios_text
					SET
		...
		<cfelseif myform CONTAINS "doc_">
			...
			<cfif docid NEQ theid>
				...
				<!--- And the keywords & desc --->
				<cfquery datasource="#request.razuna.application.datasource#" name="here_doc">
				SELECT file_id_r
				FROM #request.razuna.session.hostdbprefix#files_desc
...
<cffunction name="link_check" output="false">
	<cfargument name="thestruct" type="struct" required="true">
		...
		<cfif (request.razuna.application.isp AND ListContains(arguments.thestruct.link_path, 'home', '/\') AND directoryexists("#arguments.thestruct.link_path#")) OR (!request.razuna.application.isp AND directoryexists("#arguments.thestruct.link_path#"))>
			<!--- Set to true --->
...
<cffunction name="getsubfolders" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getsubfolders */ f.folder_id, f.folder_id_r, f.folder_of_user, f.folder_owner, f.folder_level, <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2">NVL<cfelseif request.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif request.razuna.application.thedatabase EQ "mssql">isnull</cfif>(u.user_login_name,'Obsolete') as username,
	<!--- Permission follow but not for sysadmin and admin --->
	<cfif ( not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator ) AND NOT structkeyexists(arguments,"external")>
		CASE
			<!--- Check permission on this folder --->
			WHEN EXISTS(
				SELECT fg.folder_id_r
	...
	, '0' as filecount,
	if ( (select fn.folder_name as folder_name FROM raz1_folders_name fn WHERE fn.folder_id_r = f.folder_id AND fn.lang_id_r = #request.razuna.session.thelangid#) != '',  (select fn.folder_name as folder_name FROM raz1_folders_name fn WHERE fn.folder_id_r = f.folder_id AND fn.lang_id_r = #request.razuna.session.thelangid#), f.folder_name ) as folder_name
	FROM #request.razuna.session.hostdbprefix#folders f
	LEFT JOIN users u ON u.user_id = f.folder_owner
	LEFT JOIN #request.razuna.session.hostdbprefix#folders_name fn ON f.folder_id = fn.folder_id_r
	WHERE
...
<cffunction name="getbreadcrumb" output="false">
	...
	<cfargument name="dsn" type="string" default="#request.razuna.application.datasource#" required="false">
	<cfargument name="prefix" type="string" default="#request.razuna.session.hostdbprefix#" required="false">
	<cfargument name="hostid" type="string" default="#request.razuna.session.hostid#" required="false">
	<cftry>
		<!--- Params --->
		<cfset var qry = "">
		<cfset var qryshared = "">
		...
		<cfif arguments.fromshare>
			...
			WHERE folder_id = <cfqueryparam value="#request.razuna.session.fid#" cfsqltype="CF_SQL_VARCHAR">
			</cfquery>
			<!--- Check the permission settings of a widget --->
			<cfif structKeyExists(request.razuna.session,"widget_id")>
				<cfset s = structnew()>
				<cfset s.widget_id = request.razuna.session.widget_id>
				<cfset s.external = "t">
		...
		<cfif checkperm AND (NOT request.razuna.session.is_system_admin AND NOT request.razuna.session.is_administrator)>
			<cfquery dbtype="query" name="qry">
			SELECT *
			FROM qry
			WHERE perm = <cfqueryparam cfsqltype="cf_sql_varchar" value="unlocked">
...
<cffunction name="download_folder" output="false">
	...
	<cfif arguments.thestruct.prefs.set2_meta_export EQ 't'>
		...
		<cfelse>
			<cfset var suffix = "#request.razuna.session.hostid#-#request.razuna.session.theuserid#">
		</cfif>
...
<cffunction name="download_selected" output="false">
	...
	<cfif arguments.dl_renditions>
		...
		<cfif rend_av EQ 'f'>
			...
			<cfquery name="arguments.dl_query" datasource="#request.razuna.application.datasource#">
		...
		<cfelseif rend_av EQ 't'>
			...
			<cfquery name="arguments.dl_query" datasource="#request.razuna.application.datasource#">
				...
				WHERE v.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				...
				WHERE a.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				...
				WHERE f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	...
	<cfloop query="arguments.dl_query">
		...
		<cfif theorgname NEQ "">
			...
			<!--- Local --->
			<cfif request.razuna.application.storage EQ "local" AND link_kind EQ "">
				<cftry>
				<cffile action="copy" source="#arguments.assetpath#/#request.razuna.session.hostid#/#path_to_asset#/#theorgname#" destination="#arguments.dl_folder#/#thefinalname#" mode="775">
				<cfcatch type="any">
						<cfset console("#now()# ---------------- Error")>
						<cfset consoleoutput(true, true)>
						<cfset console(cfcatch)>
			...
			<!--- Nirvanix --->
			<cfelseif request.razuna.application.storage EQ "nirvanix" AND link_kind EQ "">
				<cftry>
			...
			<!--- Akamai --->
			<cfelseif request.razuna.application.storage EQ "akamai" AND link_kind EQ "">
			...
			<!--- Amazon --->
			<cfelseif request.razuna.application.storage EQ "amazon" AND link_kind EQ "">
					<cfif rend_av EQ 't'>
...
<cffunction name="store_values" output="false" returntype="void">
	...
	<cfif arguments.thestruct.folder_id neq 1>
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR arguments.thestruct.thekind EQ "img">
			SELECT /* #cachetoken#sv */ <cfif request.razuna.application.thedatabase EQ "mssql">img_id + '-img'<cfelse>concat(img_id,'-img')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#images
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND (img_group IS NULL OR img_group = '')
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			...
			UNION
			SELECT /* #cachetoken#sv */ <cfif request.razuna.application.thedatabase EQ "mssql">img_id + '-img'<cfelse>concat(img_id,'-img')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#images i, ct_aliases ct
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR arguments.thestruct.thekind EQ "vid">
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">vid_id + '-vid'<cfelse>concat(vid_id,'-vid')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#videos
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND (vid_group IS NULL OR vid_group = '')
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			...
			UNION
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">vid_id + '-vid'<cfelse>concat(vid_id,'-vid')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#videos v, ct_aliases ct
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR arguments.thestruct.thekind EQ "aud">
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">aud_id + '-aud'<cfelse>concat(aud_id,'-aud')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#audios
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND (aud_group IS NULL OR aud_group = '')
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			...
			UNION
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">aud_id + '-aud'<cfelse>concat(aud_id,'-aud')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#audios a, ct_aliases ct
		...
		<cfif arguments.thestruct.thekind EQ "ALL" OR (arguments.thestruct.thekind NEQ "vid" AND arguments.thestruct.thekind NEQ "img" AND arguments.thestruct.thekind NEQ "aud")>
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">file_id + '-doc'<cfelse>concat(file_id,'-doc')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#files
			WHERE folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
			AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
			AND in_trash = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="F">
			...
			UNION
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">file_id + '-doc'<cfelse>concat(file_id,'-doc')</cfif> as id
			FROM #request.razuna.session.hostdbprefix#files f, ct_aliases ct
	...
	<cfelse>
		...
		<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
			SELECT <cfif request.razuna.application.thedatabase EQ "mssql">file_id_r + '-' + col_file_type<cfelse>concat(file_id_r,'-',col_file_type)</cfif> as id
			FROM #request.razuna.session.hostdbprefix#collections_ct_files
	...
	<!--- Set the sessions --->
	<cfset request.razuna.session.file_id = l>
	<cfset request.razuna.session.thefileid = l>
	<cfset request.razuna.session.editids = l>
</cffunction>
...
<cffunction name="store_selection" output="false" returntype="void">
	<cfargument name="thestruct" required="yes" type="struct">
	<!--- session --->
	<cfparam name="request.razuna.session.file_id" default="">
	...
	<cfif isdefined("arguments.thestruct.individual_select") AND arguments.thestruct.individual_select EQ 'true'>
		<cfset request.razuna.session.individual_select = true>
	</cfif>
	<!--- Now simply add the selected fileids to the session --->
	<cfset request.razuna.session.file_id = "">
	<cfset request.razuna.session.file_id = listappend(request.razuna.session.file_id,"#arguments.thestruct.file_id#")>
	<cfset request.razuna.session.thefileid = request.razuna.session.file_id>
	<cfif request.razuna.session.file_id NEQ "">
		<cfset list_file_ids = "">
		<cfloop index="idx" from="1" to="#listlen(request.razuna.session.file_id)#">
			<!--- <cfif !listFindNoCase(#arguments.thestruct.del_file_id#,#listGetAt(request.razuna.session.file_id,idx)#)> --->
				<cfset list_file_ids = listAppend(list_file_ids,#listGetAt(request.razuna.session.file_id,idx)#,',')>
			<!--- </cfif> --->
		</cfloop>
		<cfset request.razuna.session.thefileid = list_file_ids>
		<cfset request.razuna.session.file_id = list_file_ids>
		<cfset request.razuna.session.editids = list_file_ids>
	</cfif>
</cffunction>
...
<cffunction name="trashcount" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="asset_count" cachedwithin="#CreateTimeSpan(0,0,5,0)#" region="razcache">
	SELECT /* #cachetoken#trashcount */ COUNT(img_id) AS cnt FROM #request.razuna.session.hostdbprefix#images i
	...
	UNION ALL
	SELECT COUNT(aud_id) AS cnt  FROM #request.razuna.session.hostdbprefix#audios a
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	AND CASE
			<cfif request.razuna.session.is_system_admin OR request.razuna.session.is_administrator>
	...
	UNION ALL
	SELECT COUNT(vid_id) AS cnt FROM #request.razuna.session.hostdbprefix#videos v
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	AND CASE
			<cfif request.razuna.session.is_system_admin OR request.razuna.session.is_administrator>
	...
	UNION ALL
	SELECT COUNT(file_id) AS cnt FROM #request.razuna.session.hostdbprefix#files f
	WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
	AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	AND CASE
			<cfif request.razuna.session.is_system_admin OR request.razuna.session.is_administrator>
...
<cffunction name="folderTrashCount" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="folder_count" cachedwithin="#CreateTimeSpan(0,0,5,0)#" region="razcache">
...
<cffunction name="getdetailnextback" output="false" returntype="struct">
	...
	<cfif structKeyExists(arguments.thestruct,"row")>
		...
		<cfelseif arguments.thestruct.what EQ "audios">
			<cfset var thedb = "#request.razuna.session.hostdbprefix#audios">
		...
		<cfelseif arguments.thestruct.what EQ "files">
			<cfset var thedb = "#request.razuna.session.hostdbprefix#files">
		...
		<cfif request.razuna.session.sortby EQ "name">
			<cfset var sortby = "filename_forsort">
		<cfelseif request.razuna.session.sortby EQ "kind">
		...
		<cfelseif request.razuna.session.sortby EQ "datechanged">
			<cfset var sortby = "date_change DESC">
		<cfelseif request.razuna.session.sortby EQ "hashtag">
...
<cffunction name="getallassetsinfolder" access="remote" output="false" returntype="Query">
	...
	<cfset var cachetoken = getcachetoken(type="folders", hostid=request.razuna.session.hostid)>
	<!--- Query --->
	<cfquery name="qry_folder" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
...
<cffunction  name="copy" output="true">
	...
	<!---<cftry>--->
		...
		<cfif request.razuna.application.storage EQ "local">
			<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#">
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/aud')>
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img')>
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/vid')>
			<cfset directoryCreate('#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/doc')>
		</cfif>

		<!--- Insert into folders table --->
		<cfquery datasource="#request.razuna.application.datasource#" name="insert_folder">
			INSERT INTO #request.razuna.session.hostdbprefix#folders
		...
		<cfloop query="assets" >
			...
			<cfif kind EQ 'img'>
				...
				<cfquery name="select_images" datasource="#request.razuna.application.datasource#">
					SELECT * FROM #request.razuna.session.hostdbprefix#images
				...
				<cfif request.razuna.application.storage EQ "local">
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img/#newimgid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img/#id#" newdirectory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img/#newimgid#" >
					--->
					<cffile action="rename" source="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img/#newimgid#/thumb_#id#.#select_images.thumb_extension#" destination="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/img/#newimgid#/thumb_#newimgid#.#select_images.thumb_extension#" >
				</cfif>
				<cfquery name="select_images_text" datasource="#request.razuna.application.datasource#">
					SELECT * FROM #request.razuna.session.hostdbprefix#images_text
					WHERE img_id_r = <cfqueryparam value="#id#" cfsqltype="cf_sql_varchar" >
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
				</cfquery>
				<cfquery name="select_xmp" datasource="#request.razuna.application.datasource#">
					SELECT * FROM #request.razuna.session.hostdbprefix#xmp
				...
				<cfset link_path = '#arguments.thestruct.path#raz#request.razuna.session.hostid#/dam/incoming/api#newimgid#'>
				<cfquery name="updateimages" datasource="#request.razuna.application.datasource#">
					...
					<cfqueryparam value="#request.razuna.session.HOSTID#" cfsqltype="cf_sql_integer" >,
				...
				<cfquery name="updateimges_text" datasource="#request.razuna.application.datasource#">
					INSERT INTO #request.razuna.session.hostdbprefix#images_text
			...
			<cfelseif kind EQ 'doc' OR kind EQ 'other'>
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="select_files">
					SELECT * FROM #request.razuna.session.hostdbprefix#files
					WHERE file_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#id#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
					AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				</cfquery>
				...
				<cfif request.razuna.application.storage EQ "local">
					...
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/doc/#newfileid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/doc/#id#" newdirectory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/doc/#newfileid#" >
					--->
				</cfif>
				<cfquery datasource="#request.razuna.application.datasource#" name="select_files_desc">
					SELECT * FROM #request.razuna.session.hostdbprefix#files_desc
				...
				<!--- Insert into tables --->
				<cfset link_path = '#arguments.thestruct.path#raz#request.razuna.session.hostid#/dam/incoming/api#newfileid#'>
				<cfquery datasource="#request.razuna.application.datasource#" name="update_files" >
				...
				<cfif select_files_desc.recordcount>
					<cfquery datasource="#request.razuna.application.datasource#" name="update_files_desc" >
						INSERT INTO #request.razuna.session.hostdbprefix#files_desc
			...
			<cfelseif kind EQ 'aud'>
				...
				<!--- Get audio records --->
				<cfquery datasource="#request.razuna.application.datasource#" name="select_audios">
					SELECT * FROM #request.razuna.session.hostdbprefix#audios
					WHERE aud_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#id#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
					AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				</cfquery>
				...
				<cfif request.razuna.application.storage EQ "local">
					...
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/aud/#newaudid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/aud/#id#" newdirectory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/aud/#newaudid#" >
						--->
				</cfif>
				<cfquery datasource="#request.razuna.application.datasource#" name="select_audios_text">
					SELECT * FROM #request.razuna.session.hostdbprefix#audios_text
				...
				<cfset link_path = '#arguments.thestruct.path#raz#request.razuna.session.hostid#/dam/incoming/api#newaudid#'>
				<cfquery datasource="#request.razuna.application.datasource#" name="update_audios">
					...
					VALUES(
						...
						<cfqueryparam cfsqltype="cf_sql_integer" value="#request.razuna.session.HOSTID#">,
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="update_audios_text">
					INSERT INTO #request.razuna.session.hostdbprefix#audios_text
			...
			<cfelseif kind EQ 'vid'>
				...
				<!--- Get video records --->
				<cfquery datasource="#request.razuna.application.datasource#" name="select_videos">
					SELECT * FROM #request.razuna.session.hostdbprefix#videos
					WHERE vid_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#id#">
					AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
					AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				</cfquery>
				...
				<cfif request.razuna.application.storage EQ "local">
					...
					<cfinvoke component="global" method="directoryCopy">
						<cfinvokeargument name="source" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#assets.path_to_asset#">
						<cfinvokeargument name="destination" value="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/vid/#newvidid#">
						<cfinvokeargument name="directoryrecursive" value="true">
					</cfinvoke>
					<!---
					<cfdirectory action="rename" directory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/vid/#id#" newdirectory="#arguments.thestruct.assetpath#/#request.razuna.session.hostid#/#newfolderid#/vid/#newvidid#" >
					--->
				</cfif>
				<cfquery datasource="#request.razuna.application.datasource#" name="select_videos_text">
					SELECT * FROM #request.razuna.session.hostdbprefix#videos_text
				...
				<cfset link_path = '#arguments.thestruct.path#raz#request.razuna.session.hostid#/dam/incoming/api#newvidid#'>
				<!--- Insert video details --->
				<cfquery datasource="#request.razuna.application.datasource#" name="update_videos">
				...
				<cfif select_videos_text.RecordCount>
					<cfquery datasource="#request.razuna.application.datasource#" name="update_videos_text">
						INSERT INTO #request.razuna.session.hostdbprefix#videos_text
...
<cffunction name="subscribe" access="public" output="true">
	...
	<!--- Subscribe details --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qfoldersubscribe" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#subscribe */ fs_id
	FROM #request.razuna.session.hostdbprefix#folder_subscribe
	WHERE folder_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.theid#">
	AND user_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#request.razuna.session.theUserID#">
	</cfquery>
	...
	<cfelse>
		<cfif qfoldersubscribe.recordcount NEQ 0>
			...
			<cfquery datasource="#request.razuna.application.datasource#">
			UPDATE #request.razuna.session.hostdbprefix#folder_subscribe
			SET
		...
		<cfelse>
			...
			<cfquery datasource="#request.razuna.application.datasource#">
			INSERT INTO #request.razuna.session.hostdbprefix#folder_subscribe
...
<cffunction name="getsubscribefolder" output="false" access="public" returntype="query">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_folder" cachedwithin="1" region="razcache">
...
<cffunction name="download_folder_structure_flat" output="false">
	...
	<!--- <cfif structKeyExists(arguments.thestruct,'export_template') AND arguments.thestruct.export_template.recordcount NEQ 0> --->
		<!--- <cffile action="move" destination="#arguments.thestruct.newpath#" source="#arguments.thestruct.thepath#/outgoing/metadata-export-#request.razuna.session.hostid#-#request.razuna.session.theuserid#.csv"> --->
	<!--- </cfif> --->
	<!--- Feedback --->
	<cfinvoke component="defaults" method="trans" transid="download_folder_output2" returnvariable="download_folder_output2" />
...
<cffunction name="getchildfolders" access="public" returntype="string" >
    ...
    <cfquery name="objnav" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
    ...
    <cfloop query="objnav">
        ...
        <cfquery name="checkforkids" datasource="#request.razuna.application.datasource#" cachedwithin="1" region="razcache">
...
<cffunction name="checkfolder" output="false" returntype="boolean" >
	...
	<cfquery dataSource="#request.razuna.application.datasource#" name="folder_check">
		SELECT 1 FROM #request.razuna.session.hostdbprefix#folders
		WHERE folder_id = <cfqueryparam value="#arguments.folder_id#" CFSQLType="CF_SQL_VARCHAR">
...
<cffunction name="getFlatFolderList" output="false" returntype="query">
	...
	<cfquery dataSource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getFlatFolderList */ f.folder_of_user, f.folder_id, f.folder_name, '0' as folder_level, f.folder_name as folder_path, <cfif request.razuna.application.thedatabase EQ "h2">NVL<cfelseif request.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif request.razuna.application.thedatabase EQ "mssql">isnull</cfif>(u.user_login_name,'Obsolete') as username
	FROM #request.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON u.user_id = f.folder_owner
	WHERE f.folder_id = f.folder_id_r
	AND f.host_id = <cfqueryparam value="#request.razuna.session.hostid#" CFSQLType="CF_SQL_NUMERIC">
...
<cffunction name="recfoldername" output="false" access="public" returntype="query">
	...
	<cfloop query="arguments.theqry">
		<!--- Query --->
		<cfquery datasource="#request.razuna.application.datasource#" name="local_query" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#recfolder */ f.folder_of_user, f.folder_id, f.folder_name, '#thelevel#' as folder_level, <cfif request.razuna.application.thedatabase EQ "mssql">'#folder_path# / ' + f.folder_name<cfelse>concat('#folder_path# / ',f.folder_name)</cfif> as folder_path, <cfif request.razuna.application.thedatabase EQ "h2">NVL<cfelseif request.razuna.application.thedatabase EQ "mysql">ifnull<cfelseif request.razuna.application.thedatabase EQ "mssql">isnull</cfif>(u.user_login_name,'Obsolete') as username
		FROM #request.razuna.session.hostdbprefix#folders f LEFT JOIN users u ON u.user_id = f.folder_owner
		WHERE f.folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#folder_id#">
		AND f.folder_id <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> f.folder_id_r
		AND f.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND f.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		</cfquery>
