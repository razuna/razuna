...
<cffunction name="get" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#comments c LEFT JOIN users u ON u.user_id = c.user_id_r
...
<cffunction name="getlatest" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry">
	SELECT <cfif request.razuna.application.thedatabase EQ "mssql">TOP 1 </cfif>c.com_id, c.com_text, c.com_date, u.user_login_name, u.user_first_name, u.user_last_name
	FROM #request.razuna.session.hostdbprefix#comments c LEFT JOIN users u ON u.user_id = c.user_id_r
	WHERE c.asset_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.file_id#">
	AND c.asset_type = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.type#">
...
<cffunction name="add" output="false">
	...
	<cfif arguments.thestruct.comment NEQ "">
		...
		VALUES(
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#request.razuna.session.newcommentid#">,
...
<cffunction name="remove" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry">
	DELETE FROM #request.razuna.session.hostdbprefix#comments
...
<cffunction name="update" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry">
	UPDATE #request.razuna.session.hostdbprefix#comments
