...
<cffunction name="log_users" output="false" access="public">
	...
	<cfquery datasource="#request.razuna.application.datasource#" background="yes">
	...
	<!--- Flush Cache --->
	<cfset resetcachetoken(type="logs", hostid=request.razuna.session.hostid)>
	<cfreturn />
</cffunction>
...
<cffunction name="get_log_users" output="false" access="public">
	...
	<cfset var cachetoken = getcachetoken(type="logs", hostid=request.razuna.session.hostid)>
	<!--- Get all log entries --->
	<cfquery datasource="#request.razuna.application.datasource#" name="thetotal" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#get_log_users */ log_id
	FROM #request.razuna.session.hostdbprefix#log_users
	...
	<cfif thetotal.recordcount LTE request.razuna.session.rowmaxpage_log>
		<cfset request.razuna.session.offset_log = 0>
	</cfif>
	<cfif request.razuna.session.offset_log EQ 0>
		<cfset var min = 0>
		<cfset var max = request.razuna.session.rowmaxpage_log>
	<cfelse>
		<cfset var min = request.razuna.session.offset_log * request.razuna.session.rowmaxpage_log>
		<cfset var max = (request.razuna.session.offset_log + 1) * request.razuna.session.rowmaxpage_log>
	</cfif>
	<!--- MySQL Offset --->
	<cfset var mysqloffset = request.razuna.session.offset_log * request.razuna.session.rowmaxpage_log>
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		...
		<cfif request.razuna.application.thedatabase EQ "oracle">
			...
			FROM (
				...
				FROM (
					...
					FROM #request.razuna.session.hostdbprefix#log_users l
					WHERE l.log_section = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.logsection#">
					AND l.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
					<cfif structkeyexists(arguments.thestruct,"logaction") AND arguments.thestruct.logaction NEQ "">
		...
		<!--- MSSQL, MySQL, H2 --->
		<cfelse>
			SELECT /* #cachetoken#get_log_users2 */<cfif request.razuna.application.thedatabase EQ "mssql"> TOP #request.razuna.session.rowmaxpage_log#</cfif> l.log_user, l.log_action, l.log_date, l.log_time, l.log_desc, l.log_browser,
			...
			<cfif request.razuna.application.thedatabase EQ "mysql" OR request.razuna.application.thedatabase EQ "h2">
				LIMIT #mysqloffset#, #request.razuna.session.rowmaxpage_log#
			</cfif>
		</cfif>
	</cfquery>
	<!--- Return --->
...
<cffunction name="remove_log_users" output="false" access="public">
	...
	DELETE FROM #request.razuna.session.hostdbprefix#log_users
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Flush Cache --->
...
<cffunction name="get_log_assets" output="false" access="public">
	...
	<cfif structKeyExists(arguments.thestruct,"is_dashboard_update")>
		<cfset temp_rowmaxpage_log = request.razuna.session.rowmaxpage_log>
		<cfset temp_offset_log = request.razuna.session.offset_log>
		<cfset request.razuna.session.rowmaxpage_log = 50>
		<cfset request.razuna.session.offset_log = 0>
	</cfif>
	<!--- Get the cachetoken for here --->
	<cfset var cachetoken = getcachetoken(type="logs", hostid=request.razuna.session.hostid)>
	...
	FROM #request.razuna.session.hostdbprefix#log_assets
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		...
		<cfif request.razuna.application.thedatabase EQ "oracle">
			...
			FROM (
				...
				FROM (
					...
					FROM #request.razuna.session.hostdbprefix#log_assets l LEFT JOIN users u ON l.log_user = u.user_id
		...
		<!--- MSSQL, MySQL, H2 --->
		<cfelse>
			SELECT /* #cachetoken#get_log_assets2 */<cfif request.razuna.application.thedatabase EQ "mssql"> TOP #request.razuna.session.rowmaxpage_log#</cfif> l.log_user, l.log_action, l.log_date,
	...
	<!--- Get Dashboard most recently updated assets without paging records --->
	<cfif structKeyExists(arguments.thestruct,"is_dashboard_update")>
		<cfset request.razuna.session.rowmaxpage_log = temp_rowmaxpage_log>
		<cfset request.razuna.session.offset_log = temp_offset_log>
	</cfif>
...
<cffunction name="remove_log_assets" output="false" access="public">
	...
	<cfquery datasource="#request.razuna.application.datasource#">
	DELETE FROM #request.razuna.session.hostdbprefix#log_assets
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
...
<cffunction name="get_log_folders" output="false" access="public">
	...
	FROM #request.razuna.session.hostdbprefix#log_folders
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		...
		<cfif request.razuna.application.thedatabase EQ "oracle">
			...
			FROM (
				...
				FROM (
					...
					FROM #request.razuna.session.hostdbprefix#log_folders l LEFT JOIN users u ON l.log_user = u.user_id
					WHERE l.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
					<cfif structkeyexists(arguments.thestruct,"logaction") AND arguments.thestruct.logaction NEQ "">
						AND l.log_action = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.logaction#">
					</cfif>
		...
		<!--- MSSQL, MySQL, H2 --->
		<cfelse>
			SELECT /* #cachetoken#get_log_folders2 */<cfif request.razuna.application.thedatabase EQ "mssql"> TOP #request.razuna.session.rowmaxpage_log#</cfif> l.log_user, l.log_action, l.log_date, l.log_time, l.log_desc, l.log_browser,
...
<cffunction name="remove_log_folders" output="false" access="public">
	<cfquery datasource="#request.razuna.application.datasource#">
	DELETE FROM #request.razuna.session.hostdbprefix#log_folders
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Flush Cache --->
	<cfset variables.cachetoken = resetcachetoken(type="logs", hostid=request.razuna.session.hostid)>
...
<cffunction name="get_log_searches" output="false" access="public">
	...
	FROM #request.razuna.session.hostdbprefix#log_search
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Set the session for offset correctly if the total count of assets in lower the the total rowmaxpage --->
	<cfif thetotal.recordcount LTE request.razuna.session.rowmaxpage_log>
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		...
		<cfif request.razuna.application.thedatabase EQ "oracle">
			...
			FROM (
				...
				FROM (
					...
					FROM #request.razuna.session.hostdbprefix#log_search l LEFT JOIN users u ON l.log_user = u.user_id
					WHERE l.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		...
		<!--- MSSQL, MySQL, H2 --->
		<cfelse>
			SELECT /* #cachetoken#get_log_searches2 */<cfif request.razuna.application.thedatabase EQ "mssql"> TOP #request.razuna.session.rowmaxpage_log#</cfif> l.log_user, l.log_date, l.log_time, l.log_search_for, l.log_search_from,
...
<cffunction name="remove_log_searches" output="false" access="public">
	<cfquery datasource="#request.razuna.application.datasource#">
	DELETE FROM #request.razuna.session.hostdbprefix#log_search
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Flush Cache --->
	<cfset variables.cachetoken = resetcachetoken(type="logs", hostid=request.razuna.session.hostid)>
...
<cffunction name="get_log_errors" output="false" access="public">
	...
	FROM #request.razuna.session.hostdbprefix#errors
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Set the session for offset correctly if the total count of assets in lower the the total rowmaxpage --->
	<cfif thetotal.recordcount LTE request.razuna.session.rowmaxpage_log>
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
		...
		<!--- MSSQL, MySQL, H2 --->
		<cfelse>
			SELECT /* #cachetoken#get_log_errors2 */<cfif request.razuna.application.thedatabase EQ "mssql"> TOP #request.razuna.session.rowmaxpage_log#</cfif> id, err_date,
			(
				SELECT count(id)
				FROM #request.razuna.session.hostdbprefix#errors
				WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
...
<cffunction name="remove_log_errors" output="false" access="public">
	<cfquery datasource="#request.razuna.application.datasource#">
	DELETE FROM #request.razuna.session.hostdbprefix#errors
	WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	<!--- Flush Cache --->
	<cfset variables.cachetoken = resetcachetoken(type="logs", hostid=request.razuna.session.hostid)>
...
<cffunction name="log_search" output="false" access="public">
	...
	FROM #request.razuna.session.hostdbprefix##arguments.thestruct.logtype# l LEFT JOIN users u ON l.log_user = u.user_id
...
<cffunction name="log_folder_summary" output="false" access="public">
	...
	(SELECT COUNT(1) FROM #request.razuna.session.hostdbprefix#folders WHERE folder_id_r = f.folder_id AND folder_id_r <> folder_id AND in_trash = 'F') sf_cnt
	...
	AND in_trash = 'F'
	AND EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#folders WHERE folder_id = f.folder_id_r)/*Make sure folder parent exists in system to avoid orphans*/
