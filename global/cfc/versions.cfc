...
<cffunction name="playback" output="false">
	...
	<cftry>
		...
		SELECT <cfif variables.database EQ "oracle" OR variables.database EQ "h2">NVL<cfelseif variables.database EQ "mysql">ifnull<cfelseif variables.database EQ "mssql">isnull</cfif>(max(ver_version),0) + 1 AS newversion
...
<cffunction name="create" output="false">
	...
	<cftry>
		...
		SELECT <cfif arguments.thestruct.database EQ "oracle" OR arguments.thestruct.database EQ "h2">NVL<cfelseif arguments.thestruct.database EQ "mysql">ifnull<cfelseif variables.database EQ "mssql">isnull</cfif>(max(ver_version),0) + 1 AS newversion
		FROM #session.hostdbprefix#versions
