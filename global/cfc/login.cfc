...
<!--- FUNCTION: LOGIN --->
	<cffunction name="login" access="public" output="false" returntype="struct">
		...
		<cfset variables.cachetoken = getcachetoken(type="users", hostid=request.razuna.session.hostid)>
		<cfset var qryuser = "">
		<!--- Check for the user --->
		...
		<cfelse>
			<cfset request.razuna.session.is_system_admin = false>
			<cfset request.razuna.session.is_administrator = false>
			<cfset theuser.notfound = "F">
			<!--- Put the query result in the structure --->
			<cfset theuser.qryuser = qryuser>
			<!--- Set the Login into a session --->
			<cfset request.razuna.session.login = "T">
			<!--- Set the Web Login into a session --->
			<cfset request.razuna.session.weblogin = "F">
			<!--- Set search selection --->
			<cfset request.razuna.session.user_search_selection = qryuser.user_search_selection>
			<!--- Set the user ID into a session --->
			<cfset request.razuna.session.theuserid = qryuser.user_id>
			<cfset request.razuna.session.user_email = qryuser.user_email>
			<!--- Set User First and last name --->
			<cfset request.razuna.session.firstlastname = "#qryuser.user_first_name# #qryuser.user_last_name#">
			<cfif structKeyExists(arguments.thestruct,"ad_user_name") AND qryuser.user_first_name EQ '' AND qryuser.user_last_name EQ ''>
				<cfset request.razuna.session.firstlastname = "#arguments.thestruct.ad_user_name#">
			</cfif>
			<!--- Set user OS --->
			<cfif cgi.http_user_agent contains 'windows'>
				<cfset request.razuna.session.user_os = 'windows'>
			<cfelseif cgi.http_user_agent contains 'mac os'>
				<cfset request.razuna.session.user_os = 'mac'>
			<cfelseif cgi.http_user_agent contains 'linux' OR cgi.http_user_agent contains 'unix'>
				<cfset request.razuna.session.user_os = 'unix'>
			<cfelse>
				<cfset request.razuna.session.user_os = 'unknown'>
			</cfif>
			<!--- Set lib path --->
			...
			<cfloop query="groups_of_user">
				<cfif grp_id EQ "1">
					<cfset request.razuna.session.is_system_admin = true>
				<cfelseif grp_id EQ "2">
					<cfset request.razuna.session.is_administrator = true>
				</cfif>
			</cfloop>
			<!--- Admin Login: Set the domain ID into a session --->
			<cfif arguments.thestruct.loginto EQ "admin">
				<cfset request.razuna.session.hostid = "0">
			...
			<cfif arguments.thestruct.loginto EQ "dam" AND arguments.thestruct.from_share EQ "f">
				...
				<cfif !listFind(request.razuna.session.thegroupofuser, "1", ",")>
		...
		<cfif arguments.thestruct.loginto EQ "admin">
			...
			<cftry>
				<cfset var taskpath =  "#request.razuna.session.thehttp##cgi.http_host##cgi.context_path#/admin">
	...
	<cffunction name="createsysfolder" access="private" returntype="void">
		...
		AND (
			folder_is_collection <cfif request.razuna.application.thedatabase EQ "mysql" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="cf_sql_varchar" value="t">
			OR
	...
	<cffunction name="createmyfolder" access="private">
		<cfargument name="userid" required="yes" type="string">
		<!--- Get the cachetoken for here --->
		<cfset arguments.cachetoken = getcachetoken(type="general", hostid=request.razuna.session.hostid)>
		<cfthread intstruct="#arguments#">
			...
			<cfif myfolder_create>
				<cfquery datasource="#request.razuna.application.datasource#" name="myfolder">
...
<!--- FUNCTION: LOGINSTATUS --->
	<cffunction name="loginstatus" access="public" output="false" returntype="Any">
		...
		<cfif not isSystemAdmin AND not isAdministrator>
			<cfquery datasource="#request.razuna.application.datasource#" name="sameuser">
			SELECT user_id
			FROM users_login
	...
	<cffunction name="sendpassword" displayname="sendpassword"  access="public" output="true" returntype="Any">
		...
		<cfif request.razuna.session.hostid neq 0>
		AND ct.ct_u_h_host_id = <cfqueryparam value="#request.razuna.session.hostid#" cfsqltype="cf_sql_numeric">
		</cfif>
		AND ct.ct_u_h_user_id = u.user_id
		</cfquery>
...
#user_login_password#: #randompassword#
	...
	<cffunction name="razunauploadsession" access="public">
		...
		<!--- Get Host prefix --->
		<cfquery datasource="#request.razuna.application.datasource#" name="pre">
	...
	<cffunction name="checkhost" access="public">
		...
		<!--- Query --->
		<cfquery datasource="#request.razuna.application.datasource#" name="theuser" cachedwithin="1" region="razcache">
		...
		<cfif structkeyexists(request.razuna.session,"hostid")>
			AND h.host_id = <cfqueryparam CFSQLType="CF_SQL_NUMERIC" value="#request.razuna.session.hostid#">
		...
		<cfif theuser.recordcount NEQ 0>
			<!--- Set session hostid --->
			<cfset request.razuna.session.hostid = theuser.host_id>
	...
	<cffunction name="login_janrain" access="public">
		...
		<cfif cfhttp.responseheader.status_code EQ "200">
			...
			<cfif auth_info_json.stat EQ "OK">
				...
				<!--- Now check DB --->
				<cfquery datasource="#request.razuna.application.datasource#" name="qryaccount" cachedwithin="1" region="razcache">
				SELECT /* #variables.cachetoken#login_janrain */ uc.jr_identifier, uc.user_id_r, u.user_first_name, u.user_last_name
				FROM #request.razuna.session.hostdbprefix#users_accounts uc, users u
				WHERE uc.jr_identifier = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#identifier#">
				AND uc.host_id = <cfqueryparam CFSQLType="CF_SQL_NUMERIC" value="#request.razuna.session.hostid#">
				...
				<cfif qryaccount.recordcount EQ 0>
					...
					<cfif qryaccount.recordcount NEQ 0>
						<cfquery datasource="#request.razuna.application.datasource#">
						UPDATE #request.razuna.session.hostdbprefix#users_accounts
						...
						<!--- Flush Cache --->
						<cfset variables.cachetoken = resetcachetoken(type="users", hostid=request.razuna.session.hostid)>
						<!--- and let him in --->
						<cfset razgo = true>
					</cfif>
				...
				<cfif razgo>
					...
					<cfset request.razuna.session.weblogin = "F">
					<!--- Set the user ID into a session --->
					<cfset request.razuna.session.theuserid = qryaccount.user_id_r>
					<!--- Set User First and last name --->
					<cfset request.razuna.session.firstlastname = "#qryaccount.user_first_name# #qryaccount.user_last_name#">
