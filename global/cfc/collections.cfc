...
<cffunction name="init" returntype="collections" access="public" output="false">
	<cfreturn this />
</cffunction>
...
<cffunction name="getAll" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qrylist" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getAllcol */ c.col_id, c.change_date, ct.col_name, c.col_released,
	...
	<cfif not arguments.thestruct.razuna.session.is_system_admin and not arguments.thestruct.razuna.session.is_administrator>
		CASE
			WHEN EXISTS(
				SELECT fg.col_id_r
				FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups fg LEFT JOIN ct_groups_users gu ON gu.ct_g_u_grp_id = fg.grp_id_r AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">
			...
			WHEN ( c.col_owner = '#arguments.thestruct.razuna.session.theUserID#' ) THEN 'unlocked'
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections c
	...
	<cfif qry.collist.recordcount NEQ 0>
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry.collistdesc" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getAlldesccol */ col_id_r, col_desc, lang_id_r
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_text
...
<cffunction name="content_collection" output="false" returntype="query">
	...
	SELECT /* #cachetoken#content_collection */ file_id_r, col_file_type, col_item_order, col_file_format
...
<cffunction name="add" output="false">
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections c, #arguments.thestruct.razuna.session.hostdbprefix#collections_text ct
	...
	<cfif here.recordcount EQ 0>
		...
		INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#collections
		...
		<cfloop list="#arguments.thestruct.langcount#" index="langindex">
			...
			<cfif thisdesc CONTAINS "#langindex#">
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
					insert into #arguments.thestruct.razuna.session.hostdbprefix#collections_text
...
<cffunction name="add_assets" output="false">
	...
	<cfloop query="arguments.thestruct.qry_basket">
		...
		<cfif here.recordcount EQ 0>
			...
			<!--- Insert --->
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
			INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files
		...
		<cfelse> <!--- If file already in collection the set in_trash to false if set to true --->
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="theorder">
			UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files
...
<cffunction name="add_assets_loop" output="false">
	...
	<cfif arguments.thestruct.razuna.session.file_id EQ "all">
		...
		<cfinvoke component="search" method="getAllIdsMain" searchupc="#arguments.thestruct.razuna.session.search.searchupc#" searchtext="#arguments.thestruct.razuna.session.search.searchtext#" searchtype="#arguments.thestruct.razuna.session.search.searchtype#" searchrenditions="#arguments.thestruct.razuna.session.search.searchrenditions#" searchfolderid="#arguments.thestruct.razuna.session.search.searchfolderid#" hostid="#arguments.thestruct.razuna.session.hostid#" thestruct="#arguments.thestruct#" returnvariable="ids">
			<!--- Set the fileid --->
			<cfset arguments.thestruct.razuna.session.file_id = ids>
	</cfif>
	<!--- Loop over the files ids --->
	<cfloop list="#arguments.thestruct.razuna.session.file_id#" delimiters="," index="fileid">
...
<cffunction name="details" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#detailscol */ ct.col_name, ct.col_desc, ct.col_keywords, ct.lang_id_r, c.col_shared, c.col_name_shared, c.share_dl_org, 
	...
	<cfelse>
		...
		CASE
			WHEN (
				...
				FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups fg5
			...
			WHEN (c.col_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theUserID#">) THEN 'X'
		END as colaccess
	</cfif>
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_text ct, #arguments.thestruct.razuna.session.hostdbprefix#collections c
...
<cffunction name="get_assets" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#get_assetscol */ ct.col_id_r, ct.file_id_r as cart_product_id, ct.col_file_type, ct.col_item_order, ct.col_file_format,
		CASE 
		...
		END as theextension
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files ct 
	LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#images i ON ct.file_id_r = i.img_id 
	LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#audios a ON ct.file_id_r = a.aud_id AND a.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="f">
	LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#videos v ON ct.file_id_r = v.vid_id AND v.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="f">
	LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#files f ON ct.file_id_r = f.file_id AND f.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="f">
	...
	AND NOT EXISTS (SELECT 1 FROM #arguments.thestruct.razuna.session.hostdbprefix#images WHERE img_id = i.img_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
	AND NOT EXISTS (SELECT 1 FROM #arguments.thestruct.razuna.session.hostdbprefix#audios WHERE aud_id = a.aud_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
	AND NOT EXISTS (SELECT 1 FROM #arguments.thestruct.razuna.session.hostdbprefix#videos WHERE vid_id = v.vid_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
	AND NOT EXISTS (SELECT 1 FROM #arguments.thestruct.razuna.session.hostdbprefix#files WHERE file_id = f.file_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
...
<cffunction name="col_asset_move_trash" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="move_trash">
	UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files 
...
<cffunction name="col_move_trash" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="move_trash">
	UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections 
	SET in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="get_trash_files" output="false">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #arguments.thestruct.razuna.session.hostdbprefix#images i ON c.file_id_r = i.img_id
		LEFT JOIN  #arguments.thestruct.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #arguments.thestruct.razuna.session.hostdbprefix#audios a  ON c.file_id_r = a.aud_id
		LEFT JOIN  #arguments.thestruct.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #arguments.thestruct.razuna.session.hostdbprefix#videos v ON c.file_id_r = v.vid_id
		LEFT JOIN  #arguments.thestruct.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #arguments.thestruct.razuna.session.hostdbprefix#files f  ON c.file_id_r = f.file_id
		LEFT JOIN  #arguments.thestruct.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="get_trash_collection" output="false">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections AS c
		INNER JOIN #arguments.thestruct.razuna.session.hostdbprefix#collections_text AS col_text ON col_text.col_id_r = c.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
...
<cffunction name="restore_col_asset" output="false">
	...
	SELECT col_id,folder_id_r AS folder_id FROM #arguments.thestruct.razuna.session.hostdbprefix#collections 
	...
	<cfelse>
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thefolderdetail">
		SELECT folder_id,folder_id_r,folder_main_id_r FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
		...
		<cfloop query="dir_parent_id">
			...
			<cfelseif get_qry.folder_id EQ dir_parent_id.folder_id_r AND get_qry.in_trash EQ 'F'>
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="update_trash">
				UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files 
...
<cffunction name="restore_collection" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thedetail">
	SELECT folder_id_r,folder_main_id_r FROM #arguments.thestruct.razuna.session.hostdbprefix#folders
	...
	<cfelse>
		...
		<cfloop query="dir_parent_id">
			...
			<cfelseif get_qry.folder_id EQ dir_parent_id.folder_id_r AND get_qry.in_trash EQ 'F'>
				<cfset local.root = "yes">
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_update">
				UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections 
				SET in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				WHERE col_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.col_id#">
				AND folder_id_r = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.folder_id#">
	...
	<!--- Flush Cache --->
	<cfset resetcachetoken(type="general", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfset resetcachetoken(type="folder", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfset resetcachetoken(type="labels", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfif isDefined('local.istrash') AND local.istrash EQ "trash">
		<cfset var is_trash = "intrash">
	<cfelse>
...
<cffunction name="restoreasset" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_restore">
	UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files 
	SET 
...
<cffunction name="get_col_count" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_count">
		SELECT COUNT(col_id) AS cnt
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections col
...
<cffunction name="getCollectionFolderCount" output="false">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_count">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#folders f WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="getCollectionFileCount" output="false">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_count">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files c, #arguments.thestruct.razuna.session.hostdbprefix#collections col
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
...
<cffunction name="move" output="false">
	...
	<cftry>
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="oneup">
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="current">
		SELECT file_id_r, col_item_order
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files
		WHERE col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
...
<cffunction name="removeitem" output="false">
	...
	AND col_item_order <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> 1
	AND col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
	</cfquery>
	<!--- Flush Cache --->
	<cfset resetcachetoken(type="general", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
...
<cffunction name="remove" output="false">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
	DELETE FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_text
	WHERE col_id_r = <cfqueryparam value="#arguments.thestruct.id#" cfsqltype="CF_SQL_VARCHAR">
	</cfquery>
	<!--- delete collection db --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
	DELETE FROM #arguments.thestruct.razuna.session.hostdbprefix#collections
...
<cffunction name="update" output="false">
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections c, #arguments.thestruct.razuna.session.hostdbprefix#collections_text ct
	WHERE c.folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
	AND c.col_id <cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle" OR arguments.thestruct.razuna.application.thedatabase EQ "h2" OR arguments.thestruct.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.col_id#">
	AND c.col_id = ct.col_id_r
	AND ct.col_name = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.collectionname#">
	AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	</cfquery>
	...
	<cfif here.recordcount EQ 0>
		...
		UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections
		SET
		col_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.theuserid#">,
		...
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
		DELETE FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups
		WHERE col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
		</cfquery>
		...
		<cfloop collection="#arguments.thestruct#" item="myform">
			<cfif #myform# CONTAINS "grp_">
				...
				<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
				INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#collections_groups
...
<cffunction name="getcollectiongroups" output="false">
	...
	<cfif arguments.qrygroup.recordcount NEQ 0>
		...
		SELECT /* #cachetoken#getcollectiongroups */ grp_id_r, grp_permission
...
<cffunction name="getcollectiongroupszero" output="false">
	...
	<cfset var cachetoken = getcachetoken(type="general", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="thegroups" cachedwithin="1" region="razcache">
	SELECT /* #cachetoken#getcollectiongroupszero */ grp_id_r, grp_permission
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups
	WHERE col_id_r = <cfqueryparam value="#arguments.col_id#" cfsqltype="CF_SQL_VARCHAR">
...
<cffunction name="getallassets" output="false">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry.qry_files" cachedwithin="1" region="razcache">
	<!--- For pagination  --->
	<cfif NOT structKeyExists(arguments.thestruct,"searchtext")>
		<cfif arguments.thestruct.razuna.application.datasource EQ "mssql">
			SELECT * FROM (
			SELECT ROW_NUMBER() OVER ( ORDER BY theorder) AS RowNum,sorted_inline_view.* FROM (
		</cfif>
	</cfif>
	SELECT DISTINCT /* #cachetoken#getallassetscol*/ i.img_id id, i.img_filename filename, i.folder_id_r, i.thumb_extension ext, i.img_filename_org filename_org, i.is_available,
	...
	'0' as vheight, '0' as vwidth, i.hashtag,
		(
			...
			AND ct.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
			AND ct.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
		) AS theorder,
		(
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files ct, #arguments.thestruct.razuna.session.hostdbprefix#images i LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#images_text it ON i.img_id = it.img_id_r AND it.lang_id_r = 1  AND i.host_id = it.host_id
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files ct, #arguments.thestruct.razuna.session.hostdbprefix#videos v LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#videos_text vt ON v.vid_id = vt.vid_id_r AND vt.lang_id_r = 1 AND v.host_id = vt.host_id
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files ct, #arguments.thestruct.razuna.session.hostdbprefix#audios a LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#audios_text aut ON a.aud_id = aut.aud_id_r AND aut.lang_id_r = 1 AND a.host_id = aut.host_id
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files ct, #arguments.thestruct.razuna.session.hostdbprefix#files f LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#files_desc ft ON f.file_id = ft.file_id_r AND ft.lang_id_r = 1 AND f.host_id = ft.host_id
	...
	<cfif NOT structKeyExists(arguments.thestruct,"searchtext")>
		...
		<!--- MYSQL --->
		<cfif arguments.thestruct.razuna.application.datasource EQ "mysql">
			ORDER BY theorder LIMIT #mysqloffset#,#arguments.thestruct.razuna.session.rowmaxpage#
		</cfif>
	</cfif>
	</cfquery>
	<!--- Get the total --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry.qry_filecount">
		SELECT count(ct.col_id_r) AS thetotal 
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files ct
		LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#images i on ct.file_id_r = i.img_id
		LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#audios a on ct.file_id_r = a.aud_id
		LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#videos v on ct.file_id_r = v.vid_id
		LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#files f on ct.file_id_r = f.file_id
		WHERE ct.col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
		AND ct.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		<cfif arguments.thestruct.colaccess EQ 'R'>
		AND (i.expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR i.expiry_date is null)
...
<cffunction name="dorelease" output="false" returntype="void">
	...
	<cfif trim(arguments.thestruct.col_name) NEQ "">
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
		UPDATE #arguments.thestruct.razuna.session.hostdbprefix#collections_text
...
<cffunction name="docopy" output="false" returntype="void">
	...
	<cfif arguments.thestruct.copycol>
		...
		INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files
		(col_id_r, file_id_r, col_file_type, col_item_order, col_file_format, host_id, rec_uuid)
		SELECT <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, file_id_r, col_file_type, col_item_order, col_file_format, host_id, <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">newid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">uuid()</cfif>
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files
		WHERE col_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.col_id#">
		</cfquery>
		<!--- Groups --->
		...
		INSERT INTO #arguments.thestruct.razuna.session.hostdbprefix#collections_groups
		(col_id_r, grp_id_r, grp_permission, host_id, rec_uuid)
		SELECT <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, grp_id_r, grp_permission, host_id, <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">newid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">uuid()</cfif>
		FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_groups
		WHERE col_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.col_id#">
		</cfquery>
		<!--- Comments --->
		...
		SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">newid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">uuid()</cfif>, <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, asset_type, user_id_r, com_text, com_date, host_id
		...
		SELECT <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">newid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif arguments.thestruct.razuna.application.thedatabase EQ "mysql">uuid()</cfif>, <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, folder_id_r, widget_name, widget_description, widget_permission, widget_password, widget_style, widget_dl_org, widget_uploading, host_id
...
<cffunction name="trash_remove_all" output="false">
	...
	<cfif arguments.thestruct.trashkind EQ "files">
		<!--- Get all trash files--->
		<cfinvoke method="get_trash_files" noread="true" thestruct="#arguments.thestruct#" returnvariable="arguments.qry" />
		<!--- Thread --->
	...
	<cfelseif arguments.thestruct.trashkind EQ "collections">
		...
		<cfinvoke method="get_trash_collection" noread="true" thestruct="#arguments.thestruct#" returnvariable="arguments.qry" />
		<!--- Thread --->
		<cfthread instruct="#arguments#">
			<cfloop query="attributes.instruct.qry">
				<cfset attributes.instruct.thestruct.id = id>
	...
	<cfelse>
		...
		<cfinvoke method="get_trash_folders" noread="true" thestruct="#arguments.thestruct#" returnvariable="arguments.qry" />
		<!--- Thread --->
		<cfthread instruct="#arguments#">
			<cfloop query="attributes.instruct.qry">
...
<cffunction name="restoreallcollectionfiles" output="false">
	...
	<cfinvoke method="get_trash_files" noread="true" thestruct="#arguments.thestruct#" returnvariable="qry_trash" />
	<cfloop list="#valueList(qry_trash.file_id)#" index="i">
		<!--- set session file ids --->
		<cfset arguments.thestruct.razuna.session.file_id = listAppend(arguments.thestruct.razuna.session.file_id,i)>
	</cfloop>
	<!--- Return --->
	<cfreturn />
</cffunction>
...
<cffunction name="restoreallcollections" output="false">
	...
	<cfset arguments.thestruct.razuna.session.file_id ="">
	<!--- Get all trash collections--->
	<cfinvoke method="get_trash_collection" noread="true" thestruct="#arguments.thestruct#" returnvariable="qry" />
...
<cffunction name="trash_file_values" output="false">
	...
	<cfloop query="qry_trash">
		<!--- set session file ids --->
		<cfset arguments.thestruct.razuna.session.file_id = listAppend(arguments.thestruct.razuna.session.file_id,"#file_id#-#kind#")>
	</cfloop>
	<!--- Return --->
	<cfreturn />	
</cffunction>
...
<cffunction name="trash_col_values" output="false">
	...
	<cfloop list="#valueList(qry.col_id)#" index="i">
		<!--- set session col ids --->
		<cfset arguments.thestruct.razuna.session.file_id = listAppend(arguments.thestruct.razuna.session.file_id,"#i#-collection")>
	</cfloop>
	<!--- Return --->
	<cfreturn />	
</cffunction>
...
<cffunction name="trash_folder_values" output="false">
	...
	<cfset arguments.thestruct.razuna.session.file_id ="">
	<!--- Get all trash folders--->
	<cfinvoke method="get_trash_folders" noread="true" thestruct="#arguments.thestruct#" returnvariable="qry" />
	<cfloop list="#valueList(qry.folder_id)#" index="i">
		<!--- set session col ids --->
		<cfset arguments.thestruct.razuna.session.file_id = listAppend(arguments.thestruct.razuna.session.file_id,"#i#-folder")>
	</cfloop>
	<!--- Return --->
	<cfreturn />	
</cffunction>
...
<cffunction name="samecollectionnamecheck" output="false">
	...
	AND ct.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	AND ct.col_id_r <cfif arguments.thestruct.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
	AND c.col_id = ct.col_id_r
...
<cffunction name="getUsage" output="false">
	...
	FROM #arguments.thestruct.razuna.session.hostdbprefix#collections_ct_files f, #arguments.thestruct.razuna.session.hostdbprefix#collections_text t, #arguments.thestruct.razuna.session.hostdbprefix#collections c
