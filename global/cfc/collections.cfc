...
<cffunction name="getAll" output="false">
	...
	<!--- Query --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qrylist" cachedwithin="1" region="razcache">
	...
	<cfif not request.razuna.session.is_system_admin and not request.razuna.session.is_administrator>
		CASE
			WHEN EXISTS(
				SELECT fg.col_id_r
				FROM #request.razuna.session.hostdbprefix#collections_groups fg LEFT JOIN ct_groups_users gu ON gu.ct_g_u_grp_id = fg.grp_id_r AND gu.ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">
			...
			WHEN ( c.col_owner = '#request.razuna.session.theUserID#' ) THEN 'unlocked'
	...
	FROM #request.razuna.session.hostdbprefix#collections c
	...
	<cfif qry.collist.recordcount NEQ 0>
		<cfquery datasource="#request.razuna.application.datasource#" name="qry.collistdesc" cachedwithin="1" region="razcache">
		SELECT /* #cachetoken#getAlldesccol */ col_id_r, col_desc, lang_id_r
		FROM #request.razuna.session.hostdbprefix#collections_text
...
<cffunction name="add" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="here">
	SELECT c.col_id, ct.col_name
	FROM #request.razuna.session.hostdbprefix#collections c, #request.razuna.session.hostdbprefix#collections_text ct
	...
	AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	...
	<cfif here.recordcount EQ 0>
		...
		INSERT INTO #request.razuna.session.hostdbprefix#collections
		...
		<cfloop list="#arguments.thestruct.langcount#" index="langindex">
			...
			<cfif thisdesc CONTAINS "#langindex#">
				<cfquery datasource="#request.razuna.application.datasource#">
					insert into #request.razuna.session.hostdbprefix#collections_text
...
<cffunction name="add_assets" output="false">
	...
	<cfloop query="arguments.thestruct.qry_basket">
		...
		<cfif here.recordcount EQ 0>
			<!--- Get the order --->
			<cfquery datasource="#request.razuna.application.datasource#" name="theorder">
			...
			<!--- Insert --->
			<cfquery datasource="#request.razuna.application.datasource#">
			INSERT INTO #request.razuna.session.hostdbprefix#collections_ct_files
		...
		<cfelse> <!--- If file already in collection the set in_trash to false if set to true --->
			<cfquery datasource="#request.razuna.application.datasource#" name="theorder">
			UPDATE #request.razuna.session.hostdbprefix#collections_ct_files
...
<cffunction name="add_assets_loop" output="false">
	...
	<cfif request.razuna.session.file_id EQ "all">
		...
		<cfinvoke component="search" method="getAllIdsMain" searchupc="#request.razuna.session.search.searchupc#" searchtext="#request.razuna.session.search.searchtext#" searchtype="#request.razuna.session.search.searchtype#" searchrenditions="#request.razuna.session.search.searchrenditions#" searchfolderid="#request.razuna.session.search.searchfolderid#" hostid="#request.razuna.session.hostid#" returnvariable="ids">
			<!--- Set the fileid --->
			<cfset request.razuna.session.file_id = ids>
	</cfif>
	<!--- Loop over the files ids --->
	<cfloop list="#request.razuna.session.file_id#" delimiters="," index="fileid">
	...
	<cfelse>
		...
		CASE
			WHEN (
				...
				FROM #request.razuna.session.hostdbprefix#collections_groups fg5
			...
			WHEN (c.col_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theUserID#">) THEN 'X'
		END as colaccess
	</cfif>
	FROM #request.razuna.session.hostdbprefix#collections_text ct, #request.razuna.session.hostdbprefix#collections c
...
<cffunction name="get_assets" output="false">
	...
	SELECT /* #cachetoken#get_assetscol */ ct.col_id_r, ct.file_id_r as cart_product_id, ct.col_file_type, ct.col_item_order, ct.col_file_format,
		...
		END as theextension
	FROM #request.razuna.session.hostdbprefix#collections_ct_files ct 
	LEFT JOIN #request.razuna.session.hostdbprefix#images i ON ct.file_id_r = i.img_id 
	LEFT JOIN #request.razuna.session.hostdbprefix#audios a ON ct.file_id_r = a.aud_id AND a.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="f">
	LEFT JOIN #request.razuna.session.hostdbprefix#videos v ON ct.file_id_r = v.vid_id AND v.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="f">
	LEFT JOIN #request.razuna.session.hostdbprefix#files f ON ct.file_id_r = f.file_id AND f.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="f">
	...
	AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#images WHERE img_id = i.img_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
	AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#audios WHERE aud_id = a.aud_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
	AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#videos WHERE vid_id = v.vid_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
	AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#files WHERE file_id = f.file_id AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
...
<cffunction name="col_asset_move_trash" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#request.razuna.application.datasource#" name="move_trash">
	UPDATE #request.razuna.session.hostdbprefix#collections_ct_files 
...
<cffunction name="col_move_trash" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#request.razuna.application.datasource#" name="move_trash">
	UPDATE #request.razuna.session.hostdbprefix#collections 
	SET in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="get_trash_files" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry">
		...
		FROM #request.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #request.razuna.session.hostdbprefix#images i ON c.file_id_r = i.img_id
		LEFT JOIN  #request.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		...
		FROM #request.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #request.razuna.session.hostdbprefix#audios a  ON c.file_id_r = a.aud_id
		LEFT JOIN  #request.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		...
		FROM #request.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #request.razuna.session.hostdbprefix#videos v ON c.file_id_r = v.vid_id
		LEFT JOIN  #request.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		...
		FROM #request.razuna.session.hostdbprefix#collections_ct_files AS c
		INNER JOIN  #request.razuna.session.hostdbprefix#files f  ON c.file_id_r = f.file_id
		LEFT JOIN  #request.razuna.session.hostdbprefix#collections AS col ON c.col_id_r = col.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="get_trash_collection" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry">
		...
		FROM #request.razuna.session.hostdbprefix#collections AS c
		INNER JOIN #request.razuna.session.hostdbprefix#collections_text AS col_text ON col_text.col_id_r = c.col_id
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
...
<cffunction name="restore_col_asset" output="false">
	...
	SELECT col_id,folder_id_r AS folder_id FROM #request.razuna.session.hostdbprefix#collections 
	...
	<cfelse>
		<cfquery datasource="#request.razuna.application.datasource#" name="thefolderdetail">
		SELECT folder_id,folder_id_r,folder_main_id_r FROM #request.razuna.session.hostdbprefix#folders
		...
		<cfloop query="dir_parent_id">
			...
			<cfelseif get_qry.folder_id EQ dir_parent_id.folder_id_r AND get_qry.in_trash EQ 'F'>
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="update_trash">
				UPDATE #request.razuna.session.hostdbprefix#collections_ct_files 
...
<cffunction name="restore_collection" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#request.razuna.application.datasource#" name="thedetail">
	SELECT folder_id_r,folder_main_id_r FROM #request.razuna.session.hostdbprefix#folders
	...
	<cfelse>
		...
		<cfloop query="dir_parent_id">
			...
			<cfelseif get_qry.folder_id EQ dir_parent_id.folder_id_r AND get_qry.in_trash EQ 'F'>
				<cfset local.root = "yes">
				<cfquery datasource="#request.razuna.application.datasource#" name="qry_update">
				UPDATE #request.razuna.session.hostdbprefix#collections 
				SET in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
				WHERE col_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.col_id#">
				AND folder_id_r = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.folder_id#">
	...
	<!--- Flush Cache --->
	<cfset resetcachetoken(type="general", hostid=request.razuna.session.hostid)>
	<cfset resetcachetoken(type="folder", hostid=request.razuna.session.hostid)>
	<cfset resetcachetoken(type="labels", hostid=request.razuna.session.hostid)>
	<cfif isDefined('local.istrash') AND local.istrash EQ "trash">
		<cfset var is_trash = "intrash">
	<cfelse>
...
<cffunction name="restoreasset" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_restore">
	UPDATE #request.razuna.session.hostdbprefix#collections_ct_files 
	SET 
...
<cffunction name="get_col_count" output="false">
	<cfargument name="thestruct" type="struct">
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_count">
		SELECT COUNT(col_id) AS cnt
		FROM #request.razuna.session.hostdbprefix#collections col
...
<cffunction name="getCollectionFolderCount" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_count">
		...
		FROM #request.razuna.session.hostdbprefix#folders f WHERE in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
...
<cffunction name="getCollectionFileCount" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_count">
		...
		FROM #request.razuna.session.hostdbprefix#collections_ct_files c, #request.razuna.session.hostdbprefix#collections col
		WHERE c.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="T">
		AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
...
<cffunction name="move" output="false">
	...
	<cftry>
		...
		<cfquery datasource="#request.razuna.application.datasource#" name="oneup">
		...
		<cfquery datasource="#request.razuna.application.datasource#" name="current">
		SELECT file_id_r, col_item_order
		FROM #request.razuna.session.hostdbprefix#collections_ct_files
		WHERE col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
...
<cffunction name="removeitem" output="false">
	...
	AND col_item_order <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> 1
	AND col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
	</cfquery>
	<!--- Flush Cache --->
	<cfset resetcachetoken(type="general", hostid=request.razuna.session.hostid)>
...
<cffunction name="remove" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#">
	DELETE FROM #request.razuna.session.hostdbprefix#collections_text
	WHERE col_id_r = <cfqueryparam value="#arguments.thestruct.id#" cfsqltype="CF_SQL_VARCHAR">
	</cfquery>
	<!--- delete collection db --->
	<cfquery datasource="#request.razuna.application.datasource#">
	DELETE FROM #request.razuna.session.hostdbprefix#collections
...
<cffunction name="update" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#collections c, #request.razuna.session.hostdbprefix#collections_text ct
	WHERE c.folder_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.folder_id#">
	AND c.col_id <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "h2" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.col_id#">
	AND c.col_id = ct.col_id_r
	AND ct.col_name = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.thestruct.collectionname#">
	AND c.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	</cfquery>
	...
	<cfif here.recordcount EQ 0>
		...
		UPDATE #request.razuna.session.hostdbprefix#collections
		SET
		col_owner = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#">,
		...
		<cfquery datasource="#request.razuna.application.datasource#">
		DELETE FROM #request.razuna.session.hostdbprefix#collections_groups
		WHERE col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
		</cfquery>
		...
		<cfloop collection="#arguments.thestruct#" item="myform">
			<cfif #myform# CONTAINS "grp_">
				...
				<cfquery datasource="#request.razuna.application.datasource#">
				INSERT INTO #request.razuna.session.hostdbprefix#collections_groups
...
<cffunction name="getallassets" output="false">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="qry.qry_files" cachedwithin="1" region="razcache">
	<!--- For pagination  --->
	<cfif NOT structKeyExists(arguments.thestruct,"searchtext")>
		<cfif request.razuna.application.datasource EQ "mssql">
			SELECT * FROM (
	...
	'0' as vheight, '0' as vwidth, i.hashtag,
		(
			...
			AND ct.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
			AND ct.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
		) AS theorder,
		(
	...
	FROM #request.razuna.session.hostdbprefix#collections_ct_files ct, #request.razuna.session.hostdbprefix#images i LEFT JOIN #request.razuna.session.hostdbprefix#images_text it ON i.img_id = it.img_id_r AND it.lang_id_r = 1  AND i.host_id = it.host_id
	...
	FROM #request.razuna.session.hostdbprefix#collections_ct_files ct, #request.razuna.session.hostdbprefix#videos v LEFT JOIN #request.razuna.session.hostdbprefix#videos_text vt ON v.vid_id = vt.vid_id_r AND vt.lang_id_r = 1 AND v.host_id = vt.host_id
	...
	FROM #request.razuna.session.hostdbprefix#collections_ct_files ct, #request.razuna.session.hostdbprefix#audios a LEFT JOIN #request.razuna.session.hostdbprefix#audios_text aut ON a.aud_id = aut.aud_id_r AND aut.lang_id_r = 1 AND a.host_id = aut.host_id
	...
	FROM #request.razuna.session.hostdbprefix#collections_ct_files ct, #request.razuna.session.hostdbprefix#files f LEFT JOIN #request.razuna.session.hostdbprefix#files_desc ft ON f.file_id = ft.file_id_r AND ft.lang_id_r = 1 AND f.host_id = ft.host_id
	...
	<cfif NOT structKeyExists(arguments.thestruct,"searchtext")>
		...
		<!--- MYSQL --->
		<cfif request.razuna.application.datasource EQ "mysql">
			ORDER BY theorder LIMIT #mysqloffset#,#request.razuna.session.rowmaxpage#
		</cfif>
	</cfif>
	</cfquery>
	<!--- Get the total --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry.qry_filecount">
		SELECT count(ct.col_id_r) AS thetotal 
		FROM #request.razuna.session.hostdbprefix#collections_ct_files ct
		LEFT JOIN #request.razuna.session.hostdbprefix#images i on ct.file_id_r = i.img_id
		LEFT JOIN #request.razuna.session.hostdbprefix#audios a on ct.file_id_r = a.aud_id
		LEFT JOIN #request.razuna.session.hostdbprefix#videos v on ct.file_id_r = v.vid_id
		LEFT JOIN #request.razuna.session.hostdbprefix#files f on ct.file_id_r = f.file_id
		WHERE ct.col_id_r = <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
		AND ct.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		<cfif arguments.thestruct.colaccess EQ 'R'>
		AND (i.expiry_date >=<cfqueryparam cfsqltype="cf_sql_date" value="#now()#"> OR i.expiry_date is null)
...
<cffunction name="dorelease" output="false" returntype="void">
	...
	<cfif trim(arguments.thestruct.col_name) NEQ "">
		<cfquery datasource="#request.razuna.application.datasource#">
		UPDATE #request.razuna.session.hostdbprefix#collections_text
...
<cffunction name="docopy" output="false" returntype="void">
	...
	<cfif arguments.thestruct.copycol>
		...
		INSERT INTO #request.razuna.session.hostdbprefix#collections_ct_files
		(col_id_r, file_id_r, col_file_type, col_item_order, col_file_format, host_id, rec_uuid)
		SELECT <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, file_id_r, col_file_type, col_item_order, col_file_format, host_id, <cfif request.razuna.application.thedatabase EQ "mssql">newid()<cfelseif request.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif request.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif request.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif request.razuna.application.thedatabase EQ "mysql">uuid()</cfif>
		FROM #request.razuna.session.hostdbprefix#collections_ct_files
		WHERE col_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.col_id#">
		</cfquery>
		<!--- Groups --->
		...
		INSERT INTO #request.razuna.session.hostdbprefix#collections_groups
		(col_id_r, grp_id_r, grp_permission, host_id, rec_uuid)
		SELECT <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, grp_id_r, grp_permission, host_id, <cfif request.razuna.application.thedatabase EQ "mssql">newid()<cfelseif request.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif request.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif request.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif request.razuna.application.thedatabase EQ "mysql">uuid()</cfif>
		FROM #request.razuna.session.hostdbprefix#collections_groups
		WHERE col_id_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.col_id#">
		</cfquery>
		<!--- Comments --->
		<cfquery datasource="#request.razuna.application.datasource#">
		INSERT INTO #request.razuna.session.hostdbprefix#comments
		(com_id, asset_id_r, asset_type, user_id_r, com_text, com_date, host_id)
		SELECT <cfif request.razuna.application.thedatabase EQ "mssql">newid()<cfelseif request.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif request.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif request.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif request.razuna.application.thedatabase EQ "mysql">uuid()</cfif>, <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, asset_type, user_id_r, com_text, com_date, host_id
		FROM #request.razuna.session.hostdbprefix#comments
		...
		SELECT <cfif request.razuna.application.thedatabase EQ "mssql">newid()<cfelseif request.razuna.application.thedatabase EQ "oracle">sys_guid()<cfelseif request.razuna.application.thedatabase EQ "db2">generate_unique()<cfelseif request.razuna.application.thedatabase EQ "h2">random_uuid()<cfelseif request.razuna.application.thedatabase EQ "mysql">uuid()</cfif>, <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#newid#">, folder_id_r, widget_name, widget_description, widget_permission, widget_password, widget_style, widget_dl_org, widget_uploading, host_id
...
<cffunction name="restoreallcollectionfiles" output="false">
	...
	<cfloop list="#valueList(qry_trash.file_id)#" index="i">
		...
		<cfset request.razuna.session.file_id = listAppend(request.razuna.session.file_id,i)>
	</cfloop>
	<!--- Return --->
	<cfreturn />
</cffunction>
...
<cffunction name="trash_file_values" output="false">
	...
	<cfloop query="qry_trash">
		<!--- set session file ids --->
		<cfset request.razuna.session.file_id = listAppend(request.razuna.session.file_id,"#file_id#-#kind#")>
	</cfloop>
	<!--- Return --->
	<cfreturn />	
</cffunction>
...
<cffunction name="trash_col_values" output="false">
	...
	<cfloop list="#valueList(qry.col_id)#" index="i">
		<!--- set session col ids --->
		<cfset request.razuna.session.file_id = listAppend(request.razuna.session.file_id,"#i#-collection")>
	</cfloop>
	<!--- Return --->
	<cfreturn />	
</cffunction>
...
<cffunction name="trash_folder_values" output="false">
	...
	<cfloop list="#valueList(qry.folder_id)#" index="i">
		<!--- set session col ids --->
		<cfset request.razuna.session.file_id = listAppend(request.razuna.session.file_id,"#i#-folder")>
	</cfloop>
	<!--- Return --->
	<cfreturn />	
</cffunction>
...
<cffunction name="samecollectionnamecheck" output="false">
	...
	AND ct.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.hostid#">
	AND ct.col_id_r <cfif request.razuna.application.thedatabase EQ "mysql"><><cfelse>!=</cfif> <cfqueryparam value="#arguments.thestruct.col_id#" cfsqltype="CF_SQL_VARCHAR">
	AND c.col_id = ct.col_id_r
...
<cffunction name="getUsage" output="false">
	...
	FROM #request.razuna.session.hostdbprefix#collections_ct_files f, #request.razuna.session.hostdbprefix#collections_text t, #request.razuna.session.hostdbprefix#collections c
