...
<cfcomponent output="false">
	...
	<cffunction name="setup" access="public" output="false">
		...
		FOREIGN KEY (CT_G_P_PER_ID) REFERENCES #arguments.thestruct.theschema#.permissions (PER_ID) ON DELETE CASCADE, 
		...
		(
			...
			CT_U_RU_REMOTE_URL        VARCHAR(500), 
			CT_U_RU_REMOTE_USER_ID    VARCHAR(100) NOT NULL, 
			CT_U_RU_REMOTE_USER_NAME  VARCHAR(200), 
			CT_U_RU_REMOTE_USER_EMAIL VARCHAR(200), 
			CT_U_RU_REMOTE_CONFIRMED  INT DEFAULT 0 NOT NULL, 
			CT_U_RU_UUID              VARCHAR(100), 
	...
	<cffunction name="create_tables" access="public" output="false">
		...
		(
			...
			supplementalcategories	VARCHAR(300),
			urgency					varchar(300),
		...
		(
		  ...
		  IMG_CATEGORY_ONE    VARCHAR(4000),
		  IMG_REMARKS         VARCHAR(4000),
		...
		(
		  ...
		  SET2_META_KEYWORDS            VARCHAR(4000),
		  SET2_META_DESC                VARCHAR(4000),
		  ...
		  SET2_META_CUSTOM              VARCHAR(4000),
		  SET2_URL_SP_ORIGINAL          VARCHAR(200),
		  SET2_URL_SP_THUMB             VARCHAR(200),
		  SET2_URL_SP_COMP              VARCHAR(200),
		  SET2_URL_SP_COMP_UW           VARCHAR(200),
		  SET2_INTRANET_LOGO            VARCHAR(1),
		  SET2_URL_APP_SERVER           VARCHAR(200),
		  SET2_ORA_PATH_INTERNAL        VARCHAR(200),
		  ...
		  SET2_ORA_PATH_INCOMING		VARCHAR(1),
		  SET2_ORA_PATH_INCOMING_BATCH	VARCHAR(1),
		  SET2_ORA_PATH_OUTGOING		VARCHAR(1),
		  SET2_VID_PREVIEW_HEIGTH		INT,
		  SET2_VID_PREVIEW_WIDTH		INT,
		  SET2_PATH_FFMPEG				VARCHAR(200),
		  SET2_VID_PREVIEW_TIME			VARCHAR(10),
		  SET2_VID_PREVIEW_START		VARCHAR(10),
		  SET2_URL_SP_VIDEO				VARCHAR(1),
		  SET2_URL_SP_VIDEO_PREVIEW		VARCHAR(1),
		  ...
		  SET2_PATH_TO_EXIFTOOL         VARCHAR(200),
		  SET2_NIRVANIX_NAME			VARCHAR(500),
		  SET2_NIRVANIX_PASS			VARCHAR(500),
		  HOST_ID						INT,
		  SET2_AWS_BUCKET				VARCHAR(100),
		  SET2_LABELS_USERS				VARCHAR(4000),
		...
		(
		  ...
		  LANG_ID_R     INT,
		  COL_DESC      VARCHAR(4000),
		  COL_KEYWORDS  VARCHAR(4000),
		...
		VID_CATEGORY_ONE		VARCHAR(4000),
		VID_REMARKS				VARCHAR(4000),
		VID_WIDTH				INT,
		VID_HEIGHT				INT,
		...
		<!--- VIDEOS PUBLISHERS --->
		<cfquery datasource="#arguments.thestruct.dsn#">
		CREATE TABLE #arguments.thestruct.theschema#.#arguments.thestruct.host_db_prefix#videos_publishers
		(
		PUB_ID			INT NOT NULL,
		PUB_NAME		VARCHAR(100),
		PUB_RANKING		INT,
		PUB_ORDER		INT,
		PUB_ONLINE		VARCHAR(2),
		PUB_FOLDER_ID	INT,
		PRIMARY KEY (PUB_ID)
		)
		</cfquery>
		<!--- VIDEOS PUBLISHERS GROUPS --->
		<cfquery datasource="#arguments.thestruct.dsn#">
		CREATE TABLE #arguments.thestruct.theschema#.#arguments.thestruct.host_db_prefix#videos_publishers_groups
		(
		GRPNR_ID		INT NOT NULL,
		GRPNR_TEXT		VARCHAR(100),
		GRPNR_ONLINE	VARCHAR(2),
		PUB_ID_R		INT,
		PRIMARY KEY (GRPNR_ID),
		FOREIGN KEY (PUB_ID_R) REFERENCES #arguments.thestruct.theschema#.#arguments.thestruct.host_db_prefix#videos_publishers (PUB_ID) ON DELETE CASCADE
		)
		...
		(
		  	upl_temp_id			varchar(100),
		  	upl_date_create 	timestamp,
		  	upl_date_update		timestamp,
		  	upl_who				varchar(100),
		  	upl_active			VARCHAR(1) DEFAULT '0',
		  	host_id				int,
		  	upl_name			varchar(200),
		  	upl_description		varchar(2000),
	...
	<cffunction name="clearall" access="public" output="false">
		...
		SELECT tabname
		FROM syscat.tables
		WHERE tabschema = '#ucase(request.razuna.session.firsttime.db_schema)#'
		</cfquery>
