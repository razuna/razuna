          ...
          <cfset var="" qry = "0">
                                    ...
                                    WHERE fg3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
                                      ...
                                      WHERE fg3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
                                        ...
                                        WHERE fg3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
                                          ...
                                          WHERE fg3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
                                            AND fg3.folder_id_r = f.folder_id_r
                                            AND fg3.grp_id_r IN (SELECT ct_g_u_grp_id FROM ct_groups_users WHERE ct_g_u_user_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#Session.theUserID#">
                                              )) = 'R' THEN 'R'
                                              WHEN (SELECT fg3.grp_permission
                                              FROM #session.hostdbprefix#folders_groups fg3
                                              WHERE fg3.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
			...
			END as permfolder
                                ...
                                <cfif application.razuna.thedatabase="" EQ="" "mssql">
                                  f.file_id + '-doc'<cfelse>concat(f.file_id,'-doc')</cfif> as listid
                                ...
                                LEFT JOIN #session.hostdbprefix#files_desc fd ON f.file_id = fd.file_id_r AND fd.lang_id_r = <cfqueryparam value="#session.thelangid#" cfsqltype="cf_sql_numeric">
                                  WHERE f.file_id IN (<cfif qrylucene.recordcount="" EQ="" 0="" OR="" cattree.categorytree="" EQ="" "">
                                    '0'<cfelse>
                                      <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#valuelist(cattree.categorytree)#" list="Yes">
                                  </cfif>)
                              ...
                              <cfif arguments.thestruct.doctype="" NEQ="" "">
                                <cfquery dbtype="query" name="qry">
                                  ...
                                  <cfswitch expression="#arguments.thestruct.doctype#">
                                    ...
                                    <cfcase value="other">
                                      WHERE file_extension <cfif variables.database="" EQ="" "oracle" OR="" variables.database="" EQ="" "h2" OR="" variables.database="" EQ="" "db2">
                                        <><cfelse>!=
                                      </cfif> <cfqueryparam value="pdf" cfsqltype="cf_sql_varchar">
                                        AND file_extension <cfif variables.database="" EQ="" "oracle" OR="" variables.database="" EQ="" "h2" OR="" variables.database="" EQ="" "db2">
                                          <><cfelse>!=
                                        </cfif> <cfqueryparam value="xls" cfsqltype="cf_sql_varchar">
                                          AND file_extension <cfif variables.database="" EQ="" "oracle" OR="" variables.database="" EQ="" "h2" OR="" variables.database="" EQ="" "db2">
                                            <><cfelse>!=
                                          </cfif> <cfqueryparam value="xlsx" cfsqltype="cf_sql_varchar">
                                            AND file_extension <cfif variables.database="" EQ="" "oracle" OR="" variables.database="" EQ="" "h2" OR="" variables.database="" EQ="" "db2">
                                              <><cfelse>!=
                                            </cfif> <cfqueryparam value="doc" cfsqltype="cf_sql_varchar">
                                              AND file_extension <cfif variables.database="" EQ="" "oracle" OR="" variables.database="" EQ="" "h2" OR="" variables.database="" EQ="" "db2">
                                                <><cfelse>!=
                                              </cfif> <cfqueryparam value="docx" cfsqltype="cf_sql_varchar">
                                                AND perm = <cfqueryparam cfsqltype="cf_sql_varchar" value="unlocked">
					</cfcase>
                                    <cfdefaultcase>
        ...
        <cfset variables.cachetokenlogs = "getcachetoken"("logs")>
          ...
          <cfset var="" qry = "0">
			...
			END as permfolder
                                ...
                                ,
                                <cfif application.razuna.thedatabase="" EQ="" "mssql">
                                  i.img_id + '-img'<cfelse>concat(i.img_id,'-img')</cfif> as listid
                                ...
                                LEFT JOIN #session.hostdbprefix#images_text it ON i.img_id = it.img_id_r AND it.lang_id_r = <cfqueryparam value="#session.thelangid#" cfsqltype="cf_sql_numeric">
                                  WHERE i.img_id IN (<cfif qrylucene.recordcount="" EQ="" 0="" OR="" cattree.categorytree="" EQ="" "">
                                    '0'<cfelse>
                                      <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#valuelist(cattree.categorytree)#" list="Yes">
                                  </cfif>)
                                  <!--- Only if we have dates --->
        ...
        <cfset variables.cachetokenlogs = "getcachetoken"("logs")>
          ...
          <cfset var="" qry = "0">
			...
			END as permfolder
                                ...
                                ,
                                <cfif application.razuna.thedatabase="" EQ="" "mssql">
                                  v.vid_id + '-vid'<cfelse>concat(v.vid_id,'-vid')</cfif> as listid
                                ...
                                LEFT JOIN #session.hostdbprefix#videos_text vt ON vt.vid_id_r = v.vid_id AND vt.lang_id_r = <cfqueryparam value="#session.thelangid#" cfsqltype="cf_sql_numeric">
                                  ...
                                  WHERE v.vid_id IN (<cfif qrylucene.recordcount="" EQ="" 0="" OR="" cattree.categorytree="" EQ="" "">
                                    '0'<cfelse>
                                      <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#valuelist(cattree.categorytree)#" list="Yes">
                                  </cfif>)
                                  <!--- Only if we have dates --->
  ...
  <cffunction name="search_audios">
    <cfargument name="thestruct" type="struct">
      ...
      <cfset variables.cachetoken = "getcachetoken"("search")>
        <cfset variables.cachetokenlogs = "getcachetoken"("logs")>
          ...
          <cfset var="" qry = "0">
			...
			END as permfolder
                                ...
                                ,
                                <cfif application.razuna.thedatabase="" EQ="" "mssql">
                                  a.aud_id + '-aud'<cfelse>concat(a.aud_id,'-aud')</cfif> as listid
                                ...
                                LEFT JOIN #session.hostdbprefix#audios_text aut ON aut.aud_id_r = a.aud_id AND aut.lang_id_r = <cfqueryparam value="#session.thelangid#" cfsqltype="cf_sql_numeric">
                                  ...
                                  WHERE a.aud_id IN (<cfif qrylucene.recordcount="" EQ="" 0="" OR="" cattree.categorytree="" EQ="" "">
                                    '0'<cfelse>
                                      <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#valuelist(cattree.categorytree)#" list="Yes">
                                  </cfif>)
                                  <!--- Only if we have dates --->
    	...
    	GROUP BY a.aud_id, a.aud_name, a.folder_id_r,a.aud_extension, a.aud_name_org, is_available, a.aud_create_time, a.aud_change_date, link_kind, link_path_url, path_to_asset, cloud_url, cloud_url_org, aut.aud_description, aut.aud_keywords, a.aud_name, a.aud_size, hashtag, folder_name, folder_of_user, folder_Owner
		ORDER BY #sortby#
		</cfquery>
                            <!--- Select only records that are unlocked --->
                            <cfquery dbtype="query" name="qry">
