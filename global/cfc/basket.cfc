...
<cffunction name="tobasket" output="true" access="public">
	...
	<cfparam name="arguments.thestruct.datasource" default="#request.razuna.application.datasource#">
	<cfparam name="arguments.thestruct.thedatabase" default="#request.razuna.application.thedatabase#">
...
<!--- 	<cfset consoleoutput(true, true)>
	...
	<cfthread intstruct="#arguments.thestruct#">
		...
		<cfloop index="thenr" delimiters="," list="#attributes.intstruct.file_id#">
			...
			SELECT user_id
			FROM #request.razuna.session.hostdbprefix#cart
			WHERE cart_id = <cfqueryparam value="#request.razuna.session.thecart#" cfsqltype="cf_sql_varchar">
			<cfif attributes.intstruct.fromshare EQ "F">
				AND user_id = <cfqueryparam value="#request.razuna.session.theuserid#" cfsqltype="CF_SQL_VARCHAR">
			</cfif>
			...
			<!--- <cfif here.recordcount EQ 0> --->
				...
				<cfif thenr NEQ 0 AND len(thetype) LTE 5>
					...
					<cfif request.razuna.application.thedatabase EQ "mysql">INSERT IGNORE<cfelse>MERGE</cfif> INTO #attributes.intstruct.sessions.hostdbprefix#cart
...
<cffunction name="readbasketquick" output="false" returnType="query">
	...
	<!--- Get total --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_count" cachedwithin="1" region="razcache">
		SELECT  /* #cachetoken#readbasketquickcount */ count(cart_id) AS total
		FROM #request.razuna.session.hostdbprefix#cart c
		WHERE c.cart_id = <cfqueryparam value="#request.razuna.session.thecart#" cfsqltype="cf_sql_varchar">
...
<cffunction name="removeitem" output="false">
	...
	AND cart_id = <cfqueryparam value="#request.razuna.session.thecart#" cfsqltype="cf_sql_varchar">
	<!--- AND user_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.razuna.session.theuserid#"> --->
	</cfquery>
	<!--- Flush Cache --->
	<cfset variables.cachetoken = resetcachetoken(type="general", hostid=request.razuna.session.hostid)>
	<cfreturn />
...
<cffunction name="writebasket" output="true">
	...
	<cfloop query="thebasket">
		...
		<cfswitch expression="#cart_file_type#">
			...
			<cfcase value="img">
				...
				<cfif request.razuna.session.allorg EQ "true">
					<cfset arguments.thestruct.artofimage = cart_product_id & "-original,">
				</cfif>
				<cfif request.razuna.session.allthumb EQ "true">
				...
				<cfif request.razuna.session.allrend EQ "true">
				...
				<cfif request.razuna.session.allvers EQ "true">
	...
	<cfif NOT structkeyexists(arguments.thestruct,"fromzip") AND !arguments.thestruct.noemail>
		...
		<cfinvoke component="email" method="send_email" subject="#basket_download_available_sub#" themessage="#basket_download_available_msg# <br/> <a href='#request.razuna.session.thehttp##cgi.HTTP_HOST##sn#/outgoing/#arguments.thestruct.zipname#'>#request.razuna.session.thehttp##cgi.HTTP_HOST##sn#/outgoing/#arguments.thestruct.zipname#</a>">
	</cfif>
	<!--- Feedback --->
	<cfif !arguments.thestruct.noemail>
		<cfinvoke component="defaults" method="trans" transid="download_basket_output3" returnvariable="download_basket_output3" />
		<cfoutput><br/><br/><strong style="color:green;"><a href="#request.razuna.session.thehttp##cgi.HTTP_HOST##sn#/outgoing/#arguments.thestruct.zipname#" style="color:green;">#download_basket_output3#</a></strong><br><br></cfoutput>
		<cfflush>
	</cfif>
...
<cffunction name="savedesckey" access="public">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="getfileid" >
		SELECT file_id FROM #request.razuna.session.hostdbprefix#files WHERE file_name = '#arguments.thestruct.thefile#'
	</cfquery>
	...
	<cfloop delimiters="," list="#arguments.thestruct.artoffile#" index="art">
		...
		<cfif arguments.thestruct.theid EQ thefileid>
			...
			<cfif theart EQ "versions" >
				...
				<cfquery datasource="#request.razuna.application.datasource#" name="arguments.thestruct.qry" cachedwithin="1" region="razcache">
				...
				<cfif !awsfileexists>
					...
					<cftry>
						...
						<cfif art contains "doc">
							...
							<cfset request.razuna.session.theawsurl["#arguments.thestruct.thename#"] = AmazonS3geturl(
							...
							<cfif arguments.thestruct.cs.basket_awsurl NEQ "">
								<cfset request.razuna.session.theawsurl["#arguments.thestruct.thename#"] = replacenocase(request.razuna.session.theawsurl["#arguments.thestruct.thename#"] ,"https://s3.amazonaws.com","#arguments.thestruct.cs.basket_awsurl#","ALL")>
							</cfif>
						</cfif>
						<cfcatch>
			...
			<!--- Local --->
			<cfif request.razuna.application.storage EQ "local" AND arguments.thestruct.qry.link_kind EQ "">
			...
			<!--- Amazon --->
			<cfelseif request.razuna.application.storage EQ "amazon" AND arguments.thestruct.qry.link_kind EQ "">
				<!--- set asset path --->
				<cfif theart EQ "versions">
			...
			<!--- Akamai --->
			<cfelseif request.razuna.application.storage EQ "akamai" AND arguments.thestruct.qry.link_kind EQ "">
				<cfif theart EQ "versions">
					<cfthread name="#ttd#" intstruct="#arguments.thestruct#">
		...
		<cfif arguments.thestruct.theid EQ theimgid>
			...
			<cfif theart EQ "versions">
				...
				<cfquery name="qry" datasource="#request.razuna.application.datasource#">
					...
					FROM #request.razuna.session.hostdbprefix#additional_versions av LEFT JOIN #request.razuna.session.hostdbprefix#images i ON av.asset_id_r = i.img_id
			...
			<cfif qry.img_group NEQ "">
				<cfquery name="qrysub" datasource="#request.razuna.application.datasource#">
				...
				<cfif !awsfileexists>
					...
					<cftry>
						...
						<cfif art contains "original">
							...
							<cfset request.razuna.session.theawsurl["#arguments.thestruct.thefinalname#"] = AmazonS3geturl(
							 datasource='#arguments.thestruct.awsdatasource#',
							 bucket='#arguments.thestruct.awsbucket#',
							...
							<cfif arguments.thestruct.cs.basket_awsurl NEQ "">
								<cfset request.razuna.session.theawsurl["#arguments.thestruct.thefinalname#"] = replacenocase(request.razuna.session.theawsurl["#arguments.thestruct.thefinalname#"] ,"https://s3.amazonaws.com","#arguments.thestruct.cs.basket_awsurl#","ALL")>
							</cfif>
						</cfif>
						<cfcatch>
							<cfthrow detail="#cfcatch.detail#<br/>#cfcatch.message#">
		...
		<cfif arguments.thestruct.theid EQ thevidid>
			...
			<cfif theart EQ "versions">
				...
				<cfquery name="qry" datasource="#request.razuna.application.datasource#">
					...
					FROM #request.razuna.session.hostdbprefix#additional_versions av
				...
				<cfif !awsfileexists>
					...
					<cftry>
						...
						<cfif art contains "video">
							...
							<!--- Calculate average upload speed --->
							<cfset application.uploadspeed = (application.uploadspeed + arguments.thestruct.filesize/timediff)/2>
							<cfset request.razuna.session.theawsurl["#arguments.thestruct.thenewname#"] = AmazonS3geturl(
							 datasource='#arguments.thestruct.awsdatasource#',
							 bucket='#arguments.thestruct.awsbucket#',
							...
							<cfif arguments.thestruct.cs.basket_awsurl NEQ "">
								<cfset request.razuna.session.theawsurl["#arguments.thestruct.thenewname#"] = replacenocase(request.razuna.session.theawsurl["#arguments.thestruct.thenewname#"] ,"https://s3.amazonaws.com","#arguments.thestruct.cs.basket_awsurl#","ALL")>
							</cfif>
						</cfif>
						<cfcatch>
							<cfthrow detail="#cfcatch.detail#<br/>#cfcatch.message#">
...
<cffunction name="basket_order" output="false">
	<cfargument name="thestruct" type="struct">
	<!--- Query user info --->
	<cfquery datasource="#request.razuna.application.datasource#" name="qry_user">
	SELECT u.user_email, f.share_order_user
	FROM users u, #request.razuna.session.hostdbprefix#folders f
	WHERE f.folder_id = <cfqueryparam value="#request.razuna.session.fid#" cfsqltype="CF_SQL_VARCHAR">
	AND f.share_order_user = u.user_id
	</cfquery>
	<!--- Save info --->
	<cfquery datasource="#request.razuna.application.datasource#">
	UPDATE #request.razuna.session.hostdbprefix#cart
	SET
	...
	<!--- Get date format --->
	<cfinvoke component="defaults" method="getdateformat" dsn="#request.razuna.application.datasource#" returnVariable="thedateformat" />
	<!--- Send out eMail to the one who is responsible for Orders --->
	<cfset var thesubject = "Basket Order: #request.razuna.session.thecart#">
	<cfset var mailmessage = "Hello,
		The below order just came in:<br/><br/>

		Basket Order: #request.razuna.session.thecart#<br/>
...
<cffunction name="get_orders" output="false">
	...
	AND cart_order_done <cfif request.razuna.application.thedatabase EQ "oracle" OR request.razuna.application.thedatabase EQ "db2"><><cfelse>!=</cfif> <cfqueryparam value="1" cfsqltype="CF_SQL_VARCHAR">
	<!--- AND cart_order_user_r = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#request.razuna.session.theuserid#"> --->
...
<cffunction name="writebasket2aws" output="true">
	...
	<cfset request.razuna.session.theawsurl = structnew()>
	<cfset var aws_urls = "">
	<cfset var error = "">
	<cfset var done = "">
	...
	<cftry>
		...
		<cfquery name="qry_aws_bucket" datasource="#request.razuna.application.datasource#">
			SELECT set_pref
		...
		<cfquery name="qry_aws_key" datasource="#request.razuna.application.datasource#">
			SELECT set_pref
			FROM #request.razuna.session.hostdbprefix#settings
		...
		<cfquery name="qry_aws_secret_key" datasource="#request.razuna.application.datasource#">
			SELECT set_pref
			FROM #request.razuna.session.hostdbprefix#settings
		...
		<cfquery name="qry_aws_location" datasource="#request.razuna.application.datasource#">
			SELECT set_pref
			FROM #request.razuna.session.hostdbprefix#settings
		...
		<cfoutput>#serializeJSON(res)#</cfoutput>
		<cfflush>
		<cfif !structIsEmpty(request.razuna.session.theawsurl)>
			<cfoutput>
				<cfsavecontent variable="res.message">
					<strong>AWS URL's</strong><br/>#aws_urls#<br/><br/>
					<font size="1">
					<cfloop collection="#request.razuna.session.theawsurl#" item="theurl">
						#theurl# : <a href="#structfind(request.razuna.session.theawsurl,theurl)#" target="_blank">#structfind(request.razuna.session.theawsurl,theurl)#</a><br/>
					</cfloop>
					</font>
				</cfsavecontent>
				#serializeJSON(res)#
