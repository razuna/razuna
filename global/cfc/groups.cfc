...
<cffunction  name="getdetail" returntype="query">
	...
	SELECT grp_id, grp_name, grp_host_id, grp_mod_id, grp_translation_key,
		(
			...
			AND uh.ct_u_h_host_id = <cfqueryparam value="#request.razuna.session.hostid#" cfsqltype="cf_sql_numeric">
		) AS usercount
	FROM groups
	WHERE (
		grp_host_id = <cfqueryparam value="#request.razuna.session.hostid#" cfsqltype="cf_sql_numeric">
...
<cffunction name="getall" access="public" returntype="query">
	...
	<cfargument name="host_id" default="#request.razuna.session.hostid#" type="numeric" required="false">
...
<cffunction  name="update" returntype="void">
	<cfargument name="thestruct" type="Struct">
	<cfquery datasource="#request.razuna.application.datasource#" name="getfolder_subscribe_bef_change">
...
<cffunction  name="add_grp_users2notify" returntype="void">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="checkgrpsettings">
	...
	<cfif checkgrpsettings.folder_subscribe EQ 'true'>
		<cfquery datasource="#request.razuna.application.datasource#" name="getusers_fs">
			SELECT fg.folder_id_r, cu.ct_g_u_user_id user_id 
			FROM ct_groups_users cu, #request.razuna.session.hostdbprefix#folders_groups fg, #request.razuna.session.hostdbprefix#folders f
			...
			AND f.in_trash ='F'
			AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#folder_subscribe fs WHERE fs.folder_id = fg.folder_id_r AND fs.user_id = cu.ct_g_u_user_id)
		...
		<cfquery datasource="#request.razuna.application.datasource#" name="getgroups_fs">
			SELECT fs.folder_id folder_id,  cu.ct_g_u_grp_id group_id
			FROM ct_groups_users cu, #request.razuna.session.hostdbprefix#folder_subscribe fs,#request.razuna.session.hostdbprefix#folders_groups fg
			WHERE cu.ct_g_u_user_id = fs.user_id
			AND cu.ct_g_u_grp_id = fg.grp_id_r
			AND cu.ct_g_u_grp_id = <cfqueryparam value="#arguments.group_id#" cfsqltype="CF_SQL_VARCHAR">
			AND fg.folder_id_r = fs.folder_id
			AND NOT EXISTS (SELECT 1 FROM #request.razuna.session.hostdbprefix#folder_subscribe_groups fsg WHERE fsg.folder_id = fs.folder_id AND fsg.group_id= fg.grp_id_r)
			GROUP BY fs.folder_id,  cu.ct_g_u_grp_id
		</cfquery>
		<cfloop query="getgroups_fs">
			<cfquery datasource="#request.razuna.application.datasource#">
				INSERT INTO #request.razuna.session.hostdbprefix#folder_subscribe_groups (folder_id, group_id)
				VALUES(
...
<cffunction  name="notifications_unsubscribe" returntype="void">
	...
	<cfquery datasource="#request.razuna.application.datasource#" name="get_unsubscribe_items">
		SELECT fs.fs_id
		FROM ct_groups_users cu, #request.razuna.session.hostdbprefix#folder_subscribe fs
	...
	<cfloop query = "get_unsubscribe_items">
		<cfquery datasource="#request.razuna.application.datasource#">
			DELETE FROM #request.razuna.session.hostdbprefix#folder_subscribe WHERE fs_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#get_unsubscribe_items.fs_id#">
		</cfquery>
	</cfloop>
	<!--- Remove group subscription --->
	<cfquery datasource="#request.razuna.application.datasource#">
		DELETE FROM #request.razuna.session.hostdbprefix#folder_subscribe_groups WHERE group_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.group_id#">
	</cfquery>

	<!--- Delete left over users that are no longer subscribed to any groups--->
	<cfquery datasource="#request.razuna.application.datasource#" name="get_leftover_items">
		SELECT fs.fs_id
	...
	<cfloop query = "get_leftover_items">
		<cfquery datasource="#request.razuna.application.datasource#">
			DELETE FROM #request.razuna.session.hostdbprefix#folder_subscribe WHERE fs_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#get_leftover_items.fs_id#">
		</cfquery>
	</cfloop>
</cffunction>
<!--- ------------------------------------------------------------------------------------- --->
