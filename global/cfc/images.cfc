...
<cffunction name="getFolderCount" description="COUNT ALL IMAGES IN A FOLDER" output="false" access="public" returntype="numeric">
	...
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qLocal" cachedwithin="1" region="razcache">
		...
		<cfif arguments.thestruct.razuna.session.thisapp EQ "admin">
...
<cffunction name="getFolderAssets" access="public" description="GET ALL RECORDS OF THIS TYPE IN A FOLDER" output="false" returntype="query">
	...
	<cfif arguments.thestruct.razuna.session.showsubfolders EQ "T">
		<cfinvoke component="folders" method="getfoldersinlist" dsn="#arguments.thestruct.razuna.application.datasource#" folder_id="#arguments.folder_id#" hostid="#arguments.thestruct.razuna.session.hostid#" database="#arguments.thestruct.razuna.application.thedatabase#" returnvariable="thefolders" thestruct="#arguments.thestruct#">
	...
	<cfif arguments.thestruct.razuna.application.thedatabase EQ "oracle">
		...
		FROM (
			...
			FROM (
				...
				FROM #arguments.thestruct.razuna.session.hostdbprefix#images i LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#images_text it ON i.img_id = it.img_id_r AND it.lang_id_r = 1  AND i.host_id = it.host_id
	...
	<cfelse>
		...
		AND NOT EXISTS (SELECT 1 FROM #arguments.thestruct.razuna.session.hostdbprefix#images WHERE img_id = c.asset_id_r AND in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="t">)
		</cfquery>
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#images i LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#images_text it ON i.img_id = it.img_id_r AND it.lang_id_r = 1 AND i.host_id = it.host_id LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#xmp x ON x.id_r = i.img_id AND i.host_id = x.host_id
		WHERE i.folder_id_r IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#thefolderlist#" list="true">)
		AND (i.img_group IS NULL OR i.img_group = '')
		AND i.in_trash = <cfqueryparam cfsqltype="cf_sql_varchar" value="F">
		AND i.host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	...
	<cfif arguments.thestruct.razuna.session.customfileid NEQ "">
		...
		SELECT *
		FROM qLocal
		WHERE img_id IN (<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#arguments.thestruct.razuna.session.customfileid#" list="true">)
		</cfquery>
	</cfif>
...
<cffunction name="getAssetDetails" access="public" output="true" returntype="query">
	...
	<cfargument name="ColumnList" required="false" type="string" default="img_id, img_filename">
	<cfargument name="thestruct" type="struct" required="true" />
	<!--- init local vars --->
	<cfset var qLocal = 0>
	<!--- Get the cachetoken for here --->
...
<cffunction name="filedetail" access="public" output="false" returntype="query">
	...
	SELECT /* #cachetoken#filedetailimg */ #arguments.thecolumn#, CASE WHEN NOT(i.img_group ='' OR i.img_group is null) THEN (SELECT expiry_date FROM #arguments.thestruct.razuna.session.hostdbprefix#images WHERE img_id = i.img_group) ELSE expiry_date END expiry_date_actual
...
<cffunction name="removeimage" output="false">
	...
	<cfinvoke method="filedetail" theid="#arguments.thestruct.id#" thecolumn="img_filename, folder_id_r, img_filename_org filenameorg, lucene_key, link_kind, link_path_url, path_to_asset, thumb_extension, img_group" returnvariable="thedetail" thestruct="#arguments.thestruct#">
	<cfif thedetail.recordcount NEQ 0>
		...
		<cfinvoke component="global" method="update_dates" type="img" fileid="#thedetail.img_group#" thestruct="#arguments.thestruct#" />
		<!--- Delete from files DB (including referenced data) --->
		<cfquery datasource="#arguments.thestruct.razuna.application.datasource#">
		DELETE FROM #arguments.thestruct.razuna.session.hostdbprefix#images
...
<cffunction name="gettrashimage" output="false" returntype="Query">
	...
	<cfset var cachetoken = getcachetoken(type="images", hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
	<!--- Query --->
	<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_image" cachedwithin="#CreateTimeSpan(0,0,5,0)#" region="razcache">
	...
	i.hashtag, 'false' AS in_collection, 'images' as what, '' AS folder_main_id_r, <cfif arguments.thestruct.razuna.application.thedatabase EQ "mssql">i.img_id + '-img'<cfelse>concat(i.img_id,'-img')</cfif> as listid
	<!--- Permfolder --->
...
<cffunction name="removeimagemany" output="true">
	...
	<cfloop list="#arguments.thestruct.id#" index="i" delimiters=",">
		<cfset i = listfirst(i,"-")>
		<!--- Get file detail for log --->
		<cfinvoke method="filedetail" theid="#i#" thecolumn="img_filename, folder_id_r, img_filename_org filenameorg, lucene_key, link_kind, link_path_url, path_to_asset, thumb_extension" returnvariable="thedetail" thestruct="#arguments.thestruct#">
		<cfif thedetail.recordcount NEQ 0>
...
<cffunction name="deletefromfilesystem" output="false">
	...
	<cftry>
		...
		<cfif arguments.thestruct.razuna.application.storage EQ "local">
			...
			<!--- Versions --->
			<cfif DirectoryExists("#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/versions/img/#arguments.thestruct.id#") AND arguments.thestruct.id NEQ "">
				<cfdirectory action="delete" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/versions/img/#arguments.thestruct.id#" recurse="true">
			</cfif>
		<!--- Amazon --->
		<cfelseif arguments.thestruct.razuna.application.storage EQ "amazon">
			<cfif arguments.thestruct.qrydetail.path_to_asset NEQ "">
				...
				<!--- Versions --->
				<cfinvoke component="amazon" method="deletefolder" folderpath="versions/img/#arguments.thestruct.id#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
			</cfif>
		<!--- Akamai --->
...
<cffunction name="detail" output="false">
	...
	LEFT JOIN #arguments.thestruct.razuna.session.hostdbprefix#folders fo ON fo.folder_id = i.folder_id_r AND fo.host_id = i.host_id
...
<cffunction name="updatethread" output="false">
	...
	<cfloop list="#arguments.thestruct.file_id#" delimiters="," index="i">
		...
		<!--- Update main record with dates --->
		<cfinvoke component="global" method="update_dates" type="img" fileid="#arguments.thestruct.file_id#" thestruct="#arguments.thestruct#" />
		...
		<cfif qryorg.recordcount neq 0>
			...
			<cfset log_assets(theuserid=arguments.thestruct.razuna.session.theuserid,logaction='Update',logdesc='#updated##rend#: #qryorg.img_filename#',logfiletype='img',assetid='#theid#',folderid='#qryorg.folder_id_r#', hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
		<cfelse>
...
<cffunction name="convertImagethread" output="true">
	...
	<cfinvoke method="filedetail" theid="#arguments.thestruct.file_id#" thecolumn="img_id,folder_id_r,img_filename_org,img_extension,thumb_extension,img_filename,path_to_asset,img_width,img_height,cloud_url_org" returnvariable="arguments.thestruct.qry_detail" thestruct="#arguments.thestruct#">
	...
	<cfif arguments.thestruct.link_kind NEQ "lan">
		<cfif arguments.thestruct.razuna.application.storage EQ "local">
			...
			<cfthread name="#tempfolder#" intstruct="#arguments.thestruct#">
				<cffile action="copy" source="#attributes.intstruct.assetpath#/#attributes.intstruct.razuna.session.hostid#/#attributes.intstruct.qry_detail.path_to_asset#/#attributes.intstruct.qry_detail.img_filename_org#" destination="#attributes.intstruct.thisfolder#/#attributes.intstruct.thename#" mode="775">
			</cfthread>
			<!--- Thumb --->
	...
	<cfloop delimiters="," list="#arguments.thestruct.convert_to#" index="theformat">
		...
		<cfif arguments.thestruct.upl_template NEQ 0 AND arguments.thestruct.upl_template NEQ "undefined" AND arguments.thestruct.upl_template NEQ "">
			...
			<!--- Get DPI --->
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_d">
			SELECT upl_temp_value
			FROM #arguments.thestruct.razuna.session.hostdbprefix#upload_templates_val
			...
			<!--- Get watermark --->
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_wm">
			SELECT upl_temp_value
			FROM #arguments.thestruct.razuna.session.hostdbprefix#upload_templates_val
		...
		<cfif structKeyExists(arguments.thestruct,"convert_wm_#theformat#") AND #arguments.thestruct["convert_wm_" & #theformat#]# NEQ "">
			<cfif "convert_wm_#theformat#" NEQ "" >
				...
				<cfif thewm.wmval.wm_use_image>
					<cfexecute name="#thecomposite#" arguments="-dissolve #thewm.wmval.wm_image_opacity#% -gravity #thewm.wmval.wm_image_position# #arguments.thestruct.rootpath#global/host/watermark/#arguments.thestruct.razuna.session.hostid#/#thewm.wmval.wm_image_path# #theformatconv# #theformatconv#" timeout="90" errorVariable="err"/>
				</cfif>
		...
		<cfif arguments.thestruct.save_renditions>
			...
			<cfif arguments.thestruct.razuna.application.storage EQ "local">
				...
				<cfset arguments.thestruct.thisfolder = thisfolder >
				<cfset arguments.thestruct.host_id = arguments.thestruct.razuna.session.hostid>
				<cfset arguments.thestruct.theformat = theformat>
				...
				<cfdirectory action="create" directory="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#arguments.thestruct.qry_detail.folder_id_r#/img/#arguments.thestruct.newid#" mode="775">
				<!--- Move original image --->
				<cffile action="#arguments.thestruct.file_action#" source="#thisfolder#/#arguments.thestruct.thenamenoext#.#theformat#" destination="#arguments.thestruct.assetpath#/#arguments.thestruct.razuna.session.hostid#/#arguments.thestruct.qry_detail.folder_id_r#/img/#arguments.thestruct.newid#/#arguments.thestruct.thenamenoext#.#theformat#" mode="775">
				<!--- Move thumbnail --->
			...
			<cfelseif arguments.thestruct.razuna.application.storage EQ "amazon">
				...
				<!--- Get signed URLS --->
				<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url_org" key="#arguments.thestruct.qry_detail.folder_id_r#/img/#arguments.thestruct.newid#/#arguments.thestruct.thenamenoext#.#arguments.thestruct.theformat#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				<!--- Check the source file is animated or not. --->
				<cfif arguments.thestruct.qry_detail.thumb_extension EQ 'gif' AND theformat EQ 'gif'>
					<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url" key="#arguments.thestruct.qry_detail.folder_id_r#/img/#arguments.thestruct.newid#/thumb_#arguments.thestruct.file_id#.#theformat#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				<cfelse>
				<cfinvoke component="amazon" method="signedurl" returnVariable="cloud_url" key="#arguments.thestruct.qry_detail.folder_id_r#/img/#arguments.thestruct.newid#/thumb_#arguments.thestruct.file_id#.#arguments.thestruct.qry_settings_image.set2_img_format#" awsbucket="#arguments.thestruct.awsbucket#" thestruct="#arguments.thestruct#" />
				</cfif>
			<!--- Akamai --->
			<cfelseif arguments.thestruct.razuna.application.storage EQ "akamai">
				<cfthread name="upload#theformat##arguments.thestruct.newid#" intstruct="#arguments.thestruct#">
			...
			<cfquery datasource="#arguments.thestruct.razuna.application.datasource#" name="qry_colorspace">
			SELECT colorspace
...
<cffunction name="writeimage" output="true">
	...
	<cfloop delimiters="," list="#arguments.thestruct.razuna.session.artofimage#" index="art">
		...
		FROM #arguments.thestruct.razuna.session.hostdbprefix#images i, #arguments.thestruct.razuna.session.hostdbprefix#settings_2 s
		...
		<cfif qry.img_group NEQ "">
			<cfquery name="qry_fn" datasource="#arguments.thestruct.razuna.application.datasource#">
			SELECT img_filename
			FROM #arguments.thestruct.razuna.session.hostdbprefix#images
...
<cffunction name="move" output="false">
	<cfargument name="thestruct" type="struct">
		...
		<!--- Move --->
		<cfinvoke method="filedetail" theid="#arguments.thestruct.img_id#" thecolumn="img_filename, folder_id_r" thestruct="#arguments.thestruct#" returnvariable="arguments.thestruct.qryimg">
		...
		<cfinvoke component="global" method="getAlias" asset_id_r="#arguments.thestruct.img_id#" folder_id_r="#arguments.thestruct.razuna.session.thefolderorg#" thestruct="#arguments.thestruct#" returnvariable="qry_alias"/>
		<!--- If this is an alias --->
		<cfif qry_alias>
			<!--- Move alias --->
			<cfinvoke component="global" method="moveAlias" asset_id_r="#arguments.thestruct.img_id#" new_folder_id_r="#arguments.thestruct.folder_id#" pre_folder_id_r="#arguments.thestruct.razuna.session.thefolderorg#" thestruct="#arguments.thestruct#" />
		<cfelse>
			...
			<cfif arguments.thestruct.qryimg.recordcount NEQ 0 AND arguments.thestruct.folder_id NEQ arguments.thestruct.qryimg.folder_id_r>
				...
				<!--- <cfthread intstruct="#arguments.thestruct#"> --->
					<!--- Update Dates --->
					<cfinvoke component="global" method="update_dates" type="img" fileid="#arguments.thestruct.img_id#" thestruct="#arguments.thestruct#" />
				...
				<cfset log_assets(theuserid=arguments.thestruct.razuna.session.theuserid,logaction='Move',logdesc='#moved#: #arguments.thestruct.qryimg.img_filename#',logfiletype='img',assetid=arguments.thestruct.img_id,folderid='#arguments.thestruct.folder_id#', hostid=arguments.thestruct.razuna.session.hostid, thestruct=arguments.thestruct)>
			</cfif>
		</cfif>
	<cfreturn />
</cffunction>
...
<cffunction name="copymetadataupdate" output="false" >
	...
	<!--- <cfquery name="select_images" datasource="#arguments.thestruct.razuna.application.datasource#">
		SELECT img_filename,shared FROM #arguments.thestruct.razuna.session.hostdbprefix#images
		WHERE img_id = <cfqueryparam value="#arguments.thestruct.file_id#" cfsqltype="cf_sql_varchar" >
		AND host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.thestruct.razuna.session.hostid#">
	</cfquery> --->
	...
	<cfquery name="select_xmp" datasource="#arguments.thestruct.razuna.application.datasource#">
		...
		datecreated,city,ciadrregion,country,countrycode,scene,state,credit,rights FROM #arguments.thestruct.razuna.session.hostdbprefix#xmp
	...
	<cfif arguments.thestruct.insert_type EQ 'replace'>
		<cfloop query = "select_images_text">
			<cfquery name="updateimges_text" datasource="#arguments.thestruct.razuna.application.datasource#">
				UPDATE #arguments.thestruct.razuna.session.hostdbprefix#images_text SET
	...
	<cfelse>
		<cfloop list="#arguments.thestruct.idList#" index="theidtoupdate" >
			<cfloop query = "select_images_text">
				<cfquery name="append_images_text" datasource="#arguments.thestruct.razuna.application.datasource#">
				...
				<cfquery name="updateimagestext" datasource="#arguments.thestruct.razuna.application.datasource#">
					UPDATE #arguments.thestruct.razuna.session.hostdbprefix#images_text SET
			...
			<cfquery name="append_xmp" datasource="#arguments.thestruct.razuna.application.datasource#">
				SELECT subjectcode,creator,title,authorsposition,captionwriter,ciadrextadr,category,supplementalcategories,urgency,description,ciadrcity,ciadrctry,location,
				ciadrpcode,ciemailwork,ciurlwork,citelwork,intellectualgenre,instructions,source,usageterms,copyrightstatus,transmissionreference,webstatement,headline,
				datecreated,city,ciadrregion,country,countrycode,scene,state,credit,rights FROM #arguments.thestruct.razuna.session.hostdbprefix#xmp
			...
			<cfquery name="updatexmp" datasource="#arguments.thestruct.razuna.application.datasource#" >
				UPDATE #arguments.thestruct.razuna.session.hostdbprefix#xmp
					SET
