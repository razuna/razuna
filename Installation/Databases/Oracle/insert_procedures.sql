...
-- Delete Many Tables
--------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE &theuser..delete_many_tables
...
(filename IN VARCHAR2, tabname IN VARCHAR2, lang IN NUMBER, recid IN NUMBER, thedir IN VARCHAR2)
IS
img ordsys.ordimage;
ctx RAW(64) := NULL;
BEGIN
...
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET cat_image = Ordsys.OrdImage.init(''http'',''' || thedir || ''',''' || filename || ''') WHERE cat_id_r = ' || recid || ' AND lang_id_r = ' || lang ;
EXECUTE IMMEDIATE
...
-- init the column
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET ' || colname || ' = Ordsys.OrdImage.init(''http'',''' || thedir || ''',''' || filename || ''') WHERE img_id =' || theid;
...
EXECUTE IMMEDIATE
'INSERT INTO ' || tabname || ' (img_id, image, imagesignature) VALUES (' || dest_id || ', ordsys.ordimage.init(''http'',''' || thedir || ''', ''' || filename || '''), ordsys.ordimagesignature.init()) RETURNING image, imagesignature INTO :1, :2'
...
-- init the column
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET ' || colname || ' = Ordsys.OrdImage.init(''http'',''' || thedir || ''',''' || filename || ''') WHERE set2_id =' || theid;

-- select the column
EXECUTE IMMEDIATE
'Select ' || colname || ' from ' || tabname || ' WHERE set2_id = ' || theid || ' for update ' INTO img;
...
-- Write the image back into table
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET ' || colname || ' = :1 WHERE set2_id =' || theid USING img;
...
-- get the table for updating
EXECUTE IMMEDIATE
'insert into ' || tabname || ' (file_id_r, file_preview, file_name, file_preview_order) VALUES(:1, Ordsys.OrdImage.init(''http'',''' || thedir || ''',''' || filename || '''), :2, :3)' USING recid, filename, theorder;
...
-- Get the XML
meta := httpuritype.createuri(''|| dir_name ||''|| file_name ||'').getxml();
...
-- Update
--EXECUTE IMMEDIATE
--'UPDATE ' || tabname || ' SET comp = :1 WHERE img_id = ' || theid USING comp;
...
-- Update
--EXECUTE IMMEDIATE
--'UPDATE ' || tabname || ' SET comp_uw = :1 WHERE img_id = ' || theid USING comp_uw;
...
-- Import
thumb.putMetadata(meta);

-- Update
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET thumb = :1 WHERE img_id = ' || theid USING thumb;
...
-- create triggers
--------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE &theuser..create_triggers
...
--update_vid_description
execute immediate
'CREATE OR REPLACE TRIGGER ' || theschema || '.' || thepref || 'UPDATE_VID_DESCRIPTION AFTER INSERT OR UPDATE OR DELETE ON ' || theschema || '.' || thepref || 'VIDEOS_TEXT DECLARE v_job NUMBER; BEGIN IF DELETING THEN DBMS_JOB.SUBMIT (v_job, ''ctx_ddl.optimize_index(''''' || thepref || 'vid_description_idx'''',''''FULL'''');'', SYSDATE); ELSE DBMS_JOB.SUBMIT (v_job, ''ctx_ddl.sync_index(''''' || thepref || 'vid_description_idx'''');'', SYSDATE); END IF; END;';

--update_vid_keywords
execute immediate
'CREATE OR REPLACE TRIGGER ' || theschema || '.' || thepref || 'UPDATE_VID_KEYWORDS AFTER INSERT OR UPDATE OR DELETE ON ' || theschema || '.' || thepref || 'VIDEOS_TEXT DECLARE v_job NUMBER; BEGIN IF DELETING THEN DBMS_JOB.SUBMIT (v_job, ''ctx_ddl.optimize_index(''''' || thepref || 'vid_keywords_idx'''',''''FULL'''');'', SYSDATE); ELSE DBMS_JOB.SUBMIT (v_job, ''ctx_ddl.sync_index(''''' || thepref || 'vid_keywords_idx'''');'', SYSDATE); END IF; END;';
...
--body
execute immediate
'CREATE OR REPLACE PACKAGE BODY ' || theschema || '.' || thepref || 'similar AS procedure img_colour (theimgid IN number, thelang IN number, rowmax IN number, rowmin IN number, thelic IN varchar2, rc OUT TableRows ) IS compare_sig ORDSYS.ORDImageSignature; begin SELECT t.imagesignature INTO compare_sig FROM ' || thepref || 'images t WHERE t.img_id = theimgid ; case thelic when ''all'' then open rc for SELECT rn, img_id fid, img_custom_id, img_description fdesc, img_publisher, img_license, howmany FROM ( SELECT ROWNUM AS rn, img_id, img_custom_id, img_publisher, img_license, img_description, howmany FROM ( SELECT i.img_id, i.img_custom_id, i.img_publisher, i.img_license, it.img_description, count(*) over() howmany FROM ' || thepref || 'images i LEFT JOIN ' || thepref || 'images_text it ON it.img_id_r = i.img_id AND it.lang_id_r = thelang WHERE ORDSYS.IMGSimilar(i.imagesignature, compare_sig,''color="0.4" texture="0.1" shape="0.4" location="0.1"'', 25)=1 AND lower(i.img_online) = ''t'' AND (lower(i.img_license) = ''royalty-free'' OR lower(i.img_license) = ''rights-managed'') GROUP BY img_id, img_custom_id, img_publisher, img_license, img_description) WHERE ROWNUM <= rowmax) WHERE rn > rowmin; when ''rf'' then open rc for SELECT rn, img_id fid, img_custom_id, img_description fdesc, img_publisher, img_license, howmany FROM (SELECT ROWNUM AS rn, img_id, img_custom_id, img_publisher, img_license, img_description, howmany FROM (SELECT i.img_id, i.img_custom_id, i.img_publisher, i.img_license, it.img_description, count(*) over() howmany FROM ' || thepref || 'images i LEFT JOIN ' || thepref || 'images_text it ON it.img_id_r = i.img_id AND it.lang_id_r = thelang 
WHERE ORDSYS.IMGSimilar(i.imagesignature, compare_sig,''color="0.4" texture="0.1" shape="0.4" location="0.1"'', 25)=1 AND lower(i.img_online) = ''t'' AND lower(i.img_license) = ''royalty-free'' GROUP BY img_id, img_custom_id, img_publisher, img_license, img_description) WHERE ROWNUM <= rowmax) WHERE rn > rowmin; when ''rm'' then open rc for SELECT rn, img_id fid, img_custom_id, img_description fdesc, img_publisher, img_license, howmany FROM (SELECT ROWNUM AS rn, img_id, img_custom_id, img_publisher, img_license, img_description, howmany FROM (SELECT i.img_id, i.img_custom_id, i.img_publisher, i.img_license, it.img_description, count(*) over() howmany FROM ' || thepref || 'images i LEFT JOIN ' || thepref || 'images_text it ON it.img_id_r = i.img_id AND it.lang_id_r = thelang WHERE ORDSYS.IMGSimilar(i.imagesignature, compare_sig,''color="0.4" texture="0.1" shape="0.4" location="0.1"'', 25)=1 AND lower(i.img_online) = ''t'' AND lower(i.img_license) = ''rights-managed'' GROUP BY img_id, img_custom_id, img_publisher, img_license, img_description) WHERE ROWNUM <= rowmax) WHERE rn > rowmin; end case; END img_colour; END ' || thepref || 'similar;';
...
-- IMPORT VIDEO
--------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE &theuser..import_video
(nextid in number, filename in varchar2, filenamepreview in varchar2, filenameimage in varchar2, filenameimageorg in varchar2, thedir in varchar2, tabname in varchar2)
as
obj ORDSYS.ORDvideo;
preview ORDSYS.ORDvideo;
img ORDSYS.ORDimage;
imgorg ORDSYS.ORDimage;
ctx raw(4000):=null;
width integer;
height integer;
...
-- Initialize the video record
EXECUTE IMMEDIATE
'INSERT INTO ' || tabname || ' (vid_id, video, video_preview, video_image, video_image_org) VALUES (' || nextid || ', ordsys.ORDvideo.init(''http'',''' || thedir || ''', ''' || filename || '''), ordsys.ORDvideo.init(''http'',''' || thedir || ''', ''' || filenamepreview || '''), ordsys.ORDimage.init(''http'',''' || thedir || ''', ''' || filenameimage || '''), ordsys.ORDimage.init(''http'',''' || thedir || ''', ''' || filenameimageorg || ''')) RETURNING video, video_preview, video_image, video_image_org INTO :1, :2, :3, :4'
RETURNING INTO obj, preview, img, imgorg;

-- Import the two videos and the image
obj.import(ctx);
preview.import(ctx);
img.import(ctx);
imgorg.import(ctx);

-- Update the record and insert the two videos and the image
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET video = :1, video_preview = :2, video_image = :3, video_image_org = :4 WHERE vid_id = ' || nextid USING obj, preview, img, imgorg;

-- GET PROPERTIES OF VIDEO NOW

-- Select the video record again

EXECUTE IMMEDIATE
'SELECT video FROM ' || tabname || ' WHERE vid_id = ' || nextid INTO obj;

frameResolution := obj.getFrameResolution();
frameRate := obj.getFrameRate();
videoDuration := obj.getVideoDuration();
numberOfFrames := obj.getNumberOfFrames();
compressionType := obj.getCompressionType();
numberOfColors := obj.getNumberOfColors();
bitRate := obj.getBitRate();
mimetype := obj.getMimeType();

-- Update record with the above values
EXECUTE IMMEDIATE
'UPDATE ' || tabname || ' SET vid_frameResolution = :1, vid_framerate = :2, vid_videoDuration = :3, vid_numberOfFrames = :4, vid_compressionType = :5, vid_numberOfColors = :6, vid_bitRate = :7, vid_mimetype = :8 WHERE vid_id = ' || nextid using frameResolution, framerate, videoDuration, numberOfFrames, compressionType, numberOfColors, bitRate, mimetype;
...
-- GET VIDEO
--------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE &theuser..Get_Video
(ID IN VARCHAR2, tabname IN VARCHAR2, colname IN VARCHAR2, review in varchar2)
AS
  localObject ORDSYS.ORDVIDEO;
  localBlob  BLOB;
  localBfile BFILE;
  httpStatus NUMBER;
  lastModDate VARCHAR2(256);
...
BEGIN
  ...
  BEGIN
  EXECUTE IMMEDIATE
  'select mtbl.' || colname || ' FROM ' || tabname || ' mtbl WHERE mtbl.vid_id =' || ID INTO localObject;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      ordplsgwyutil.resource_not_found( 'ID', ID );
      RETURN;
...
-- GET VIDEO IMAGE
--------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE &theuser..get_video_image
(ID IN VARCHAR2, tabname IN VARCHAR2, colname IN VARCHAR2, review in varchar2)
AS
  localObject ORDSYS.ORDIMAGE;
  localBlob  BLOB;
...
--------------------------------------------------------------------------------
create or replace
FUNCTION       &theuser..datetostring (datefield date)
RETURN VARCHAR2
AS
BEGIN
	IF datefield IS NULL THEN
		RETURN NULL;
	ELSE
		RETURN '{ts ' || to_char(datefield, '''yyyy-mm-dd hh24:mi:ss''}');
	END IF;
...
--------------------------------------------------------------------------------
-- Create Stored Procedure ExecInsertIntoSQL
--------------------------------------------------------------------------------
create or replace
PROCEDURE       &theuser..exec_insertinto_sql( sqlstatement IN VARCHAR2 )
AS
BEGIN   
